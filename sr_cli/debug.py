"""``sr debug`` debug tools for SR Agent.

Currently supports:
    sr debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.sr/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[sr debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.sr/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``sr debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``sr debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `sr debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "sr-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``sr debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.sr/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``sr debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "sr-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from sr_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_sr_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``sr dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``sr dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "sr-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``sr debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "sr debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `sr debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  sr debug delete <url>")

    print("\nShare these links with the SR team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/SR version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``sr debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from sr_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "sr debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `sr debug share --local` to print the report instead, "
            "or `sr debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: sr debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'sr debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``sr debug`` call.
    ...
    # best-effort — any failure is swallowed so ``sr debug`` stays
