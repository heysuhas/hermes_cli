"""Migrate SR' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads SR' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  SR mcp_servers.<n>.command/args/env  → codex stdio transport
  SR mcp_servers.<n>.url/headers       → codex streamable_http transport
  SR mcp_servers.<n>.timeout           → codex tool_timeout_sec
  SR mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  SR-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. SR' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in SR config.")
...
# SR keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # SR' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, sr_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one SR MCP server config to the codex inline-table dict
    ...
    has_command = bool(sr_cfg.get("command"))
    has_url = bool(sr_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(sr_cfg["command"])
        args = sr_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = sr_cfg.get("env") or {}
        if env:
        ...
        out["url"] = str(sr_cfg["url"])
        headers = sr_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # SR' transport: sse hint is informational; codex auto-negotiates
        if sr_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in sr_cfg:
        try:
            out["tool_timeout_sec"] = float(sr_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in sr_cfg:
        try:
            out["startup_timeout_sec"] = float(sr_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if sr_cfg.get("enabled") is False:
        out["enabled"] = False
    ...
    for key in sr_cfg:
        ...
        elif key not in _KNOWN_SR_KEYS:
            skipped.append(f"{key} (unknown SR key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (SR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by SR)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert SR' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before SR' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior SR
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="sr-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a SR_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed sr-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.sr`` symlink that happens to live under ``/private/var/folders``
...
def _build_sr_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches SR' own
    ...
    so a sr installed under /opt/, /usr/local/, or a venv all work.
    SR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # SR_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_sr_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever SR_HOME its launcher sets
    ...
    # — that would override the launcher's SR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("SR_HOME", tmp_path) would
    ...
    sr_home = os.environ.get("SR_HOME") or ""
    if sr_home and _looks_like_test_tempdir(sr_home):
        sr_home = ""
    if sr_home:
        env["SR_HOME"] = sr_home
    # PYTHONPATH passes through so a worktree-launched sr finds the
    ...
    env["SR_QUIET"] = "1"
    env["SR_REDACT_SECRETS"] = env.get("SR_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_sr_tools: bool = True,
) -> MigrationReport:
    """Translate SR mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        sr_config: full ~/.sr/config.yaml dict
    ...
    sr_servers = (sr_config or {}).get("mcp_servers") or {}
    if not isinstance(sr_servers, dict):
        report.errors.append(
            "mcp_servers in SR config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in sr_servers.items():
    ...
    # Inject SR' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into SR for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/sr_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_sr_tools:
        translated["sr-tools"] = _build_sr_tools_mcp_entry()
        if "sr-tools" not in report.migrated:
            report.migrated.append("sr-tools")
