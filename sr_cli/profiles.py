"""
Profile management for multiple isolated SR instances.

Each profile is a fully independent SR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.sr/profiles/<name>/`` by default.

The "default" profile is ``~/.sr`` itself — backward compatible,
zero migration needed.

Usage::

    sr profile create coder          # fresh profile + bundled skills
    sr profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sr profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sr -p coder chat                 # or via flag
    sr profile use coder             # set as sticky default
    sr profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See sr_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `sr profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `sr
...
# `sr skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         SR infrastructure directories that only the default profile
         (``~/.sr``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_sr_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sr) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sr-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when SR_HOME equals the
...
# into the export. The set below identifies the *known SR profile
# artifacts* at the root of SR_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``sr_constants``.
...
_RESERVED_NAMES = frozenset({
    "sr", "default", "test", "tmp", "root", "sudo",
})

# SR subcommands that cannot be used as profile names/aliases
_SR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the sr root, NOT to the current SR_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SR_HOME points outside
    ``~/.sr``, profiles live under ``SR_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_sr_home() / "profiles"


def _get_default_sr_home() -> Path:
    """Return the default (pre-profile) SR_HOME path.

    In standard deployments this is ``~/.sr``.
    In Docker/custom deployments where SR_HOME is outside ``~/.sr``
    (e.g. ``/opt/data``), returns SR_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sr_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the SR installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SR_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_sr_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, sr subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _SR_SUBCOMMANDS:
        return f"'{canon}' conflicts with a sr subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            sr_exe = shutil.which("sr") or "sr"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(sr_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running SR. If we
    ...
    try:
        ...
        from sr_cli.config import check_config_version, migrate_config
        ...
        # not be migrated. The next `sr doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``sr -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``sr profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``sr -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``sr profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``SR_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing SR config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sr_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, sr_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own SR_HOME.
    ...
    The returned ``sr_home`` is the path to pass to
    ``set_sr_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_sr_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``sr update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sr)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sr/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `sr -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this SR profile.\n"
    ...
    if not soul_path.exists():
        try:
            from sr_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `sr update`'s
    ...
    # Cloned configs can be older than the running SR (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `sr profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `sr -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SR_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``sr profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SR_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_sr_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running SR *backends* bound to this profile.
    ...
    names it, so find it by inspection: a SR backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``SR_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `sr -p <canon> profile
    ...
    sr_markers = ("sr_cli.main", "sr-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a SR process: either an entrypoint marker in argv, or
            # a resolved executable named `sr`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_sr = (
                any(marker in joined for marker in sr_markers)
                or exe_name == "sr"
                or exe_name.startswith("sr")
            )
            if not is_sr:
                continue
            ...
            # ...or by SR_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("SR_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sr).\n"
            "To remove everything, use: sr uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_sr_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``SR_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from sr_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SR_HOME")
    try:
        os.environ["SR_HOME"] = str(profile_dir)
        from sr_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SR_HOME"] = old_home
        elif "SR_HOME" in os.environ:
            del os.environ["SR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sr/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: sr profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SR_HOME.

    Returns ``"default"`` if SR_HOME is not set or points to ``~/.sr``.
    Returns the profile name if SR_HOME points into ``~/.sr/profiles/<name>``.
    Returns ``"custom"`` if SR_HOME is set to an unrecognized path.
    """
    from sr_constants import get_sr_home
    sr_home = get_sr_home()
    resolved = sr_home.resolve()

    default_resolved = _get_default_sr_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      SR_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-SR file the
      user may have lying alongside SR_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.sr itself — its parent is ~/ and its
        # directory name is ".sr", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sr profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sr itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sr). "
            "Specify a different name: sr profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="sr_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"sr_{old_name}"
    legacy_old_host = f"sr.{old_name}"
    new_host = f"sr_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_sr_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SR_HOME path string.

    Called early in the CLI entry point, before any sr modules
    are imported, to set the SR_HOME environment variable.
    """
