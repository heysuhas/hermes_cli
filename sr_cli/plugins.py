...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with sr-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.sr/plugins/<name>/``
3. **Project plugins** – ``./.sr/plugins/<name>/`` (opt-in via
   ``SR_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``sr_agent.plugins``
   entry-point group.
...
from sr_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``SR_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("SR_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``SR_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.sr/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("SR_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When SR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other SR loggers.
    """
    ...
    logger.debug(
        "SR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. SR' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by sr_cli.kanban_db when a task
    ...
    # separate `sr -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `sr kanban dispatch`),
...
ENTRY_POINTS_GROUP = "sr_agent.plugins"

_NS_PARENT = "sr_plugins"
...
class PluginManifest:
    ...
    #              in ~/.sr/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``sr plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active SR profile name (e.g. ``"default"``).

        Derived from ``SR_HOME`` via
        :func:`sr_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.sr/profiles/<name>``, or ``"custom"`` when
        ``SR_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with SR core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``sr honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``sr <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`sr_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from sr_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_sr_dotenv()`` startup — after
        ``~/.sr/.env`` loads, before SR reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        SR' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``sr model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: sr_cli.main imports plugins indirectly
        from sr_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.sr/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("SR_SAFE_MODE"):
            logger.info("SR_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to sr_cli/. Two layouts are
        ...
        # 2. User plugins (~/.sr/plugins/)
        user_dir = get_sr_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.sr/plugins/)
        if _env_enabled("SR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".sr" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set SR_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with sr and must
            ...
            # all ~20 of them added several seconds to every `sr`
            # invocation — including plain `sr chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform SR ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `sr plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `sr plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``sr_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``sr_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``sr tools`` TUI so plugin-provided toolsets appear
