"""
Single source of truth for provider identity in SR Agent.
...
2. **SR overlays** — transport type, auth patterns, aggregator flags,
...
# -- SR overlay ----------------------------------------------------------
# SR-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class SROverlay:
    """SR-specific provider metadata layered on top of models.dev."""
...
SR_OVERLAYS: Dict[str, SROverlay] = {
    "moa": SROverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": SROverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nous": SROverlay(
        ...
        base_url_override="https://inference-api.samsung.com/v1",
    ),
    "openai-codex": SROverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": SROverlay(
        transport="codex_responses",
    ...
    "qwen-oauth": SROverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SR_QWEN_BASE_URL",
    ),
    "lmstudio": SROverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": SROverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": SROverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "sr", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. SR overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + SR overlay
    ...
    overlay = SR_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + sr extra
    ...
    if overlay is not None:
        # SR-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in SR_OVERLAYS
    if provider == "bedrock":
