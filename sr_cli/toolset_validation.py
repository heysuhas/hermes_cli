...
the tool registry or launching SR (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``sr-cli`` to the non-existent ``sr``. ``resolve_toolset('sr')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``sr-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``sr`` instead of ``sr-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `sr tools` to reconfigure."
        )
    return warnings
