...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for SR Agent. Use 'sr debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sr debug share              Upload debug report (asks for confirmation)
    sr debug share --yes        Skip confirmation (for scripts/CI)
    sr debug share --lines 500  Include more log lines
    sr debug share --expire 30  Keep paste for 30 days
    sr debug share --local      Print report locally (no upload)
    sr debug share --no-redact  Disable upload-time secret redaction
    sr debug share --nous       Upload to Nous-internal storage (private)
    sr debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sr debug share'",
    )
