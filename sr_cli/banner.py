...
Pure display functions with no SRCLI state dependency.
"""
import json
import logging
import os
...
from sr_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built sr — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/heysuhas/hermes_cli.git"
_OFFICIAL_REPO_CANONICAL = "github.com/heysuhas/hermes_cli"
...
def _fetch_pypi_latest(package: str = "sr-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a SR update is available.
    (Disabled for corporate air-gapped environment)
    """
    return None


def _resolve_repo_dir() -> Optional[Path]:
    """Return the active SR git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SR_HOME/sr-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
        sr_home = get_sr_home()
        repo_dir = sr_home / "sr-agent"
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``sr_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from sr_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/heysuhas/hermes_cli/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    SR checkout. Cached per-process. Release URL always points at the
    canonical SamsungResearch/sr-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"SR Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else SR_CADUCEUS
    except Exception:
        _bskin = None
        _hero = SR_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Samsung Research[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Samsung Research[/]")

    if os.getenv("SR_YOLO_MODE"):
    ...
    try:
        from sr_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from sr_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built sr; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else SR_AGENT_LOGO
        console.print(_logo)
        console.print()
