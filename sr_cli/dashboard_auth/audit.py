...
Profile-aware location: ``$SR_HOME/logs/dashboard-auth.log``.
...
from ``sr_constants`` or other sr_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$SR_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``sr_constants.get_sr_home`` semantics: env var wins,
    else ``~/.sr``. A local copy avoids an import cycle with the
    middleware which lives below ``sr_cli``.
    """
    home = os.environ.get("SR_HOME") or str(Path.home() / ".sr")
