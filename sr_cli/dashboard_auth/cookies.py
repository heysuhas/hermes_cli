...
Three cookies in play:
  ...
  - sr_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-sr_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "sr_session_at"
SESSION_RT_COOKIE = "sr_session_rt"
PKCE_COOKIE = "sr_session_pkce"
...
SSO_ATTEMPT_COOKIE = "sr_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /sr`` we want ``Path=/sr`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (sr #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/sr``)
