...
- ``sr_cli.providers``        -- canonical provider identity + overlays
- ``sr_cli.model_normalize``  -- per-provider name formatting
...
_SR_MODEL_WARNING = (
    "Samsung Research SR 3 & 4 models are NOT agentic and are not designed "
    "for use with SR Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Samsung Research SR 3 / SR 4 chat families.
# The previous substring check (`"sr" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``sr-brain:qwen3-14b-ctx16k`` that just
# happen to carry "sr" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   SamsungResearch/SR-3-Llama-3.1-70B, sr-4-405b, openrouter/sr3:70b
# Negative examples it must NOT match:
#   sr-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_SR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])sr[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_sr_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous SR 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_SR_NON_AGENTIC_RE.search(model_name))


def _check_sr_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous SR 3/4 chat model."""
    if is_nous_sr_non_agentic(model_name):
        return _SR_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``sr config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from sr_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sr model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'sr doctor' — config issues detected:"
        ...
        from sr_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from sr_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from sr_cli.config import load_config
            from sr_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from sr_cli.providers import resolve_user_provider as _ruser
    ...
    # sr_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from sr_cli.models import normalize_opencode_base_url
    ...
    sr_warn = _check_sr_model_warning(new_model)
    if sr_warn:
        warnings.append(sr_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from sr_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from sr_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from sr_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from sr_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by sr provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://sr-agent.samsung.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a SR release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from sr_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from sr_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check SR-mapped providers ---
    from sr_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from sr_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for sr_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(sr_id)
        if (
            _alias_target
            and _alias_target != sr_id
        ...
        pconfig = PROVIDER_REGISTRY.get(sr_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (SR_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers SR may not route yet.
        ...
        from sr_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(sr_id):
            continue
        ...
        if not has_creds:
            try:
                from sr_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(sr_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        sr_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `sr model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(sr_id)
        if not model_ids:
            model_ids = curated.get(sr_id, [])
            if sr_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(sr_id, model_ids)
    ...
    # --- 2. Check SR-only providers (nous, openai-codex, copilot, opencode-go) ---
    from sr_cli.providers import SR_OVERLAYS
    from sr_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → SR provider ID.
    # SR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use SR IDs ("copilot").
    _mdev_to_sr = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in SR_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve SR slug — e.g. "github-copilot" → "copilot"
        sr_slug = _mdev_to_sr.get(pid, pid)
        if sr_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(sr_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, sr_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or sr_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(sr_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", sr_slug, exc)
        ...
        # configured.
        if not has_creds and sr_slug == "anthropic":
            try:
                ...
                if (sr_creds and sr_creds.get("accessToken")) or \
        ...
        if sr_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(sr_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(sr_slug)
                model_ids = _ids if _ids else (curated.get(sr_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(sr_slug, []) or curated.get(pid, [])
        elif sr_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `sr model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from sr_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(sr_slug, []) or curated.get(pid, [])
                if sr_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(sr_slug, model_ids)
        total = len(model_ids)
        if sr_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": sr_slug,
            "name": get_label(sr_slug),
            "is_current": sr_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "sr",
        ...
        seen_slugs.add(sr_slug.lower())
        _record_builtin_endpoint(sr_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or SR_OVERLAYS (keeps /model in sync
    # with `sr model`).
    try:
        from sr_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is SR's canonical write key (matches
            ...
            # SR writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from sr_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # SR's own writer uses; a hyphen variant is accepted
            ...
            # active model. SR's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, sr_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from sr_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`sr_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from sr_cli.models import fetch_openrouter_models
