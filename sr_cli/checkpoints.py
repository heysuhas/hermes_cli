"""`sr checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sr/checkpoints/``.  Actions:

    sr checkpoints               # same as `status`
    sr checkpoints status        # total size, project count, breakdown
    sr checkpoints list          # per-project checkpoint counts + workdir
    sr checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sr checkpoints clear [-f]    # nuke the entire base (asks first)
    sr checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sr checkpoints
    sr checkpoints prune --retention-days 3 --max-size-mb 200
    sr checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sr checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sr checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sr checkpoints` → status
