#!/usr/bin/env python3
"""
SR CLI - Main entry point.

Usage:
    sr                     # Interactive chat (default)
    sr chat                # Interactive chat
    sr gateway             # Run gateway in foreground
    sr gateway start       # Start gateway as service
    sr gateway stop        # Stop gateway service
    sr gateway status      # Show gateway status
    sr gateway install     # Install gateway service
    sr gateway uninstall   # Uninstall gateway service
    sr setup               # Interactive setup wizard
    sr logout              # Clear stored authentication
    sr status              # Show status of all components
    sr cron                # Manage cron jobs
    sr cron list           # List cron jobs
    sr cron status         # Check if cron scheduler is running
    sr doctor              # Check configuration and dependencies
    sr honcho setup                    # Configure Honcho AI memory integration
    sr honcho status                   # Show Honcho config and connection status
    sr honcho sessions                 # List directory → session name mappings
    sr honcho map <name>               # Map current directory to a session name
    sr honcho peer                     # Show peer names and dialectic settings
    sr honcho peer --user NAME         # Set user peer name
    sr honcho peer --ai NAME           # Set AI peer name
    sr honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sr honcho mode                     # Show current memory mode
    sr honcho mode [hybrid|honcho|local]  # Set memory mode
    sr honcho tokens                   # Show token budget settings
    sr honcho tokens --context N       # Set session.context() token cap
    sr honcho tokens --dialectic N     # Set dialectic result char cap
    sr honcho identity                 # Show AI peer identity representation
    sr honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sr honcho migrate                  # Step-by-step migration guide: OpenClaw native → SR + Honcho
    sr version             Show version
    sr update              Update to latest version
    sr uninstall           Uninstall SR Agent
    sr acp                 Run as an ACP server for editor integration
    sr sessions browse     Interactive session picker with search
...
# When the user upgrades code via ``git pull`` (or ``sr update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``sr_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, sr crashes on import and the user can't run
# ``sr update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import sr_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'sr' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``sr tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``sr.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("sr")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"sr", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"sr")
        # Windows: the .exe name is already ``sr.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* sr_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("SR_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".sr", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``SR_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``sr … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("SR_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``SR_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("SR_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`sr --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from sr_cli import __release_date__, __version__

    print(f"SR Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``sr --version`` before config/logging imports on Termux."""
    if os.environ.get("SR_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from sr_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from sr_cli.subcommands.cron import build_cron_parser
from sr_cli.subcommands.gateway import build_gateway_parser
from sr_cli.subcommands.profile import build_profile_parser
from sr_cli.subcommands.model import build_model_parser
from sr_cli.subcommands.setup import build_setup_parser
from sr_cli.subcommands.postinstall import build_postinstall_parser
from sr_cli.subcommands.whatsapp import build_whatsapp_parser
from sr_cli.subcommands.slack import build_slack_parser
from sr_cli.subcommands.login import build_login_parser
from sr_cli.subcommands.logout import build_logout_parser
from sr_cli.subcommands.auth import build_auth_parser
from sr_cli.subcommands.status import build_status_parser
from sr_cli.subcommands.webhook import build_webhook_parser
from sr_cli.subcommands.hooks import build_hooks_parser
from sr_cli.subcommands.doctor import build_doctor_parser
from sr_cli.subcommands.security import build_security_parser
from sr_cli.subcommands.dump import build_dump_parser
from sr_cli.subcommands.debug import build_debug_parser
from sr_cli.subcommands.backup import build_backup_parser
from sr_cli.subcommands.import_cmd import build_import_cmd_parser
from sr_cli.subcommands.config import build_config_parser
from sr_cli.subcommands.console import build_console_parser
from sr_cli.subcommands.version import build_version_parser
from sr_cli.subcommands.update import build_update_parser
from sr_cli.subcommands.uninstall import build_uninstall_parser
from sr_cli.subcommands.dashboard import build_dashboard_parser
from sr_cli.subcommands.gui import build_gui_parser
from sr_cli.subcommands.logs import build_logs_parser
from sr_cli.subcommands.prompt_size import build_prompt_size_parser
from sr_cli.subcommands.memory import build_memory_parser
from sr_cli.subcommands.acp import build_acp_parser
from sr_cli.subcommands.tools import build_tools_parser
from sr_cli.subcommands.insights import build_insights_parser
from sr_cli.subcommands.skills import build_skills_parser
from sr_cli.subcommands.pairing import build_pairing_parser
from sr_cli.subcommands.plugins import build_plugins_parser
from sr_cli.subcommands.mcp import build_mcp_parser
from sr_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (sr tools, sr setup, sr model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sr {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any sr module import.
#
# Many modules cache SR_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SR_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.sr/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SR_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `sr mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to SR' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo sr -p <name>` against the invoking user's home.
        ...
        candidate = home / ".sr" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`sr chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors sr_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SR_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.sr/profiles/coder or
    # /opt/data/profiles/coder).  If SR_HOME points to the sr root
    # instead (e.g. systemd hardcodes SR_HOME=/root/.sr), we must
    # still read active_profile — the user may have switched profiles via
    # `sr profile use` and the gateway should honour that choice.
    # See issue #22502.
    sr_home_env = os.environ.get("SR_HOME", "")
    if profile_name is None and sr_home_env:
        if Path(sr_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the sr root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as SR_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``sr gateway run`` to mean
    # "the root SR_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("SR_S6_SUPERVISED_CHILD"):
        try:
            from sr_constants import get_default_sr_root

            active_path = get_default_sr_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SR_HOME
    if profile_name is not None:
        try:
            from sr_cli.profiles import resolve_profile_env

            sr_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            sr_home = _resolve_sudo_user_profile_env(profile_name)
            if not sr_home:
            ...
            # A bug in profiles.py must NEVER prevent sr from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SR_HOME"] = sr_home
...
# Load .env from ~/.sr/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from sr_cli.config import get_sr_home
from sr_cli.env_loader import load_sr_dotenv

load_sr_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → SR_REDACT_SECRETS env
# var BEFORE sr_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_sr_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from sr_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "SR_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["SR_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `sr` subcommands
...
try:
    from sr_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from sr_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if sr_constants not importable yet
...
from sr_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to sr_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (sr_cli.main._model_flow_*) keep resolving unchanged.
from sr_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_sr_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("SR_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("SR_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from sr_cli.config import get_env_path, get_sr_home, load_config
    from sr_cli.auth import get_auth_status

    # Determine whether SR itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from sr_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_sr_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from sr_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_sr_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if SR has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants SR to use their tokens.
    if _has_sr_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, sr_bin
        cli_args: the original CLI arguments (everything after 'sr')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    sr_bin = container_info["sr_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sr {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sr {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  sr --tui --resume {target}")
    if title:
        print(f'  sr --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sr/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@sr" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/sr-ink/package.json",
    "packages/sr-ink/index.js",
    "packages/sr-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``SR_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("SR_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``SR_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SR_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    sr_home = os.environ.get("SR_HOME") or str(Path.home() / ".sr")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SR_HOME": sr_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(sr_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(sr_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if sr_cli_dir is None:
        sr_cli_dir = Path(__file__).parent
    bundled = sr_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``sr update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("SR_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this SR checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `sr update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the SR checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `sr --tui`\n"
        "If the checkout is still inconsistent, run `sr update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SR_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SR_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from sr_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("SR_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SR_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SR_TUI_DIR (e.g. `unset SR_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @sr/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @sr/ink resolves through
        # packages/sr-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "sr-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sr_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from sr_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sr-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SR_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SR_PYTHON_SRC_ROOT"] = os.environ.get(
        "SR_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SR_PYTHON", sys.executable)
    env.setdefault("SR_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SR_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SR_MODEL"] = model
        env["SR_INFERENCE_MODEL"] = model
    if provider:
        env["SR_TUI_PROVIDER"] = provider
        env["SR_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SR_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SR_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SR_TUI_SKILLS"] = value
    if query:
        env["SR_TUI_QUERY"] = query
    if image:
        env["SR_TUI_IMAGE"] = image
    if checkpoints:
        env["SR_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SR_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SR_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SR_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SR_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SR_ACCEPT_HOOKS"] = "1"
    ...
    # SR_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `sr --tui` try to
    ...
    # still set SR_TUI_RESUME themselves.
    env.pop("SR_TUI_RESUME", None)
    if resume_session_id:
        env["SR_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `sr update` so
    ...
    if code == 42:
        from sr_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SR_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``sr kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``sr kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SR_KANBAN_BOARD"):
        return
    try:
        from sr_cli.kanban_db import get_current_board

        os.environ["SR_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.sr/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with SR — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. SR still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``sr … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("SR_TUI") == "1":
        return True
    try:
        from sr_cli.config import load_config
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sr sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from sr_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from sr_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'sr doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like SR isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  sr setup")
        print()

        from sr_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'sr setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sr_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["SR_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.sr/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SR_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SR_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SR_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from sr_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from sr_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from sr_cli.config import get_env_value, save_env_value
    from sr_constants import find_node_executable, with_sr_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with SR?")
        print()
    ...
    # has no creds.json.  Every subsequent `sr gateway` then paid a 30s
    ...
    session_dir = get_sr_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: sr gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `sr gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sr gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ SR Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sr gateway")
        ...
        print()
        print("  Or install as a service: sr gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sr whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``sr whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``sr_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from sr_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from sr_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from sr_cli.config import stamp_install_method
    from sr_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ SR post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from sr_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sr model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from sr_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from sr_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from sr_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SR_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from sr_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sr model' for "
                "available providers, or run 'sr doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in sr_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.sr/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sr/.env if the active provider is not 'custom'.
    ...
    """
    from sr_cli.config import get_env_value, save_env_value, load_config
...
#
# SR uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `sr model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sr model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`sr_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from sr_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from sr_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  SR only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``sr model`` flow, then route aux tasks to them here.
    """
    from sr_cli.config import load_config
    from sr_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from sr_cli.auth import _prompt_model_selection
    from sr_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from sr_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from sr_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from sr_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use SR URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from sr_cli.curses_ui import curses_radiolist
...
# downstream call sites read `sr_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `sr` invocation — including
# fast paths like `sr --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from sr_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``sr setup`` / ``sr model``.
    ...
    recover from a malformed paste without editing ``~/.sr/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from sr_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from sr_cli.config import save_env_value
    from sr_cli.secret_prompt import masked_secret_prompt
    ...
    from sr_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `sr setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from sr_constants import display_sr_home as _dhh_fn

            print(
                f"    SR will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate SR CLI with a provider."""
    from sr_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from sr_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sr_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from sr_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from sr_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from sr_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``sr slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: sr slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sr slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from sr_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sr_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from sr_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from sr_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from sr_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `sr security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from sr_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from sr_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from sr_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from sr_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up SR home directory to a zip file."""
    if getattr(args, "quick", False):
        from sr_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from sr_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a SR backup from a zip file."""
    from sr_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from sr_cli.config import detect_install_method
    from sr_cli.banner import format_banner_version_label
    ...
    try:
        from sr_cli.banner import check_for_updates
        from sr_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall SR Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from sr_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from sr_cli.uninstall import run_gui_uninstall
    ...
    from sr_cli.uninstall import run_uninstall
...
# Critical files that SR must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``sr`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="sr-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sr update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``sr_cli/web_dist/`` (per vite.config.ts
    outDir: "../sr_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "sr_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``sr`` launcher present but ``sr_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the sr venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — sr venv python3, no nix-shell overhead
    ...
    # a sr venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sr update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sr web``).
    ...
    # build path usable on a stock `py -m sr_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (sr web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``sr update`` can unconditionally call ``sr desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``sr desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $SR_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $SR_HOME."""
    from sr_constants import get_sr_home
    return get_sr_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``sr update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/SR.app/Contents/MacOS/SR"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "SR.exe",
            release_dir / "win-ia32-unpacked" / "SR.exe",
            release_dir / "win-arm64-unpacked" / "SR.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "sr",
            release_dir / "linux-unpacked" / "SR",
            release_dir / "linux-arm64-unpacked" / "sr",
            release_dir / "linux-arm64-unpacked" / "SR",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/SR'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``SR.exe`` keeps an exclusive lock on
    ``release/win-unpacked/SR.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\SR.exe: Access is
    ...
    an unrelated "SR" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "SR is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../SR.app/Contents/MacOS/SR  ->  app bundle = .../SR.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``sr desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ SR Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ SR Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the SR_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from sr_logging import setup_logging as _setup_logging_gui
    ...
    # with_sr_node_path() copies os.environ when called with no arg.
    env = with_sr_node_path()
    if getattr(args, "fake_boot", False):
        env["SR_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["SR_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "sr_root", None):
        env["SR_DESKTOP_SR_ROOT"] = str(Path(args.sr_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["SR_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["SR_DESKTOP_CWD"] = os.getcwd()
    ...
    # `SR_DESKTOP_DISABLE_GPU=... sr desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "SR_DESKTOP_DISABLE_GPU" not in os.environ:
        env["SR_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  sr gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds SR.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose SR.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on SR.exe, close any")
                    print("  running SR desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> sr desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "SR is
    ...
    if source_mode:
        print("→ Launching SR Desktop from source build...")
    ...
    print(f"→ Launching packaged SR Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``sr dashboard`` processes other than ourselves.

    ``sr dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sr update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the SR Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``sr serve`` as
    ...
    environment variable ``SR_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "sr dashboard",
        "sr_cli.main dashboard",
        "sr_cli/main.py dashboard",
        # The headless backend (`sr serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "sr serve",
        "sr_cli.main serve",
        "sr_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            from sr_cli._subprocess_compat import windows_hide_flags
            ...
            # (rather than `pgrep -f "sr.*dashboard"`) keeps us consistent
            # with `sr_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sr update`.
    ...
    print("  Preview now:  sr curator run --dry-run")
    print("  Pause it:     sr curator pause")
    print(
        "  Docs:         https://sr-agent.samsung.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``sr update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``sr update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: sr curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``sr dashboard`` processes.

    Called at the end of ``sr update`` (default ``reason``) and also
    from ``sr dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the SR Desktop Electron app spawns this dashboard as a
    # backend child, it sets SR_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("SR_DESKTOP_CHILD_PID")
    if raw_pid:
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.sr-update-staging"
    backup = f"{dst}.sr-update-old"
...
def _update_via_zip(args):
    """Update SR Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `sr update "
            f"--branch {branch}`, or update against main with `sr update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/SamsungResearch/sr-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="sr-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"sr-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for sr-agent itself
        ...
        # GitHub ZIPs extract to sr-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sr-agent-{branch}")
    ...
    from sr_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: sr skills list-modified  "
    ...
    try:
        from sr_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sr-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if SR behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes sr completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but SR couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but SR couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if SR behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but SR couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but SR couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `sr update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/SamsungResearch/sr-agent.git",
    "git@github.com:SamsungResearch/sr-agent.git",
    "https://github.com/SamsungResearch/sr-agent",
    "git@github.com:SamsungResearch/sr-agent",
}
OFFICIAL_REPO_URL = "https://github.com/SamsungResearch/sr-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sr_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sr_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official SR repository.")
        print("  This means you may miss updates from SamsungResearch/sr-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/SamsungResearch/sr-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/SamsungResearch/sr-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``sr update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from sr_constants import get_default_sr_root

    default_home = get_default_sr_root()
...
# Install-scoped breadcrumb dropped right before ``sr update`` mutates the
...
# survives and the next ``sr`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $SR_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``sr update`` left half-done.
    ...
    protocol stream (``sr acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if sr.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _sr_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        print(
            "⚠ A previous `sr update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``sr update`` is still progressing even if pip/uv itself is silent.
    """
...
def _sr_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_sr_instances(
    ...
    The SR Desktop Electron app spawns ``sr.EXE`` as a backend child,
    so during ``sr update`` the user-invoked process and the desktop's
    ...
    shims (``sr.exe`` / ``sr-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``sr update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``sr.exe``) is a separate process from the Python
    ...
    for shim in _sr_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated sr.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``sr update``
    ...
    #      sr.exe sitting *under* a non-SR parent (e.g. a SR
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "sr.exe"
    lines = ["✗ Another sr.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close SR Desktop, exit any open `sr` REPLs, and")
    lines.append("  stop the gateway (`sr gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `sr update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_sr_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``sr.exe``.
    ...
    when ``sr update`` runs, ``sr.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``sr.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next sr invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the SR Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       SR Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _sr_exe_shims(scripts_dir):
        ...
        print(
            "    Close SR Desktop, exit other `sr` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `sr update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_sr_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``sr.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``sr.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``sr`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_sr_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``sr.exe.old.*`` left by prior updates.

    Called early on every sr invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``sr update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `sr update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``sr.exe`` / ``sr-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_sr_exe`` for the rationale.
    """
    ...
    # hours later inside ``sr update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``sr.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``sr.exe`` shim is locked during ``sr update``, or when uv/distlib
    skips a launcher write. The symptom is ``sr-agent.exe`` and
    ``sr-acp.exe`` present but ``sr.exe`` missing, so ``sr`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `sr update --force` after "
            "closing other sr processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m sr_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``sr update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``sr.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``sr`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `sr update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `sr update --force` after closing other sr processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from sr_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$SR_HOME/bin/uv``, but on Termux the official
    ...
    """
    from sr_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `sr update`, so we install root-only first
    ...
    nixos_env = with_sr_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `sr update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sr update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sr/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sr update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sr update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sr/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sr update --gateway``) the update is already
    ...
    try:
        ...
        # sr_cli.config.get_sr_home to simulate setup failure.
        from sr_cli.config import get_sr_home as _get_sr_home

        logs_dir = _get_sr_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.sr/logs/update.log`` only, never the terminal.

    During ``sr update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``sr update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``sr update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from sr_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from sr_cli.config import recommended_update_command
        from sr_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sr update`` without requiring a reinstall.  The
    ...
    ``sr`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/sr.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v sr'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/sr, code at /usr/local/lib/sr-agent).
    fhs_link = Path("/usr/local/bin/sr")
    ...
    path_comment = (
        "# SR Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SR_HOME before running the update.
    ...
    SR_HOME directories.  The ``--backup`` flag on ``sr update``
    ...
    # ``sr_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # SR_HOME can add minutes to every update. Users who want the
    ...
    try:
        from sr_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_sr_home so the user sees ~/.sr/...
    try:
        from sr_constants import get_sr_home, display_sr_home

        home = get_sr_home()
        try:
            display_path = f"{display_sr_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  sr import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``sr update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``sr update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run sr from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.sr-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".sr-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The sr.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m sr_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``sr
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other SR processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← SR Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the SR desktop app / other SR terminals, then re-run:"
    )
    lines.append("    sr update")
    lines.append("  (or use `sr update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    sr.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from sr_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating SR...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m sr_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: sr gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``sr gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from sr_cli import gateway_windows
        from sr_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``sr update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update SR Agent to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `sr update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update SR via pip (for PyPI installs)."""
    from sr_cli import __version__
    from sr_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "sr-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "sr-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "sr-agent"]
    ...
    print("✓ Update complete! Restart sr to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating SR Agent...")
    print()

    # On Windows, abort early if another sr.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_sr_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `sr serve` backend) will keep .pyd
    ...
    # the sr.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all SR windows/gateways and re-run: sr update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $SR_HOME, this is cheap
        ...
        try:
            from sr_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in sr_cli/config.py bricked
        # every user who ran ``sr update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``sr
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``sr update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_sr_home added to sr_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``sr`` launch finishes the
        ...
        # build.  ``sr desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``sr desktop`` shouldn't be forced into a full
        # Electron build by ``sr update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from sr_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "sr_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `sr desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from sr_constants import display_sr_home as _dhh
        ...
        # current — so copy it straight over ~/.sr/cache/model_catalog.json
        ...
        # modules in this process.  Reload sr_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_sr_home() added since the last release.
        try:
            import importlib
            import sr_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SR_HOME so
        # it is not affected by sync_skills()'s module-level SR_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SR_HOME env var points at the default or a named profile.
        try:
            from sr_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from sr_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'sr config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: sr config migrate")
            else:
                print()
                print("Skipped. Run 'sr config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from sr_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `sr curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``sr update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from sr_cli.tools_config import install_cua_driver
        ...
        # When running as ``sr update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_sr_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... sr-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... sr-gateway*)
            ...
            # Discover all sr-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sr-gateway.service or sr-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `sr update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: sr gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sr gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sr -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy SR gateway unit files are still installed.
        # When both sr.service (from a pre-rename install) and the
        # current sr-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `sr update` surfaces the issue until the user migrates.
        try:
            from sr_cli.gateway import (
                has_legacy_sr_units,
                _find_legacy_sr_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_sr_units():
                print()
                print("⚠ Legacy SR gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_sr_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (sr.service) fight the current")
                print("  sr-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    sr gateway migrate-legacy")
                print()
        ...
        print("  sr model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sr -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from sr_constants import display_sr_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `sr profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → sr -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sr profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sr -p {name} chat")
                else:
        ...
        from sr_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from sr_constants import get_sr_home as _hh
        ...
        from sr_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `sr profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → sr -p {name}  ({wrapper})")
        print()
        ...
        from sr_cli.profiles import profile_exists, validate_alias_name
        ...
        from sr_cli.profiles import rename_profile
        ...
        from sr_cli.profiles import export_profile
        ...
        from sr_cli.profiles import import_profile
        ...
        import tempfile
        from sr_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="sr_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  sr -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      sr -p {plan.manifest.name} chat")
        ...
        from sr_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `sr profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"sr -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from sr_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("sr_requires"):
            print(f"Requires:     SR {data['sr_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from sr_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.sr_requires:
        print(f"  Requires: SR {mf.sr_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``sr dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``sr dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No sr dashboard processes running.")
        return 0

    print(f"{len(pids)} sr dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``sr dashboard register`` for OAuth.
    ...
    try:
        from sr_cli.web_server import should_require_auth
    ...
    try:
        from sr_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `sr dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    sr dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "SR_DASHBOARD_OAUTH_CLIENT_ID into ~/.sr/.env for you.\n"
            "  Docs: https://sr-agent.samsung.com/docs/"
    ...
    try:
        from sr_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sr dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sr update`.
    ...
    # launches the dashboard (`worker dashboard` → SR_HOME points into
    ...
    try:
        from sr_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("SR_DESKTOP") != "1"
    ):
        ...
        # SR_HOME.  We must resolve the root explicitly instead of just
        # dropping SR_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV SR_HOME=/opt/data`), so an unset
        # SR_HOME falls back to $HOME/.sr = /opt/data/.sr — an
        ...
        # update-button guard also misfires).  get_default_sr_root()
        # returns the root for both layouts: ~/.sr for a standard install
        ...
        try:
            from sr_constants import get_default_sr_root
            env["SR_HOME"] = str(get_default_sr_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop SR_HOME) rather than block the reroute.
            env.pop("SR_HOME", None)
    ...
    # the same logs directory as every other SR surface.
    try:
        from sr_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["SR_SERVE_HEADLESS"] = "1"
    elif "SR_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SR_WEB_DIST"])
            if "SR_WEB_DIST" in os.environ
            else PROJECT_ROOT / "sr_cli" / "web_dist"
        )
        ...
        # SR_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["SR_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ SR_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset SR_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads SR_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["SR_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from SR_WEB_DIST: {_dist_root}")
    ...
    try:
        from sr_cli.mcp_startup import start_background_mcp_discovery
    ...
    from sr_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from sr_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from sr_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from sr_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from sr_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter SR log files."""
    from sr_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe SR command console."""
    from sr_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from sr_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``sr -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``sr_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``sr -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``sr`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("SR_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that SR_YOLO_MODE is set before ANY
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from sr_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from sr_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["SR_DEFER_AGENT_STARTUP"] = "1"
            os.environ["SR_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["SR_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `sr --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_sr_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_sr_home()}/memories/\n")
    else:
        from sr_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch SR Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from sr_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from sr_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from sr_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from sr_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from sr_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from sr_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from sr_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from sr_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from sr_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for sr CLI."""
    # Corporate launches enter through sr_cli.corporate, which deliberately
    # supplies an empty argv and owns setup/configuration. Also recognize the
    # install marker so manually launching the managed venv cannot become a
    # back door to the general operator command parser.
    corporate_marker = False
    try:
        from sr_constants import get_sr_home
        corporate_marker = (get_sr_home() / "sr-agent" / ".corporate-install").exists()
    except Exception:
        pass
    if corporate_marker:
        # Only the employee launcher may enter corporate mode. A direct
        # invocation of the developer entry point must not become a second
        # way to launch the managed agent, even when it has no arguments.
        if os.environ.get("SR_CORPORATE_MODE") != "1":
            print("SR is managed by your organization and must be launched from its installed shortcut.")
            raise SystemExit(2)
        if len(sys.argv) == 1:
            try:
                from sr_cli.corporate import _verify_sealed_config
                _verify_sealed_config()
            except RuntimeError as exc:
                print(f"SR cannot start: {exc}")
                raise SystemExit(1) from exc
    if os.environ.get("SR_CORPORATE_MODE") == "1" and len(sys.argv) > 1:
        print("SR is managed by your organization and does not accept command-line arguments.")
        raise SystemExit(2)

    # Cosmetic: make the process show up as 'sr' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from sr_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``sr.exe.old.*`` quarantine files left by previous
    # ``sr update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_sr_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``sr update``
    ...
    # ``sr skills install update``) merely defers recovery one launch;
    # under-matching (missing ``sr -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in sr_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from sr_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from sr_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sr-agent.samsung.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sr model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.sr/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://sr-agent.samsung.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from sr_cli import secrets_cli as _secrets_cli
    from sr_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from sr_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in sr_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in sr_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in sr_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in sr_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `sr whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in sr_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from sr_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in sr_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in sr_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in sr_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in sr_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in sr_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in sr_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from sr_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from sr_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from sr_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in sr_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in sr_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in sr_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in sr_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in sr_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in sr_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.sr/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo sr uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from sr_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in sr_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in sr_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in sr_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in sr_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in sr_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from sr_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in sr_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``sr --help``, ``sr version``, ``sr logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from sr_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from sr_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and SR shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from sr_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from sr_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in sr_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in sr_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `sr computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `sr tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `sr computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the SR app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from sr_cli.tools_config import _cua_driver_cmd
            # Honor SR_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from sr_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: sr computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: sr computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: sr computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: sr computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: sr computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: sr computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in sr_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <sr home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            from sr_state import workspace_key as _ws_key
            ...
            from sr_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from sr_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from sr_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_sr_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from sr_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_sr_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_sr_home() / "sessions"
            ...
            # `sr sessions prune`: no time window and no filters at all
            ...
            # Launch sr --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in sr_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in sr_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in sr_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in sr_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in sr_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in sr_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in sr_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in sr_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The SR-Setup.exe success screen tells users to
    # run `sr desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in sr_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in sr_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in sr_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``sr -c Pokemon Agent Dev`` → ``sr -c 'Pokemon Agent Dev'``
    ...
    from sr_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'sr -c model' where 'model' is consumed as the
    ...
    # --yolo: set SR_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (sr hooks list, cron
