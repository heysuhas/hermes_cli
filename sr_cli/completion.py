"""Shell completion script generation for sr CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_sr_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# SR Agent bash completion
# Add to ~/.bashrc:
#   eval "$(sr completion bash)"

_sr_profiles() {{
    local profiles_dir="$HOME/.sr/profiles"
...
_sr_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sr_profiles)" -- "$cur"))
        return
    fi
...
_sr_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.sr/profiles" ]]; then
        profiles+=($HOME/.sr/profiles/*(N/:t))
    fi
...
_sr() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_sr_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# SR Agent fish completion",
        "# Add to your config:",
        "#   sr completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __sr_profiles",
        "    echo default",
        "    if test -d $HOME/.sr/profiles",
        "        for d in $HOME/.sr/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c sr -f -s p -l profile"
        " -d 'Profile name' -xa '(__sr_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__sr_profiles)' -d 'Profile name'"
                )
