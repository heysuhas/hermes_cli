"""Slash command definitions and autocomplete for the SR CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of SR config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal SR works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while SR is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sr/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update SR Agent to the latest version", "Info"),
    CommandDef("version", "Show SR Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sr_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/sr`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. SR ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full SR
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``sr skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sr skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sr skills list`` and
...
# unrelated command landed. These claim their slots right after /sr,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /sr <command>).
...
# native slot, the alias spelling stays reachable via /sr reset).
...
# ``/sr <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/sr" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /sr credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /sr billing.
#   - moa: high-cost slash mode, available through /sr moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /sr debug on Slack.
_SLACK_VIA_SR_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/sr <verb>`` subcommand.
    ...
    can still reach them via ``/sr <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/sr`` is always reserved as the first entry so the
    legacy ``/sr <subcommand>`` form keeps working for anything that
    ...
    # Reserve /sr as the catch-all top-level command.
    entries.append(("sr", "Talk to SR or run a subcommand", "[subcommand] [args]"))
    seen.add("sr")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_SR_ONLY:
            # Intentionally Slack-via-/sr only (see _SLACK_VIA_SR_ONLY).
            return
    ...
    # everything except /sr, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://sr-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /sr handler.

    Maps both canonical names and aliases so /sr bg do stuff works
    the same as /sr background do stuff.

    Plugin-registered slash commands are included so ``/sr <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from sr_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
