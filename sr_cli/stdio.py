...
on Japanese locales, etc.).  SR's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``sr_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. SR is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the SR-managed Git install directories so
    ...
    # already-running shells don't see the change — which means sr
    ...
    # install without SR-managed PortableGit).
    ...
    # stdin is re-configured for completeness; SR's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and SR's
    ``sr config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching SR (User env var in Windows Settings, or
    export in a PowerShell profile) and SR picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known SR-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\sr\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``sr`` from right after install) retain their old PATH.

    Any subprocess SR spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known SR-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "sr", "git", "cmd"),
        os.path.join(local_appdata, "sr", "git", "bin"),
        os.path.join(local_appdata, "sr", "git", "usr", "bin"),
        # SR venv Scripts directory — host of the sr.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes sr, but harmless to include.
        os.path.join(local_appdata, "sr", "sr-agent", "venv", "Scripts"),
