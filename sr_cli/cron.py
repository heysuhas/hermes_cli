...
# commands at execution time when ``_SR_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: sr gateway install", Colors.DIM))
    print(color("                    sudo sr gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  sr cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'sr cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: sr gateway restart")
        ...
        print("    sr gateway install    # Install as a user service")
        print("    sudo sr gateway install --system  # Linux servers: boot-time system service")
        print("    sr gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: sr cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
