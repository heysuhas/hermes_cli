"""sr hooks — inspect and manage shell-script hooks.

Usage::

    sr hooks list
    sr hooks test <event> [--for-tool X] [--payload-file F]
    sr hooks revoke <command>
    sr hooks doctor

Consent records live under ``~/.sr/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.sr/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``sr hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: sr hooks {list|test|revoke|doctor}")
        print("Run 'sr hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.sr/config.yaml.")
        print("See `sr hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `sr hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `sr hooks test` and `sr hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (SR wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `sr hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `sr hooks doctor` would execute
