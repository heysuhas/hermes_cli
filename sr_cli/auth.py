"""
Multi-provider authentication system for SR Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.sr/auth.json with cross-process file locking.
...
from sr_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_SR_PORTAL_URL = "https://portal.samsung.com"
DEFAULT_SR_INFERENCE_URL = "https://inference-api.samsung.com/v1"
DEFAULT_SR_CLIENT_ID = "sr-cli"
SR_INFERENCE_INVOKE_SCOPE = "inference:invoke"
SR_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_SR_SCOPE = SR_INFERENCE_INVOKE_SCOPE
SR_DEVICE_CODE_SOURCE = "device_code"
SR_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
SR_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from sr_cli import __version__ as _SR_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _SR_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"sr-cli/{_SR_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://sr-agent.samsung.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://sr-agent.samsung.com/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "nous": ProviderConfig(
        ...
        portal_base_url=DEFAULT_SR_PORTAL_URL,
        inference_base_url=DEFAULT_SR_INFERENCE_URL,
        client_id=DEFAULT_SR_CLIENT_ID,
        scope=DEFAULT_SR_SCOPE,
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.sr/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from sr_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from sr_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.sr/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``sr auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `sr model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("SR_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.sr/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_sr_home() / "auth.json"
    # Seat belt: if pytest is running and SR_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch SR_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".sr" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set SR_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom SR_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_sr_root()
    except Exception:
        return None
    profile_home = get_sr_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.sr/auth.json`` even when SR_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_sr_root()`` for a profile-shaped SR_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".sr" / "auth.json"
            try:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``sr auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``sr auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``sr auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in SR.
    ...
    # the user initiated inside SR (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "sr_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `sr logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from sr_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'sr doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'sr model' for available providers, or run 'sr doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `sr auth add openrouter`
    ...
    # `sr auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.sr/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'sr model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.sr/.env.",
...
_SR_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.samsung.com",
})
...
_SR_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.samsung.com",
    "localhost",
    "127.0.0.1",
})
...
def _migrate_stale_nous_portal_url(providers: Dict[str, Any]) -> None:
    ...
    if stored:
        ...
        if parsed.hostname in _SR_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale nous portal_base_url %s -> %s",
                stored, DEFAULT_SR_PORTAL_URL,
            )
            nous["portal_base_url"] = DEFAULT_SR_PORTAL_URL
...
# (SR_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_SR_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.samsung.com",
})
...
def _validate_nous_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``SR_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        ...
        return None
    if parsed.hostname not in _ALLOWED_SR_INFERENCE_HOSTS:
        logger.warning(
...
def _nous_inference_env_override() -> Optional[str]:
    """Return the user-set ``SR_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("SR_INFERENCE_BASE_URL"))


def _nous_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_nous_inference_env_override()``: ``SR_PORTAL_BASE_URL`` /
    ``SR_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing SR at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``SR_PORTAL_BASE_URL=https://portal.staging-samsung.com`` into
    ...
    gated by ``_SR_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("SR_PORTAL_BASE_URL") or os.getenv("SR_PORTAL_BASE_URL")
    )
...
def _nous_invoke_jwt_status(
    ...
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = SR_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if SR_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: sr auth add nous",
        provider="nous",
        code=reason,
...
_SR_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _nous_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        if key not in _SR_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("SR_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `sr model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.sr/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("SR_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set SR_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("SR_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SR_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("SR_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `sr auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `sr auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `sr auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `sr auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.sr/.env, and return it.
    ...
    print("       App name:     anything (e.g. sr-agent)")
    ...
    # Persist so subsequent `sr auth spotify` runs skip the wizard.
    save_env_value("SR_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("SR_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved SR_SPOTIFY_CLIENT_ID to ~/.sr/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "SR_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize SR:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"SR is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.sr/auth.json (not ~/.codex/)
#
# SR maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from SR auth store (~/.sr/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `sr auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `sr auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `sr auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `sr auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``sr setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``sr auth add openai-codex``
      ...
      ``sr auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to SR auth store (~/.sr/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``sr auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into SR auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating SR auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run sr auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `sr auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to SR auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. SR keeps its OWN
        ...
        # the Codex CLI (or another SR process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from SR's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("SR_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("SR_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other SR processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.sr/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `sr model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `sr model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `sr model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `sr model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.sr/auth.json even when SR_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``sr auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.sr/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `sr model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``SR_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `sr model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older SR (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `sr model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``sr model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("SR_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("SR_XAI_BASE_URL", "").strip().rstrip("/")
...
# so a new `sr --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${SR_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<sr-root>/shared/nous_auth.json`` where ``<sr-root>`` is what
# ``get_default_sr_root()`` returns — ``~/.sr`` on Linux/macOS,
# ``%LOCALAPPDATA%\sr`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's SR_HOME so named profiles (which
# typically live under ``<sr-root>/profiles/<name>/``) all see the
# same file.
#
...
SR_SHARED_STORE_FILENAME = "nous_auth.json"
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``SR_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<sr-root>/shared/``, where ``<sr-root>`` is what
    :func:`sr_constants.get_default_sr_root` returns — so
    Linux/macOS classic installs land at ``~/.sr/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\sr\\shared\\``, and
    Docker / custom ``SR_HOME`` deployments at
    ``<SR_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("SR_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from sr_constants import get_default_sr_root
    return get_default_sr_root() / "shared"


def _nous_shared_store_path() -> Path:
    path = _nous_shared_auth_dir() / SR_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's SR root, refuse rather than silently corrupt cross-profile
    # state. Tests must set SR_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from sr_constants import get_default_sr_root
        real_home_shared = (
            get_default_sr_root() / "shared" / SR_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set SR_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No SR_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_SR_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_SR_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_SR_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_SR_INFERENCE_URL,
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — SR
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    retained = []
    removed = False
    singleton_sources = {SR_DEVICE_CODE_SOURCE, f"manual:{SR_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    try:
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_SR_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_SR_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_SR_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_SR_SCOPE,
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with SR's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, SR's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another SR install sharing "
            "~/.sr/auth.json) called POST /api/oauth/token with SR's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only SR may call the "
            "refresh endpoint. For health checks, use `sr auth status` "
            "instead.\n"
            "Re-authenticate with: sr auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip SR models — they're not reliable for agentic tool-calling
            if "sr" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "SR is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # SR_PORTAL_BASE_URL / SR_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors SR_INFERENCE_BASE_URL) and
        ...
        if env_portal_override:
            ...
            portal_base_url = (
                ...
                or DEFAULT_SR_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _SR_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_SR_CLIENT_ID)
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_SR_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_SR_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_SR_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_SR_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            state["inference_base_url"] = refreshed_url or DEFAULT_SR_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "sr-cli"),
        state.get("portal_base_url", DEFAULT_SR_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_SR_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_SR_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``sr auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``sr auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `sr auth add nous --type oauth`. Best-
    ...
    pool = load_pool("nous")
    return next(
        (e for e in pool.entries() if e.source == SR_DEVICE_CODE_SOURCE),
        None,
    )
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("SR is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("SR_PORTAL_BASE_URL")
                or os.getenv("SR_PORTAL_BASE_URL")
                or DEFAULT_SR_PORTAL_URL
            ).rstrip("/")
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _SR_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_SR_PORTAL_URL
            ...
            if force_refresh or invoke_jwt_status is not None:
                with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        # persisted to auth.json below. The SR_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_nous_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_SR_INFERENCE_URL
                        inference_base_url = (
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": SR_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": SR_AUTH_PATH_INVOKE_JWT,
...
# which does a synchronous OAuth refresh POST to portal.samsung.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`sr tools`, status panels,
# subscription-feature checks) call it many times per render — `sr tools` → "All Platforms"
...
# `sr auth login/logout/add/remove` invalidate naturally on the next call.
_SR_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
...
def get_nous_auth_status() -> Dict[str, Any]:
    ...
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _SR_AUTH_STATUS_CACHE_TTL
        ):
...
SR_SESSION_VALID = "valid"
SR_SESSION_TERMINAL = "terminal"
SR_SESSION_UNKNOWN = "unknown"
    ...
    if state:
        ...
        if isinstance(last_err, dict) and last_err.get("relogin_required"):
            ...
            if not (state.get("access_token") or state.get("refresh_token")):
                return SR_SESSION_TERMINAL

    try:
        status = get_nous_auth_status()
    except Exception:
        # Status computation itself failed — indeterminate, not terminal.
        return SR_SESSION_UNKNOWN

    if status.get("logged_in"):
        return SR_SESSION_VALID
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `sr auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `sr auth` and
    # `sr model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("SR_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("SR_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``sr doctor`` runs the live
    ...
    try:
        from sr_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on SR' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `sr doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from sr_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set SR_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from sr_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `sr logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from sr_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_SR_PORTAL_URL).rstrip("/")
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from sr_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'sr model' or 'sr setup' instead."""
    print("The 'sr login' command has been removed.")
    print("Use 'sr auth' to manage credentials,")
    print("'sr model' to select a provider, or 'sr setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.sr/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing SR-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in SR auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("SR will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("SR_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("SR will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — SR gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(SR creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to SR auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in SR auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(SR creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``sr auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``sr auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to SR auth store (~/.sr/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting SR login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `sr model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``sr status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("SR_INFERENCE_BASE_URL")
    ...
    print(f"Starting SR login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `sr model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def nous_token_has_billing_scope() -> bool:
    ...
    if not isinstance(scope, str):
        return False
    return SR_BILLING_MANAGE_SCOPE in scope.split()
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``SR_PORTAL_BASE_URL`` set
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [SR_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if SR_BILLING_MANAGE_SCOPE not in requested:
        requested.append(SR_BILLING_MANAGE_SCOPE)
    ...
    return isinstance(granted, str) and SR_BILLING_MANAGE_SCOPE in granted.split()
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older SR builds still see
            ...
            if model_ids:
                ...
                _url = (_portal or DEFAULT_SR_PORTAL_URL).rstrip("/")
        ...
        if not selected_model:
            ...
            print("  Run `sr model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("SR will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `sr model` or configure an API key to use SR.")
        else:
