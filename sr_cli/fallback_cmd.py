"""
sr fallback — manage the fallback provider chain.
...
Subcommands:
  sr fallback [list]   Show the current fallback chain (default when no subcommand)
  sr fallback add      Pick provider + model via the same picker as `sr model`,
                           then append the selection to the chain
  sr fallback remove   Pick an entry to delete from the chain
  sr fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.sr/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from sr_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `sr model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  sr fallback add")
        print()
        return
    ...
    print("  Docs: https://sr-agent.samsung.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `sr model`, then append the selection to the chain."""
    from sr_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `sr model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `sr fallback list` to view, or `sr fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``sr fallback [subcommand]``."""
