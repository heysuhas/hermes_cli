...
# xAI's public API is OpenAI-compatible for the endpoints SR commonly
# uses. The Responses endpoint is included because SR' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via SR-managed OAuth credentials."""

    auth_hint = "sr auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`sr auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`sr auth reset xai-oauth` or re-authenticate with "
                    "`sr auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `sr auth add xai-oauth --type oauth`."
            )
