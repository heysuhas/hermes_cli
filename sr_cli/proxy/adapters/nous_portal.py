...
Reads the user's Nous OAuth state from ``~/.sr/auth.json`` through the
...
# Endpoints inference-api.samsung.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `sr auth add nous` first."
                )
    ...
    # to sr_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
