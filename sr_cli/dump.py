"""
Dump command for sr CLI.

Outputs a compact, plain-text summary of the user's SR setup
...
from sr_cli.config import get_sr_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.sr/.env.
    ...
    exports. ``sr debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.sr_build_sha`` by the Dockerfile's
    ``SR_GIT_SHA`` build-arg (see ``sr_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``sr dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from sr_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(sr_home: Path) -> int:
...
def _cron_summary(sr_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sr_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sr_home:      {display_sr_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.sr/.env: a managed
        ...
        # A credential added via `sr auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `sr auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["sr-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sr_home)}")
    lines.append(f"  skills:             {_count_skills(sr_home)}")
