"""
SR Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``sr desktop`` / ``sr gui``)
     Built inside the agent checkout under ``$SR_HOME/sr-agent/``:
       ...
       - ``$SR_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/SR.app`` or ``~/Applications/SR.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\SR`` (NSIS per-user)
...
the app name ("SR"), separate from ``$SR_HOME``:
  - macOS:   ``~/Library/Application Support/SR``
  - Windows: ``%APPDATA%\\SR``
  - Linux:   ``$XDG_CONFIG_HOME/SR`` (default ``~/.config/SR``)
...
``sr uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "SR"
...
def source_built_gui_artifacts(sr_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``sr desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``sr-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(sr_home)
    ...
    return [
        ...
        sr_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "SR" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/SR.app"),
            home / "Applications" / "SR.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\SR).
            local_base / "Programs" / "SR",
            # Older / alternate layout some builds used.
            local_base / "sr-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "SR")
    else:
        ...
        paths += [
            data_base / "applications" / "sr.desktop",
            data_base / "applications" / "SR.desktop",
        ]
    return paths


def agent_is_installed(sr_home: Path) -> bool:
    """Return True when a usable Python agent install exists under SR_HOME.
...
def gui_is_installed(sr_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(sr_home):
        if p.exists():
            return True
...
def gui_install_summary(sr_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = sr_home if sr_home is not None else get_sr_home()
...
def uninstall_gui(sr_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``sr-agent/sr_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$SR_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove sr' or "
            "'sudo dnf remove sr'). AppImage builds are a single file you "
