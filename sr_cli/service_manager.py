...
the existing module-level functions in sr_cli.gateway and
sr_cli.gateway_windows directly. This protocol is a thin facade
...
``sr gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``sr gateway start/stop/restart``).
    """
    ...
    # `sr gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged sr
    ...
    detection always fails. Since every SR runtime call inside the
    container drops to sr via ``s6-setuidgid``, that silent failure
...
# in ``sr_cli.gateway`` (systemd/launchd) and ``sr_cli.gateway_windows``
...
# active (set via the ``sr -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in sr_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from sr_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from sr_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from sr_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from sr_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in sr_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from sr_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from sr_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from sr_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from sr_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``sr_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `sr profile create`
# runs inside the container (Phase 4). Static services (main-sr, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's SR_HOME blindly;
    derive the shared profile root from the current SR_HOME and map the
    ...
    sr_home = Path(os.environ.get("SR_HOME", "/opt/data"))
    if sr_home.parent.name == "profiles":
        root = sr_home.parent.parent
    else:
        root = sr_home
...
# UID/GID of the in-image ``sr`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the sr user.
    ...
    (the ``sr`` user — UID 10000 — running every SR runtime
    ...
    with sr ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           sr:sr, 0755 (parent must already exist)
    ``svc_dir/event/``                     sr:sr, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 sr:sr, 0755
    ``svc_dir/supervise/event/``           sr:sr, 03730
    ``svc_dir/supervise/control``          sr:sr, 0660    (FIFO)
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _SR_UID, _SR_GID)
        except PermissionError:
            # Running as the sr user already — directory is sr-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _SR_UID, _SR_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is sr-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _SR_UID, _SR_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``sr -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`sr profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-sr, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SR_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SR_HOME=/data/sr`` is honored at run
          ...
          4. Drops to the sr user and exec's
             ``sr -p <profile> gateway run`` (or just ``sr
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``sr gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SR_HOME profile" (the implicit profile that exists at
        the top of $SR_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``sr_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SR_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``SR_HOME``,
        ...
        allocator in ``sr_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export SR_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's SR_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``sr gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-SR_HOME PID lock first;
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SR_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SR_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SR_HOME=/data/sr``
        gets its logs under /data/sr/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``sr logs`` reads and what
        ...
        return (
            ...
            f': "${{SR_HOME:=/opt/data}}"\n'
            f'log_dir="$SR_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as sr and crash-loops on
            ...
            f'chown sr:sr "$SR_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R sr:sr "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid sr s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``sr gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``sr -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with sr ownership
            ...
            # the sr user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is sr-owned (see ``_seed_supervise_skeleton``), and on
