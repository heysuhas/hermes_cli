"""``sr slack ...`` CLI subcommands.

Today only ``sr slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ sr slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of SR. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a SR deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from sr_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with SR in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your SR agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``sr_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SR_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "SR")
    ...
    """
    name = getattr(args, "name", None) or "SR"
    description = getattr(args, "description", None) or "Your SR agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(get_sr_home()) / "slack-manifest.json"
            except Exception:
                target = Path(os.environ.get("SR_HOME") or str(Path.home() / ".sr")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your SR app\n"
