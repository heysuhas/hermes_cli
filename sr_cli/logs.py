"""``sr logs`` — view and filter SR log files.
...
Usage examples::
    ...
    sr logs                    # last 50 lines of agent.log
    sr logs -f                 # follow agent.log in real time
    sr logs errors             # last 50 lines of errors.log
    sr logs gateway -n 100    # last 100 lines of gateway.log
    sr logs gui -f            # follow gui.log (dashboard/pty/ws)
    sr logs desktop -f        # follow desktop.log (Electron app boot/backend)
    sr logs --level WARNING    # only WARNING+ lines
    sr logs --session abc123   # filter by session ID substring
    sr logs --component tools  # only tool-related lines
    sr logs --since 1h         # lines from the last hour
    sr logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_sr_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when SR runs — try 'sr chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_sr_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sr_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sr_home()}/logs/")
        return

    print(f"Log files in {display_sr_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sr chat' to generate logs)")
