"""
SR Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sr/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove SR PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing sr-agent or sr PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# SR Agent" comment and following line
                if '# SR Agent' in line or '# sr-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('sr' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing sr
                if 'sr' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the sr wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "sr",
        Path("/usr/local/bin/sr"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains sr_cli reference)
                content = wrapper.read_text()
                if 'sr_cli' in content or 'sr-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(sr_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``sr`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this SR home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (sr_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sr gateway run`` processes
    ...
    try:
        from sr_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sr_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SR_HOME`` and ``SR_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\sr\git\cmd``, ``%LOCALAPPDATA%\sr\git\bin``,
#      ``%LOCALAPPDATA%\sr\git\usr\bin``, ``%LOCALAPPDATA%\sr\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\sr\git\`` and Node to
#      ``%LOCALAPPDATA%\sr\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``sr dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\sr\gateway-service\`` and sometimes
...
def _sr_path_markers(sr_home: Path) -> list[str]:
    """Path-entry substrings that identify SR-owned User-PATH entries."""
    root = str(sr_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare sr-agent install dir.
    markers = [root + "\\sr-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SR_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "sr".  We don't want to catch
    # unrelated entries like "csr-foo" or "ephermeral", so we look for
    # backslash-sr as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(sr_home: Path) -> list[str]:
    """Strip SR-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _sr_path_markers(sr_home)
            kept: list[str] = []
...
def remove_sr_env_vars_windows() -> list[str]:
    """Delete SR_HOME and SR_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SR_HOME", "SR_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(sr_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\sr\\``.  Only called on full uninstall; they're
...
def _is_default_sr_home(sr_home: Path) -> bool:
    """Return True when ``sr_home`` points at the default (non-profile) root."""
    try:
        from sr_constants import get_default_sr_root
        return sr_home.resolve() == get_default_sr_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SR_HOME directory.

    We shell out to ``sr -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SR_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m sr_cli.main` so we don't depend on a `sr`
    #    wrapper that may be half-removed mid-uninstall.
    sr_invocation = [_sys.executable, "-m", "sr_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                sr_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SR_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``sr uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$SR_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from sr_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    sr_home = get_sr_home()
    summary = gui_install_summary(sr_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ SR Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No SR Chat GUI installation was found.")
        print(f"  Checked: {sr_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The SR agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(sr_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The SR agent at {sr_home / 'sr-agent'}")
        print(f"  • Your config, sessions, and secrets under {sr_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The SR agent is still installed. Run 'sr' to use the CLI,")
    print("or 'sr uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sr/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.sr/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie SR_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_sr_home(sr_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.sr data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ SR Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {sr_home / 'config.yaml'}")
    print(f"  Secrets: {sr_home / '.env'}")
    print(f"  Data:    {sr_home / 'cron/'}, {sr_home / 'sessions/'}, {sr_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SR_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL SR data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the SR code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, sr_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • SR PATH entries from shell configs / Windows User PATH")
    print("  • SR wrapper scripts and SR-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • SR config/data: {sr_home}")
        if _is_default_sr_home(sr_home):
        ...
        print(f"  • Keep SR config/data: {sr_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``sr`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$SR_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in sr_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\sr\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(sr_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No SR-owned PATH entries in User environment")

        log_info("Removing SR_HOME / SR_GIT_BASH_PATH User env vars...")
        removed_env = remove_sr_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No SR-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing sr command...")
    ...
    #     (only when they still point into this SR home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing SR-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(sr_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No SR-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(sr_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE SR_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from sr_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(sr_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.sr/, just remove the sr-agent subdir
            if sr_home in project_root.parents or project_root.parent == sr_home:
    ...
    #     under SR_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(sr_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(sr_home)
    ...
    # 5. Optionally remove ~/.sr/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SR_HOME dirs live under
        ...
        try:
            if sr_home.exists():
                shutil.rmtree(sr_home)
                log_success(f"Removed {sr_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {sr_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {sr_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://sr-agent.samsung.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://sr-agent.samsung.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using SR Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m sr_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import sr_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``sr_constants`` + ``sr_cli.colors``
    (and lazily ``sr_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m sr_cli.uninstall")
    parser.add_argument(
        "--mode",
