"""MCP picker — interactive `sr mcp picker` (also the default `sr mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``sr mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `sr plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from sr_cli.cli_output import prompt_yes_no
from sr_cli.curses_ui import curses_single_select
from sr_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new SR session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from sr_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `sr mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: sr mcp install <name>    Picker: sr mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their SR is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer SR — run `sr update` "
...
def show_catalog() -> None:
    """`sr mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`sr mcp picker` (and default `sr mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`sr mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `sr mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
