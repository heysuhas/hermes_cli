"""Profile distributions — shareable, packaged SR profiles via git.

A distribution is a SR profile published as a git repository (or
...
* ``sr profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``sr skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``sr profile``, not a parallel tree):

    sr profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "sr-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            sr_requires=str(data.get("sr_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.sr_requires:
            out["sr_requires"] = self.sr_requires
        if self.author:
...
def check_sr_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires SR {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``sr profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a SR profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a SR distribution."
        )

    # Version check up-front so we fail fast
    check_sr_requires(manifest.sr_requires, sr_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.sr).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="sr_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `sr profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`sr profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="sr_dist_update_") as tmp:
        plan = plan_install(
