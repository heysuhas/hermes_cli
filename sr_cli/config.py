"""
Configuration management for SR Agent.

Config files are stored in ~/.sr/ for easy access:
- ~/.sr/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sr/.env         - API keys and secrets

This module provides:
- sr config          - Show current configuration
- sr config edit     - Open config in editor
- sr config set      - Set a specific value
- sr config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``sr config set`` (which rewrites
    ...
    ``config.yaml`` in place: sr never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.sr/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``sr logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  sr config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` SR makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. SR itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``sr update``, the TUI build.
...
#   on every plugin install / ``sr update``.
...
# * ``SR_HOME`` / ``SR_PROFILE`` / ``SR_CONFIG`` /
#   ``SR_ENV`` — SR runtime location flags. Writing these into
...
#
# IMPORTANT: ``SR_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (SR_LANGFUSE_PUBLIC_KEY,
# SR_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # SR runtime location — never via dashboard env writer.
    # NOT a SR_* blanket: integration credentials (SR_GEMINI_*,
    # SR_LANGFUSE_*, SR_SPOTIFY_*, ...) ARE allowed.
    "SR_HOME", "SR_PROFILE", "SR_CONFIG", "SR_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or SR runtime location "
            "(SR_HOME, SR_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "SR_TOOL_PROGRESS", "SR_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `sr plugins enable
    # observability/langfuse` or `sr tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SR_LANGFUSE_ENV",
    "SR_LANGFUSE_RELEASE",
    "SR_LANGFUSE_SAMPLE_RATE",
    "SR_LANGFUSE_MAX_CHARS",
    "SR_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from sr_cli.colors import Colors, color
from sr_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SR_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_sr_home() / ".managed"
...
def is_managed() -> bool:
    """Check if SR is running in package-manager-managed mode.

    Two signals: the SR_MANAGED env var (set by the systemd service),
    or a .managed marker file in SR_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``sr_cli/`` — i.e. the git checkout for source
    installs, ``/opt/sr`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$SR_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how SR was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$SR_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. SR_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.sr``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$SR_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.sr:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$SR_HOME``, a
    ...
    ``docker`` and ``sr update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``samsungresearch/sr-agent`` image bakes a ``docker``
        stamp into ``/opt/sr`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $SR_HOME.
    try:
    ...
    #    host install whose shared $SR_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``sr update``.
    try:
        method = (
            (get_sr_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``sr_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$SR_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/sr`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* SR lives in a ``uv tool`` layout.

    ``uv tool install sr-agent`` places the install at
    ``.../uv/tools/sr-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``sr-agent`` happens to be uv-tool-installed on the machine while
    the *active* SR is a regular pip/venv install, causing
    ``sr update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/sr-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade sr-agent"
    if method == "docker":
        return "docker pull samsungresearch/sr-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade sr-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade sr-agent"
        return "pip install --upgrade sr-agent"
    return "sr update"
...
# banner, the TUI/desktop session info panel, and ``sr update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://sr-agent.samsung.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``sr update`` / ``sr update --check`` so the wording — and the
...
# Long-form text for ``sr update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# sr_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side SR, it doesn't update
...
✗ ``sr update`` doesn't apply inside the Docker container.

SR Agent runs as a published image (samsungresearch/sr-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull samsungresearch/sr-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate sr-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm samsungresearch/sr-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/samsungresearch/sr-agent/tags
  • Your config and session history live under ``$SR_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``sr update`` inside Docker.
...
def format_managed_message(action: str = "modify this SR installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SR_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this SR installation is managed by NixOS "
            f"(SR_MANAGED={env_hint}).\n"
            "Edit services.sr-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this SR installation is managed by Homebrew "
            f"(SR_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade sr-agent"
        )

    return (
        f"Cannot {action}: this SR installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall SR."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SR_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, sr_bin
    or None if container mode is not active, we're already inside the
    container, or SR_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SR_DEV") == "1":
        return None
    ...
    container_mode_file = get_sr_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "sr-agent")
    exec_user = info.get("exec_user", "sr")
    sr_bin = info.get("sr_bin", "/data/current-package/bin/sr")
...
# Re-export from sr_constants — canonical definition lives there.
from sr_constants import get_sr_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_sr_home() / "config.yaml"


def is_corporate_mode() -> bool:
    """Return whether SR is running through the employee corporate launcher."""
    return os.environ.get("SR_CORPORATE_MODE", "").strip().lower() in {"1", "true", "yes"}


def _corporate_config_write_allowed() -> bool:
    """Allow config writes only during the launcher-owned setup transaction."""
    return os.environ.get("SR_CORPORATE_CONFIG_WRITE", "") == "1"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_sr_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_sr_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the SR_UID / SR_GID env vars set by Docker deployments.

    Docker containers running SR commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level SR_HOME once, but
    subdirectories created at runtime by ``ensure_sr_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("SR_UID", "").strip()
    gid_str = os.environ.get("SR_GID", "").strip()
    try:
...
def _chown_to_sr_uid(path) -> None:
    """Chown ``path`` to ``SR_UID:SR_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_sr_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_sr_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the sr group can
    share state with the gateway service.

    The mode can be overridden via the SR_HOME_MODE environment variable
    (e.g. SR_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SR_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``SR_UID``/``SR_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SR_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_sr_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When SR runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SR_CONTAINER") or os.environ.get("SR_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SR_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SR_HOME, upgrading legacy empty templates.
...
def ensure_sr_home():
    """Ensure ~/.sr directory structure exists with secure permissions.
    ...
    """
    home = get_sr_home()
    # Named profiles must be created explicitly (e.g. ``sr profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old SR_HOME would resurrect an empty skeleton
...
def _ensure_sr_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SR_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the SR-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps SR
        ...
        # identity slot (SOUL.md). Empty by default. The SR_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, SR adds a coding operating brief
    ...
    "terminal": {
        ...
        #             SR_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force SR_HOME/home when it exists (old strict
        ...
        # skipped. When empty, SR auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), SR sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when SR
        ...
        # bundled SR image, which drops to the `sr` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, SR sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and SR should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``sr chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sr/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: sr sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .sr.md, or .cursorrules before SR applies
    ...
    # single read_file page exceeds these limits, SR truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so SR' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); sr =
                                      # SR' compression threshold triggers
    ...
    # See tools/kanban_tools.py and sr_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``sr dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``sr kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``sr kanban decompose`` and the kanban
        ...
        # ``sr profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `sr model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `sr` (and `sr chat`) launches by default:
        ...
        # REPL and `--tui` (or SR_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `sr --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `sr -c` muscle memory.  Default off so existing
        # users aren't surprised.  SR_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `sr --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to SR_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/sr`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `sr pets`. Disabled until
        ...
        "pet": {
            ...
            # get_sr_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``SR_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``SR_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.samsung.com).
        "oauth": {
        ...
        # (``SR_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``SR_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``SR_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/sr``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; SR appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sr/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sr/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.sr/cache/delegation/
    ...
    # response. If not, SR feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before SR auto-pauses the goal and
    ...
    "moa": {
        ...
        # <sr_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.sr/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SR_SKILL_DIR} and ${SR_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `sr curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `sr curator run --consolidate` overrides
        ...
        # normally restored on every `sr update`, so pruning them only
        ...
        # skipped), snapshot ~/.sr/skills/ into
        # ~/.sr/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sr curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for sr-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *SR Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SR_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace SR' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.sr/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SR_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `sr doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``sr_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow SR to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $SR_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        "chronos": {
            ...
            "portal_url": "https://portal.samsung.com",
        ...
        # Also overridable via SR_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `sr -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `sr` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `sr kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     sr-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core SR tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.sr/logs/.
    ...
    # update model picker lists without shipping a sr-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": False,
        "url": "https://sr-agent.samsung.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `sr model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal SR_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.sr/sessions/sessions.json entirely.
        ...
        # the SR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.sr.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.sr/.env,
        ...
        # behavior: files must live under the SR cache, under
        ...
        # user. Bridged to SR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the SR
        # cache (~/.sr/cache/{documents,images,audio,video,screenshots})
        ...
        # to SR_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
    ...
    # Session storage — controls automatic cleanup of ~/.sr/state.db.
    ...
    "sessions": {
        ...
        # rewrites ``~/.sr/sessions/session_{sid}.json`` on every turn
    ...
    # ``sr update`` behaviour.
    "updates": {
        # Run a full ``sr backup``-style zip of SR_HOME before every
        # ``sr update``.  Backups land in ``<SR_HOME>/backups/`` and
        # can be restored with ``sr import <path>``.  Off by default:
        # zipping a large SR_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``sr update
        ...
        "backup_keep": 5,
        # What `sr update` does with uncommitted local changes to the
    ...
    # rather than storing them in ~/.sr/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.sr/.env (or your shell) and never in config.yaml.
            ...
            # ~/.sr/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.sr/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. SR disables it for our users unless they
        # explicitly opt in here. When false (default), SR sets
    ...
    # SR Desktop (Electron app) launch options. These only affect
    # `sr desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the SR_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials SR mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sr model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. samsung.com -> firecrawl-gateway.samsung.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the SR auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use SR, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @sr:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SR_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #sr)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sr-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sr-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote SR API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote SR API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote SR API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SR_TOOL_PROGRESS and SR_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.sr.config`` entries, then checks
    ...
    try:
        ...
        # should never break `sr update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but SR' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older SR versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sr_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sr setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — SR won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sr doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SR_HOME", "~/.sr")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "sr update / sr -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SR_TOOL_PROGRESS")
            old_mode = get_env_value("SR_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SR_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SR_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SR_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SR_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SR_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_sr_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `sr plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `sr curator status` has no
    ...
    #   3. Creates `~/.sr/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sr_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sr_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `sr config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `sr config set`"
                    )
    ...
    # behavior set it to true explicitly via `sr config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from sr_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from sr_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: sr config set <key> <value>")
    ...
    # `sr update`, but only the version bump is persisted.
    ...
    # metadata.sr.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_sr_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``sr config set``
    ...
    backends) — while display paths (``sr status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sr_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sr/config.yaml as-is, without merging defaults or migrating.
...
def atomic_config_write(config_path: Path, data: Any, **kwargs: Any) -> None:
    ...
    """
    if is_corporate_mode() and not _corporate_config_write_allowed():
        raise PermissionError(
            "config.yaml is managed by your organization and can only be "
            "changed through the SR setup flow."
        )
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.sr/config.yaml.
    ...
    (which change ``SR_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/sr/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_sr_dotenv()
...
#   openai-codex (OAuth — sr auth) — OpenAI Codex
#   nous         (OAuth — sr auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.sr/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset SR writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sr/.env.
    ...
    interactive menu render (`sr tools`, `sr setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `sr tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real SR env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sr/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sr/.env."""
    if is_corporate_mode() and not _corporate_config_write_allowed():
        raise PermissionError(
            ".env is managed by your organization and can only be changed "
            "through the SR setup flow."
        )
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sr/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.sr/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to SR — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known SR vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sr/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.sr/.env`` over ``os.environ``.

    Used for SR-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ SR Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from sr_cli.auth import get_anthropic_key
    ...
    # Warn on stale SR_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("SR_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale SR_MAX_ITERATIONS={_env_ghost} "
                f"(run 'sr doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  sr config edit     # Edit config file", Colors.DIM))
    print(color("  sr config set <key> <value>", Colors.DIM))
    print(color("  sr setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `sr config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `sr config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: sr config set <key> <value>")
            print()
            print("Examples:")
            print("  sr config set model anthropic/claude-sonnet-4")
            print("  sr config set terminal.backend docker")
            print("  sr config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sr config migrate' to add them")
        ...
        print("  sr config           Show current configuration")
        print("  sr config edit      Open config in editor")
        print("  sr config set <key> <value>   Set a config value")
        print("  sr config check     Check for missing/outdated config")
        print("  sr config migrate   Update config with new options")
        print("  sr config path      Show config file path")
        print("  sr config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sr config`` UI so users
