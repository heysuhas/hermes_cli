...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            if (
                corporate_policy is not None
                and corporate_policy.enabled
                and not corporate_policy.allows_plugin(lookup_key, manifest.name)
            ):
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = "blocked by corporate product policy"
                self._plugins[lookup_key] = loaded
                logger.debug(
                    "Skipping plugin '%s' (corporate policy)", lookup_key
                )
                continue
