...
import logging
import mimetypes
# Explicitly force correct MIME types on Windows
...
# CORS: restrict to localhost origins only, unless an enterprise dashboard origin is defined.
_default_cors_regex = r"^https?://(localhost|127\.0\.0\.1)(:\d+)?$"
_custom_origin = os.environ.get("HERMES_ALLOWED_DASHBOARD_ORIGIN")
if _custom_origin:
    _cors_regex = rf"({_default_cors_regex})|({_custom_origin})"
else:
    _cors_regex = _default_cors_regex
