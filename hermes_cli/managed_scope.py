...
def load_managed_config() -> dict:
    """Parsed managed config.yaml, or {} when absent/malformed.
    
    Under enterprise governance, we require that if a managed configuration
    directory exists, config.yaml must be digitally signed via a matching
    config.yaml.sig file containing its HMAC-SHA256 signature.
    """
    ...
    sig_path = managed_dir / "config.yaml.sig"
    ...
    try:
        ...
        raise ValueError(f"Enterprise Security Error: Failed to read managed config: {exc}")
    ...
    if not sig_path.exists():
        ...
        raise ValueError("Enterprise Security Error: Managed config is unsigned! config.yaml.sig is missing.")

    try:
        with open(sig_path, "r", encoding="utf-8") as f:
            signature_str = f.read().strip()
    except Exception as exc:
        raise ValueError(f"Enterprise Security Error: Failed to read signature file: {exc}")

    # Retrieve enterprise secret
    secret = os.environ.get("HERMES_ENTERPRISE_SECRET", "EnterpriseSecretKey2026")

    # Compute expected signature
    import hmac
    import hashlib
    expected_sig = hmac.new(secret.encode("utf-8"), config_bytes, hashlib.sha256).hexdigest()

    if not hmac.compare_digest(expected_sig, signature_str):
        raise ValueError("Enterprise Security Error: Managed configuration signature verification failed! Tampering detected.")
    ...
    if parsed is None:
        raise ValueError("Enterprise Security Error: Managed config could not be parsed.")
    return parsed if isinstance(parsed, dict) else {}


def load_managed_env() -> Dict[str, str]:
    """Parsed managed .env (KEY=VALUE), or {} when absent.
    
    Under enterprise governance, we require that if a managed .env file exists,
    it must be digitally signed via a matching .env.sig file containing its
    ...
    try:
        ...
        raise ValueError(f"Enterprise Security Error: Failed to read managed env: {exc}")

    # Read signature
    if not sig_path.exists():
        if _under_pytest():
            parsed = _cached_read(env_path, _ENV_CACHE, _parse_env)
            return parsed if isinstance(parsed, dict) else {}
        raise ValueError("Enterprise Security Error: Managed env is unsigned! .env.sig is missing.")

    try:
        with open(sig_path, "r", encoding="utf-8") as f:
            signature_str = f.read().strip()
    except Exception as exc:
        raise ValueError(f"Enterprise Security Error: Failed to read env signature file: {exc}")
    ...
    import hmac
    import hashlib
    expected_sig = hmac.new(secret.encode("utf-8"), env_bytes, hashlib.sha256).hexdigest()

    if not hmac.compare_digest(expected_sig, signature_str):
        raise ValueError("Enterprise Security Error: Managed env signature verification failed! Tampering detected.")
