...
DEFAULT_CONFIG = {
    "product": {
        # General Hermes remains the default. Managed Windows deployments can
        # select ``corporate_local`` to enforce the administrator policy in
        # %ProgramData%\Hermes\policy.yaml.
        "mode": "general",
        "allowed_roots": [],
        "allowed_tools": [],
        "allowed_plugins": [],
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        product_cfg = expanded.get("product")
        selected_product_mode = (
            os.environ.get("HERMES_PRODUCT_MODE")
            or (
                product_cfg.get("mode")
                if isinstance(product_cfg, dict)
                else ""
            )
            or ""
        )
        if str(selected_product_mode).strip().lower() == "corporate_local":
            # Product policy ceiling. These settings remove implicit external
            # discovery/fallback paths before downstream subsystems inspect
            # configuration. The administrator policy provides finer-grained
            # tool/plugin/path decisions at their execution boundaries.
            expanded["fallback_providers"] = []
            expanded["fallback_model"] = None
            expanded.setdefault("model_catalog", {})["enabled"] = False
            expanded.setdefault("security", {})["allow_lazy_installs"] = False
            expanded.setdefault("curator", {})["enabled"] = False
            expanded.setdefault("tools", {}).setdefault("tool_search", {})[
                "enabled"
            ] = "off"
            expanded.setdefault("platform_toolsets", {})["cli"] = [
                "corporate_local",
                "desktop_mail",
                "corporate_office",
            ]
            ...
            expanded.setdefault("web", {})["backend"] = ""
            expanded["web"]["search_backend"] = ""
            expanded["web"]["extract_backend"] = ""
            expanded.setdefault("browser", {})["engine"] = "disabled"
            agent_cfg = expanded.setdefault("agent", {})
            try:
                agent_cfg["max_turns"] = min(
                    24, max(1, int(agent_cfg.get("max_turns") or 24))
                )
            except (TypeError, ValueError):
                agent_cfg["max_turns"] = 24
            agent_cfg["api_max_retries"] = 1
            agent_cfg["parallel_tool_call_guidance"] = False
            agent_cfg["environment_probe"] = False
            agent_cfg["coding_context"] = "off"
