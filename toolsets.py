...
TOOLSETS = {
    ...
    "corporate_local": {
        "description": (
            "Governed local employee-assistant capabilities: local terminal "
            "and files, self-improving skills, Outlook, and Office documents. "
            "The corporate policy applies an additional immutable allowlist."
        ),
        "tools": [
            "terminal", "process",
            "read_file", "write_file", "patch", "search_files",
            ...
            "local_access_status",
            "mail_client_status", "mail_list_folders", "mail_list_messages",
            "mail_get_message", "mail_create_draft", "mail_update_draft",
            "document_inspect", "document_extract",
            "office_plan_changes", "office_apply_changes",
        ],
        "includes": [],
    },
...
def get_all_toolsets() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        corporate_mode = is_corporate_mode()
    except Exception:
    ...
    for ts_name in _get_plugin_toolset_names():
        if corporate_mode and ts_name not in {"desktop_mail", "corporate_office"}:
            continue
