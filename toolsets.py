...
_SR_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on SR_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (SR_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `sr tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SR_KANBAN_TASK env "
    ...
    # Coding posture (base SR — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in sr_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full SR toolsets (CLI + messaging platforms)
    #
    ...
    # the `sr send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "sr-cli": {
        ...
        "tools": _SR_CORE_TOOLS,
        "includes": []
    },

    "sr-cron": {
        # Mirrors sr-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `sr tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa)
        # are excluded by _get_platform_tools() unless the user explicitly
        # enables them.
        "description": "Default cron toolset - same core tools as sr-cli; gated by `sr tools`",
        "tools": _SR_CORE_TOOLS,
        "includes": []
    },
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``sr-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_SR_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``sr-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (sr-<name>).
        # Gives them _SR_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("sr-"):
            platform_name = name[len("sr-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SR_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
