# SR Agent Has Had "Routines" Since March
...
SR equivalent — works today:
```bash
sr webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working SR implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | SR Agent |
|---|---|---|
...
## Things SR Does That Routines Can't
...
SR has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
SR Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
sr gateway setup    # enable webhooks
sr webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [sr-agent.samsung.com/docs/reference/automation-blueprints-catalog](https://sr-agent.samsung.com/docs/reference/automation-blueprints-catalog)

Documentation: [sr-agent.samsung.com](https://sr-agent.samsung.com)

GitHub: [github.com/SamsungResearch/sr-agent](https://github.com/SamsungResearch/sr-agent)

---

*SR Agent is built by [Samsung Research](https://samsung.com). Open source, model-agnostic, runs on your infrastructure.*
