#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the SR Skills Hub.
...
Used by sr_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _sr_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            sr_meta = metadata.get("sr", {})
            if isinstance(sr_meta, dict):
                tags = sr_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Samsung Research) but not activated
    ...
    OFFICIAL_REPO = "SamsungResearch/sr-agent"
...
# ---------------------------------------------------------------------------
# SR centralized index source
# ---------------------------------------------------------------------------

SR_INDEX_URL = "https://sr-agent.samsung.com/docs/api/skills-index.json"
SR_INDEX_TTL = 6 * 3600  # 6 hours


def _sr_index_cache_file() -> Path:
    return _index_cache_dir() / "sr-index.json"


def _load_sr_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SR_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    sr_index_cache_file = _sr_index_cache_file()
    if sr_index_cache_file.exists():
        try:
            age = time.time() - sr_index_cache_file.stat().st_mtime
            if age < SR_INDEX_TTL:
                return json.loads(sr_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("SR index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "SR index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("SR index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        sr_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        sr_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class SRIndexSource(SkillSource):
    """Skill source backed by the centralized SR Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "sr-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "sr-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        SRIndexSource(auth=auth), # Centralized index (search + resolved install paths)
