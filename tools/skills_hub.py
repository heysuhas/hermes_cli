...
def get_internal_store_domain() -> str:
    try:
        from hermes_cli.config import load_config
        config = load_config()
        store_url = config.get("skills", {}).get("store_url")
        if store_url:
        ...
        pass
    return "skills.domain.network"


def is_internal_allowed_url(url: str) -> bool:
    try:
        parsed = urlparse(url)
        ...
        allowed_domain = get_internal_store_domain()
        if ":" in allowed_domain:
            allowed_domain = allowed_domain.split(":")[0]

        if host == allowed_domain or host.endswith(".domain.network"):
            return True
    except Exception:
        pass
    return False
...
def _guarded_http_get(url: str, *, timeout: int = 20) -> Optional[httpx.Response]:
    ...
    if not is_internal_allowed_url(current_url):
        logger.warning("Blocked non-internal network Skills Hub URL: %s", current_url)
        return None
...
def install_from_quarantine(
    ...
    if bundle.source == "corporate-broker":
        try:
            _post_corporate_install_event(
                {
                    "package_id": bundle.metadata.get("broker_package_id", ""),
                    "version": bundle.metadata.get("version", ""),
                    "artifact_sha256": bundle.metadata.get("artifact_sha256", ""),
                    "installed_name": safe_skill_name,
                    "scan_verdict": scan_result.verdict,
                    "installed": True,
                }
            )
        except Exception:
            logger.debug("Corporate broker install event failed", exc_info=True)
...
class InternalSkillStoreSource(SkillSource):
    """Fetch skills from the company's internal skill store on the domain network."""
    ...
    def get_store_url(self) -> str:
        try:
            from hermes_cli.config import load_config
            config = load_config()
            url = config.get("skills", {}).get("store_url")
            if url:
                return url.rstrip("/")
        except Exception:
            pass
        return "http://skills.domain.network"

    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            store_url = self.get_store_url()
            index_url = f"{store_url}/index.json"
            resp = _guarded_http_get(index_url, timeout=15)
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        for s in index.get("skills", []):
            if s.get("identifier") == identifier or s.get("name") == identifier:
                return self._to_meta(s)
        return None

    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        # Strip internal-store/ prefix if present
        slug = identifier.lower().replace("internal-store/", "").replace(" ", "-").replace("_", "-")
        store_url = self.get_store_url()
        skill_url = f"{store_url}/skills/{slug}.json"

        resp = _guarded_http_get(skill_url, timeout=20)
        ...
        try:
            ...
            logger.debug("Failed to parse internal skill bundle: %s", e)
            return None

    def _to_meta(self, s: dict) -> SkillMeta:
        extra = dict(s.get("extra", {}))
        extra.setdefault("version", s.get("version", "1.0.0"))
        extra.setdefault("author", s.get("author", "Internal Store"))
        return SkillMeta(
            identifier=s.get("identifier", f"internal-store/{s.get('name', '').lower()}"),
            name=s.get("name", "Unnamed Skill"),
            description=s.get("description", ""),
            source=self.source_id(),
            trust_level=self.trust_level_for(s.get("identifier", "")),
            path=s.get("path", s.get("name", "").lower()),
            tags=s.get("tags", []),
            extra=extra,
        )


class CorporateBrokerSource(SkillSource):
    """Approved, immutable skill bundles from the corporate artifact broker."""

    def __init__(self):
        from agent.corporate_policy import get_corporate_policy

        self.policy = get_corporate_policy()
        self.base_url = self.policy.broker_url.rstrip("/")
        self._catalog: Optional[dict] = None
        self.cache_dir = HUB_DIR / "corporate-broker-cache"

    def source_id(self) -> str:
        return "corporate-broker"
    ...
    def _headers(self) -> dict[str, str]:
        ...
        token_file = self.policy.broker_token_file
        if token_file:
            try:
    ...
    def _get(self, path: str, *, timeout: int = 20) -> Optional[httpx.Response]:
        from agent.corporate_policy import validate_outbound_url

        url = f"{self.base_url}{path}"
        allowed, reason = validate_outbound_url(url)
        if not allowed:
            logger.warning("Blocked corporate broker URL %s: %s", url, reason)
            return None
        try:
            return httpx.get(
                url,
            ...
            logger.debug("Corporate broker request failed: %s", exc)
            return None

    def _load_catalog(self) -> dict:
        if self._catalog is not None:
            return self._catalog
        response = self._get("/v1/catalog")
        if response is None or response.status_code != 200:
            ...
            try:
                ...
                value = {}
            self._catalog = value if isinstance(value, dict) else {}
        return self._catalog

    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        needle = query.casefold().strip()
        results: List[SkillMeta] = []
        for item in self._load_catalog().get("packages", []):
            if not isinstance(item, dict) or not item.get("approved", False):
                continue
            text = " ".join(
                [
                    str(item.get("id", "")),
            ...
            if needle and needle not in text:
                continue
            results.append(self._to_meta(item))
            if len(results) >= limit:
                break
        return results

    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        package_id, _ = self._parse_identifier(identifier)
        for item in self._load_catalog().get("packages", []):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        package_id, requested_version = self._parse_identifier(identifier)
        version = requested_version or self._latest_version(package_id)
        if not package_id or not version:
            return None
        response = self._get(f"/v1/packages/{package_id}/{version}/manifest")
        manifest_cache = self.cache_dir / "manifests" / package_id / f"{version}.json"
        if response is not None and response.status_code == 200:
            ...
            # A definitive broker response (not found, revoked, forbidden)
            # must never fall back to an older approved cache entry.
            return None
        if not isinstance(manifest, dict) or not manifest.get("approved", False):
            return None
        artifact_sha = str(manifest.get("artifact_sha256") or "").lower()
        if not re.fullmatch(r"[0-9a-f]{64}", artifact_sha):
            return None
        if not self._verify_manifest(manifest):
            logger.warning("Corporate skill manifest signature verification failed")
            return None
        artifact_response = self._get(f"/v1/artifacts/{artifact_sha}", timeout=60)
        artifact_cache = self.cache_dir / "artifacts" / artifact_sha
        if artifact_response is not None and artifact_response.status_code == 200:
            artifact = artifact_response.content
        elif artifact_response is None or artifact_response.status_code >= 500:
            try:
                artifact = artifact_cache.read_bytes()
            except OSError:
                return None
        else:
            return None
        if hashlib.sha256(artifact).hexdigest() != artifact_sha:
            logger.warning("Corporate skill artifact hash mismatch for %s", package_id)
            return None
        if response is not None and response.status_code == 200:
        ...
        if artifact_response is not None and artifact_response.status_code == 200:
            ...
            temporary = artifact_cache.with_suffix(".tmp")
            temporary.write_bytes(artifact)
            os.replace(temporary, artifact_cache)
        ...
        if "SKILL.md" not in files:
            return None
        return SkillBundle(
            name=_validate_skill_name(str(manifest.get("name") or package_id)),
            files=files,
            source=self.source_id(),
            identifier=f"corporate-broker/{package_id}@{version}",
            trust_level="trusted",
            metadata={
                ...
                "scanner_results": manifest.get("scanner_results", {}),
            },
        )

    def _verify_manifest(self, manifest: dict) -> bool:
        public_key_file = self.policy.broker_public_key_file
        signature = str(manifest.get("signature") or "")
        if not public_key_file or not signature:
            return not self.policy.skill_require_broker_signature
        try:
            import base64
            ...
            logger.debug("Broker signature verification error: %s", exc)
            return False

    def _latest_version(self, package_id: str) -> str:
        response = self._get(f"/v1/packages/{package_id}/versions")
        if response is None or response.status_code != 200:
            return ""
        try:
    ...
    def _parse_identifier(identifier: str) -> tuple[str, str]:
        value = identifier.removeprefix("corporate-broker/").strip()
        package_id, separator, version = value.partition("@")
        if not re.fullmatch(r"[A-Za-z0-9._-]+", package_id):
            return "", ""
        if separator and not re.fullmatch(r"[A-Za-z0-9._+-]+", version):
            return "", ""
    ...
    def _to_meta(self, item: dict) -> SkillMeta:
        ...
        return SkillMeta(
            ...
            source=self.source_id(),
            identifier=f"corporate-broker/{package_id}",
            trust_level="trusted",
...
def _post_corporate_install_event(payload: dict[str, Any]) -> None:
    from agent.corporate_policy import get_corporate_policy, validate_outbound_url

    policy = get_corporate_policy()
    url = f"{policy.broker_url.rstrip('/')}/v1/install-events"
    allowed, _ = validate_outbound_url(url)
    if not allowed:
        return
    headers = {"Content-Type": "application/json"}
    if policy.broker_token_file:
        try:
            token = policy.broker_token_file.read_text(encoding="utf-8").strip()
        except OSError:
            token = ""
        if token:
            headers["Authorization"] = f"Bearer {token}"
