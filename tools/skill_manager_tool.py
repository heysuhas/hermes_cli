...
def _security_scan_skill(skill_dir: Path) -> Optional[str]:
    ...
    if corporate_mode:
        prohibited = re.compile(
            r"(?i)(?:https?|ftp|ssh|git|ws|wss)://|"
            r"\b(?:curl|wget|Invoke-WebRequest|Invoke-RestMethod|"
            r"pip(?:3)?\s+install|uv\s+(?:pip\s+)?install|"
            r"npm\s+install|git\s+(?:clone|fetch|pull))\b"
        )
        for candidate in skill_dir.rglob("*"):
            ...
            if prohibited.search(content):
                return (
                    "Corporate policy blocked this skill because it declares "
                    "direct network access or an internet-based dependency "
                    f"installation ({candidate.relative_to(skill_dir)}). "
                    "Use local dependencies or the approved corporate broker."
                )
    try:
