...
def propagate_context_to_thread(target: Callable) -> Callable:
    ...
    parent_approval_cb = parent_sudo_cb = parent_clarify_cb = None
    setters = None
    try:
        get_approval, get_sudo, get_clarify, set_approval, set_sudo, set_clarify = _callback_api()
        parent_approval_cb = get_approval()
        parent_sudo_cb = get_sudo()
        parent_clarify_cb = get_clarify()
        setters = (set_approval, set_sudo, set_clarify)
    except Exception:
        logger.debug("Could not capture parent approval/sudo/clarify callbacks", exc_info=True)

    def _runner(*args, **kwargs):
        def _inner():
            if setters is not None:
                set_approval, set_sudo, set_clarify = setters
                try:
                    if parent_approval_cb is not None:
                        set_approval(parent_approval_cb)
                    if parent_sudo_cb is not None:
                        set_sudo(parent_sudo_cb)
                    if parent_clarify_cb is not None:
                        set_clarify(parent_clarify_cb)
                except Exception:
                    logger.debug(
                        "Failed to install propagated approval/sudo/clarify callbacks",
                        exc_info=True,
                    )
            try:
                ...
                if setters is not None:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Failed to clear propagated approval/sudo/clarify callbacks",
                            exc_info=True,
                        )
