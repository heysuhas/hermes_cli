...
it is automatically downloaded from GitHub releases to $SR_HOME/bin/tirith.
...
def _get_sr_home() -> str:
    """Return the SR home directory, respecting SR_HOME env var."""
    return str(get_sr_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sr_home(), ".tirith-install-failed")
...
def _sr_bin_dir() -> str:
    """Return $SR_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sr_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SR_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sr_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SR_HOME/bin/tirith
    ...
    sr_bin = os.path.join(_sr_bin_dir(), "tirith")
    if os.path.isfile(sr_bin) and os.access(sr_bin, os.X_OK):
        _resolved_path = sr_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sr_bin
