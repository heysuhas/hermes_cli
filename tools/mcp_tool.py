...
transport, discovers their tools, and registers them into the sr-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.sr/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.sr/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach SR' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            sr_home = os.path.expanduser(
                os.getenv(
                    "SR_HOME", os.path.join(os.path.expanduser("~"), ".sr")
                )
            )
            candidates = [
                os.path.join(sr_home, "node", "bin", resolved_command),
                ...
                # image (which the SR Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → SR MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that SR gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through SR' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (SR_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit SR_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into SR' logging (agent.log via sr_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'sr-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'sr-agent[all]'"
            )
        ...
        # ungraceful exit of this SR process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through SR' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until SR is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `sr mcp login {server_name}` (or delete the tokens "
            f"file under ~/.sr/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local SR_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_sr_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_sr_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local SR_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_sr_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the SR config file.
    ...
    ``os.environ`` (which includes ``~/.sr/.env`` loaded at startup).
    """
    try:
        from sr_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("SR_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through SR' MEDIA: tag convention and out to messaging
            ...
            # SR' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves SR's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. SR uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the SR registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``sr tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the SR registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple sr
