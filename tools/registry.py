...
class ToolRegistry:
    ...
    def register(
        ...
        """
        # Enterprise blocked tools/toolsets filter
        blocked_names = {
        ...
        toolset_lower = toolset.lower() if toolset else ""
        if (
            name in blocked_names 
            or toolset in blocked_names
            or any(b in name_lower or b in toolset_lower for b in blocked_names if b != "apple")
        ):
            # Allow outlook tools, do not block outlook!
            is_outlook = "outlook" in name_lower or "outlook" in toolset_lower
            if not is_outlook:
                logger.warning("Tool '%s' from toolset '%s' is BLOCKED by enterprise policy.", name, toolset)
                return
