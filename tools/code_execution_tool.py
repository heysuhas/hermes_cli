...
Lets the LLM write a Python script that calls SR tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `sr_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `sr_tools.py` with file-based RPC stubs
...
# available on every platform SR itself runs on.
...
# match a safe prefix, the operational SR_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "SR_" prefix was deliberately removed (#27303) — it leaked
# SR_*-named config that lacks a secret substring (e.g. SR_BASE_URL,
# SR_KANBAN_DB, SR_*_WEBHOOK).  The child only needs the few
# location/profile vars in _SR_CHILD_ALLOWED below; SR_RPC_SOCKET /
# SR_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational SR_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set sr_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_SR_CHILD_ALLOWED = frozenset({
    "SR_HOME",
    "SR_PROFILE",
    "SR_CONFIG",
    "SR_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational SR_* vars (_SR_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret SR_* vars dropped by the tightened allowlist (#27303). The
    # broad "SR_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like SR_KANBAN_DB / SR_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("SR_"):
            ...
            # allowlist — a deliberately-dropped SR_* var.
            _dropped_sr.append(k)
    if _dropped_sr:
        logger.debug(
            "execute_code: dropped %d non-allowlisted SR_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-SR-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-SR-profile soft guard."""',
...
def generate_sr_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the sr_tools.py stub module.
...
"""Auto-generated SR tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SR_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("SR_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated SR tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SR_RPC_DIR") or os.path.join(tempfile.gettempdir(), "sr_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated sr_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/sr_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_sr_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/sr_tools.py", tools_src)
        ...
        env_prefix = (
            f"SR_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"SR_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SR_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with sr_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="sr_sandbox_")
    ...
    # selector from SR_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"sr_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated sr_tools module.
        ...
        tools_src = generate_sr_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "sr_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SR_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SR_RPC_SOCKET"] = rpc_endpoint
        child_env["SR_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the sr-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from sr_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _sr_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _sr_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # SR_TIMEZONE is an internal SR setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SR_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SR_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.sr/.env')).
...
def _load_config() -> dict:
    ...
    ``sr --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (sr-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``sr tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and sr-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.sr/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call SR tools programmatically. "
        ...
        f"Available via `from sr_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into sr_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from sr_tools import {import_str}` "
