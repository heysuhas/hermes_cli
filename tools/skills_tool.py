    ...
    for scan_dir in dirs_to_scan:
        for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
            ...
            try:
                ...
                if not _corporate_skill_allowed(name):
                    continue
...
_CORPORATE_BUNDLED_SKILLS = frozenset(
    {
        "outlook-local",
        "local-files",
        "pdf",
...
def _corporate_skill_allowed(name: str) -> bool:
    try:
        ...
        return True
    bare = str(name).replace("\\", "/").rstrip("/").rsplit("/", 1)[-1]
    if bare in _CORPORATE_BUNDLED_SKILLS:
        return True
    try:
    ...
    try:
        ...
        entry = HubLockFile().get_installed(bare)
        if entry:
            return entry.get("source") == "corporate-broker"
    except Exception:
        pass
    # Non-bundled, non-hub skills are private local skills.
    return True
            ...
            return json.dumps(
                {
                    "success": False,
                    "error": (
                        "This bundled or externally sourced skill is not "
                        "approved for corporate-local mode."
                    ),
                },
                ensure_ascii=False,
            )
