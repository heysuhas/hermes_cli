...
def _get_clarify_callback():
    return getattr(_callback_tls, "clarify", None)
...
def set_clarify_callback(cb):
    """Register a callback for clarify prompts.

    Per-thread scope.
    """
    _callback_tls.clarify = cb
...
def terminal_tool(
    ...
    try:
        ...
        try:
            ...
            _corporate_policy = get_corporate_policy()
            _corporate_error = _corporate_policy.terminal_command_error(command)
            if not _corporate_error:
                _corporate_error = (
                    _corporate_policy.terminal_referenced_path_error(command)
                )
        except Exception:
            _corporate_policy = None
            _corporate_error = None
        if _corporate_error:
            import hashlib
            ...
            try:
                record_compliance(
                    ComplianceEvent(
                        ...
                        action="terminal",
                        success=False,
                        policy_violation="network_command",
            ...
            return json.dumps(
                {
                    "output": "",
                    "exit_code": -1,
                    "error": _corporate_error,
                    "status": "blocked",
                },
                ensure_ascii=False,
            )

        if _corporate_policy is not None and _corporate_policy.enabled and not force:
            _corporate_approval = _corporate_policy.terminal_approval_reason(command)
            if _corporate_approval:
                    ...
                    return json.dumps(
                        {
                            ...
                            "exit_code": -1,
                            "error": (
                                "Corporate package installation approval was "
        ...
        if _corporate_policy is not None and _corporate_policy.enabled:
            work_target = workdir or cwd
            path_error = _corporate_policy.path_error(work_target)
            if path_error:
                return json.dumps(
                    {
                        "output": "",
...
def _terminal_schema_overrides():
    try:
        ...
        if is_corporate_mode():
            return {
                "description": (
                    "Execute governed local shell commands inside approved "
                    "working roots. Public-network commands, direct downloads, "
                    "remote package installation, and cloud access are blocked "
                    "by corporate policy. Prefer file and Office tools when a "
                    "structured capability exists."
                )
            }
    except Exception:
        pass
