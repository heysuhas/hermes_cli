...
def _detect_image_mime_type(image_path: Path) -> Optional[str]:
    ...
    if header.startswith(b"%PDF") or image_path.suffix.lower() == ".pdf":
        return "application/pdf"
    return None


def _pdf_pages_to_base64_pngs(pdf_path: Path, max_pages: int = 5) -> list[str]:
    """Render up to max_pages of a PDF file to base64-encoded PNG data URLs.

    Uses fitz (PyMuPDF) to render pages at 150 DPI.
    """
    try:
        import fitz
    except ImportError:
        logger.error("fitz (PyMuPDF) is not installed; cannot render PDF to images")
        return []
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if detected_mime_type == "application/pdf":
            image_data_urls = _pdf_pages_to_base64_pngs(temp_image_path, max_pages=5)
            if not image_data_urls:
                return tool_error(
                    "Failed to extract pages from PDF. Ensure fitz is installed and the PDF is valid.",
                    success=False,
                )

            text_part = (
                f"PDF ({len(image_data_urls)} pages) loaded into your context — you can see it natively now. "
                "Use your built-in vision to answer the user."
            )
            if isinstance(question, str) and question.strip():
                text_part += f"\n\nQuestion: {question.strip()}"
            ...
            summary = (
                f"PDF ({len(image_data_urls)} pages) attached natively for the main model. "
                "Answer using built-in vision."
            )
...
async def vision_analyze_tool(
    ...
    try:
        ...
        if detected_mime_type == "application/pdf":
            logger.info("Converting PDF pages to base64 PNGs...")
            image_data_urls = _pdf_pages_to_base64_pngs(temp_image_path, max_pages=5)
            if not image_data_urls:
            ...
            image_data_url = ""  # defined to avoid NameError
        else:
