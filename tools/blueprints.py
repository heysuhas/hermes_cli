...
`sr skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.sr.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.sr.blueprint`` is a mapping containing
    ...
    sr = meta.get("sr") if isinstance(meta, dict) else None
    blueprint = sr.get("blueprint") if isinstance(sr, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.sr.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.sr.blueprint`` block) they can hand
    to ``sr skills publish`` to share. ``body`` is the plain-language
