...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``sr tools`` repaint, tool-registration scans,
    ...
    2. ``~/.sr/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_sr_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.sr/.env`` first, then ``os.environ``.

    Wraps :func:`sr_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from sr_cli.config import get_env_value as _sr_get_env_value

        value = _sr_get_env_value(name)
...
def sr_xai_user_agent() -> str:
    """Return a stable SR-specific User-Agent for xAI HTTP calls."""
    try:
        from sr_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"SR-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level SR config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per SR home, then mark it seen."""
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers SR-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``sr_cli.config.get_env_value`` so keys
    stored in ``~/.sr/.env`` (the standard SR location) are honored —
