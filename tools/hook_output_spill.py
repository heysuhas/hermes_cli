...
* Never raises. Any I/O error (disk full, permission denied, missing
  SR_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_sr_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: SR_HOME env var, then ~/.sr
            home = os.environ.get("SR_HOME") or os.path.expanduser("~/.sr")
