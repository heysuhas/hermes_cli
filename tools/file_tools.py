...
def _resolve_path(filepath: str, task_id: str = "default") -> Path:
    ...
    error = request_path_access(
        resolved,
        purpose="access this local file or folder",
    )
    if error:
...
def _corporate_path_error(filepath: str, task_id: str = "default") -> str | None:
    """Apply the corporate root boundary to every file-tool entry point."""
    from agent.corporate_path_access import request_path_access

    resolved = _resolve_path_for_task(filepath, task_id)
    return request_path_access(
        resolved,
        purpose="access this local file or folder",
    )
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        corporate_err = _corporate_path_error(path, task_id)
        if corporate_err:
            return tool_error(corporate_err)
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    for _p in _paths_to_check:
        corporate_err = _corporate_path_error(_p, task_id)
        if corporate_err:
            return tool_error(corporate_err)
