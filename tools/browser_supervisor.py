...
One ``CDPSupervisor`` runs per SR ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach SR logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "sr-dialog-bridge.invalid"
...
(() => {
  if (window.__srDialogBridgeInstalled) return;
  window.__srDialogBridgeInstalled = true;
  const ENDPOINT = "http://sr-dialog-bridge.invalid/";
