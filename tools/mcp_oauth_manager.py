...
`mcp_tool.py`, and `sr_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# SRMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_sr_provider_class() -> Optional[type]:
    ...
    class SRMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._sr_server_name = server_name
            ...
            self._sr_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SRTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, SRTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import SRTokenStorage
                        if isinstance(storage, SRTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._sr_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SRTokenStorage
            if not isinstance(storage, SRTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``sr mcp reauth``.
            ...
            back to ``sr mcp reauth``.
            """
            try:
                if self._sr_preregistered:
                    return
    ...
    return SRMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SR_PROVIDER_CLS: Optional[type] = _make_sr_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`SRMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `sr mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``sr mcp remove <name>`` and (indirectly) by
        ``sr mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
