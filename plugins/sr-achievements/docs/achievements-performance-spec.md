# SR Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: sr-achievements plugin
...
### `GET /api/plugins/sr-achievements/achievements`
...
### `POST /api/plugins/sr-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/sr-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/sr-achievements/overview`
...
- Persist plugin data under:
  - `~/.sr/plugins/sr-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.sr/plugins/sr-achievements/scan_checkpoint.json`
