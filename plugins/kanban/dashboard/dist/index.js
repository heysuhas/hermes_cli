/**
 * SR Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.sr/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SR_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SR_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "sr-kanban-dot-triage",
    todo: "sr-kanban-dot-todo",
    ready: "sr-kanban-dot-ready",
    running: "sr-kanban-dot-running",
    blocked: "sr-kanban-dot-blocked",
    done: "sr-kanban-dot-done",
    archived: "sr-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-sr-task";
  ...
  const DOCS_URL = "https://sr-agent.samsung.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://sr-agent.samsung.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "sr.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="sr-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])sr-kanban-md-code\1/i.test(attrs)) {
      return ' class="sr-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "sr-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "sr-kanban-md",
  ...
  // standard `drop` event and our `sr-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("sr-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("sr-kanban-column--drop");
          if (target) target.classList.add("sr-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("sr-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("sr-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("sr-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__SR_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "sr-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "sr-kanban-attention",
        "sr-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "sr-kanban-attention-bar" },
        h("span", { className: "sr-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "sr-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "sr-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "sr-kanban-attention-row",
                  "sr-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "sr-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "sr-kanban-attention-row-id" }, task.id),
                h("span", { className: "sr-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "sr-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "sr-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "sr-kanban-diag-action-btn",
      action.suggested ? "sr-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " sr-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".sr-kanban-drawer-comment-row input, .sr-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "sr-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("sr-kanban-diag", sevClass) },
      h("div", { className: "sr-kanban-diag-header" },
        h("span", { className: "sr-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "sr-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "sr-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "sr-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "sr-kanban-diag-data-row" },
                  h("span", { className: "sr-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "sr-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "sr-kanban-diag-data-row" },
                h("span", { className: "sr-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "sr-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "sr-kanban-diag-reassign-row" },
            h("span", { className: "sr-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "sr-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "sr-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "sr-kanban-diag-msg",
              msg.ok ? "sr-kanban-diag-msg--ok" : "sr-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "sr-kanban-section" },
      h("div", { className: "sr-kanban-section-head-row" },
        h("span", { className: "sr-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "sr-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "sr-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "sr-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "sr-kanban-docs-link",
      title: "Open SR Kanban docs in a new tab",
      "aria-label": "SR Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `sr kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "sr-kanban-msg-ok" : "sr-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "sr-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "sr-kanban-boardswitcher" },
      h("div", { className: "sr-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "sr-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "sr-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "sr-kanban-dialog-title" },
        ...
        h("div", { className: "sr-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned SR profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "sr-kanban-bulk" },
      h("span", { className: "sr-kanban-bulk-count" },
      ...
      h("div", { className: "sr-kanban-bulk-priority",
      ...
      h("div", { className: "sr-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different SR profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "sr-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sr-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("sr-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "sr-kanban-trash",
        dragOver ? "sr-kanban-trash--drop" : "",
        props.draggingTaskId ? "sr-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "sr-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "sr-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".sr-kanban-card")) return true;
      if (target.closest && target.closest(".sr-kanban-column-add")) return true;
      if (target.closest && target.closest(".sr-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("sr-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("sr-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".sr-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "sr-kanban-columns",
        isScrollable ? "sr-kanban-columns--scrollable" : "",
        isPanning ? "sr-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("sr-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("sr-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "sr-kanban-column",
        dragOver ? "sr-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "sr-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "sr-kanban-col-check",
      ...
      h("div", { className: "sr-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "sr-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "sr-kanban-lane" },
                  h("div", { className: "sr-kanban-lane-head" },
                    h("span", { className: "sr-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "sr-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "sr-kanban-card--stale-red";
    if (age >= tier.amber) return "sr-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".sr-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "sr-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "sr-kanban-card",
        props.selected ? "sr-kanban-card--selected" : "",
        props.failed ? "sr-kanban-card--failed" : "",
        props.draggingSource ? "sr-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "sr-kanban-card-content" },
          h("div", { className: "sr-kanban-card-row" },
            h("label", {
              className: "sr-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "sr-kanban-card-check",
            ...
            h("span", { className: "sr-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or sr kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "sr-kanban-warning-badge",
                    "sr-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "sr-kanban-progress",
                    progress.done === progress.total ? "sr-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "sr-kanban-needs-assignee",
          ...
          h("div", { className: "sr-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "sr-kanban-card-row sr-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "sr-kanban-assignee",
                            title: `Assigned to SR profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "sr-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "sr-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "sr-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "sr-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "SR profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "SR profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "sr-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "sr-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "sr-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "sr-kanban-drawer-close",
        ...
        data ? h("div", { className: "sr-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "sr-kanban-section" },
      h("div", { className: "sr-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "sr-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "sr-kanban-drawer-body" },
      h("div", { className: "sr-kanban-drawer-title" },
        h("span", { className: cn("sr-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "sr-kanban-section" },
        h("div", { className: "sr-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "sr-kanban-section" },
        h("div", { className: "sr-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "sr-kanban-comment" },
                h("div", { className: "sr-kanban-comment-head" },
                  h("span", { className: "sr-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "sr-kanban-comment-ago" },
      ...
      h("div", { className: "sr-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "sr-kanban-event",
              isDiag ? "sr-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "sr-kanban-event-header" },
                  h("span", { className: "sr-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "sr-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "sr-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "sr-kanban-event-header-plain" },
                  h("span", { className: "sr-kanban-event-kind" }, e.kind),
                  h("span", { className: "sr-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "sr-kanban-event-phantom-row" },
                  h("span", { className: "sr-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "sr-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "sr-kanban-event-payload" },
    ...
    return h("div", { className: "sr-kanban-section" },
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `sr-kanban-run--${r.outcome || r.status || "ended"}`
          : "sr-kanban-run--active";
        return h("div", { key: r.id, className: cn("sr-kanban-run", outcomeClass) },
          h("div", { className: "sr-kanban-run-head" },
            h("span", { className: "sr-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "sr-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "sr-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "sr-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "sr-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "sr-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "sr-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "sr-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "sr-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "sr-kanban-pre sr-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "sr-kanban-meta-row" },
      h("span", { className: "sr-kanban-meta-label" }, props.label),
      h("span", { className: "sr-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "sr-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sr-kanban-meta-row" },
        h("span", { className: "sr-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "sr-kanban-meta-value sr-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "sr-kanban-meta-row" },
        h("span", { className: "sr-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "sr-kanban-meta-value sr-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "sr-kanban-section" },
      h("div", { className: "sr-kanban-section-head-row" },
        h("span", { className: "sr-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "sr-kanban-section" },
      h("div", { className: "sr-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "sr-kanban-deps-row" },
        h("span", { className: "sr-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "sr-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "sr-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "sr-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "sr-kanban-dep-chip-x",
      ...
      h("div", { className: "sr-kanban-deps-row" },
        h("span", { className: "sr-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "sr-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "sr-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "sr-kanban-section" },
      ...
      h("div", { className: "sr-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "sr-kanban-home-sub sr-kanban-home-sub--on"
              : "sr-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__SR_PLUGINS__ && typeof window.__SR_PLUGINS__.register === "function") {
    window.__SR_PLUGINS__.register("kanban", KanbanPage);
  }
})();
