...
Auto-tracks and cleans up ephemeral files created during SR Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SR_HOME` and `/tmp/sr-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SR_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SR_HOME` or `/tmp/sr-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SR_HOME/disk-cleanup/` is itself excluded
- `$SR_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
