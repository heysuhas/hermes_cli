"""disk_cleanup — ephemeral file cleanup for SR Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SR_HOME and /tmp/sr-*
Never touches: ~/.sr/logs/ or any system directory.
"""
...
try:
    ...
    def get_sr_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".sr").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SR_HOME/logs/``."""
    return get_sr_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SR_HOME or ``/tmp/sr-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    sr_home = get_sr_home()
    try:
        path.resolve().relative_to(sr_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/sr-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("sr-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "sr-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $SR_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so SR_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SR_HOME, but never recurse into known
    # durable state trees.  Some installs place the SR checkout, venv,
    # and desktop build under SR_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    sr_home = get_sr_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in sr_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sr_home = get_sr_home()
    try:
        rel = path.resolve().relative_to(sr_home)
        ...
        # Path isn't under SR_HOME (e.g. /tmp/sr-*) — fall through.
        pass
