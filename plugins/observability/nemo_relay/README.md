...
Optional SR observability plugin that maps SR observer hooks to
...
not replace SR Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets SR emit NeMo Relay lifecycle events for
the work SR already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, SR Agent can:

- Preserve SR execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
sr plugins enable observability/nemo_relay
```

The `SR_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `SR_HOME` that the
agent run will use:

```bash
env SR_HOME=/tmp/sr-nemo-relay-test \
  sr plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`SR_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`SR_HOME` is the SR profile/config home used by both
`sr plugins enable ...` and the later `sr chat ...` run. If unset,
SR uses the user's default home, usually `~/.sr`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export SR_HOME=/tmp/sr-nemo-relay-test
sr plugins enable observability/nemo_relay
sr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `sr` command you run is built from the
...
uv run sr plugins enable observability/nemo_relay
uv run sr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/sr_agent-*.whl
...
The plugin can configure exporters directly from `SR_NEMO_RELAY_*`
...
```bash
export SR_NEMO_RELAY_ATOF_ENABLED=1
export SR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export SR_NEMO_RELAY_ATIF_ENABLED=1
export SR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `SR_NEMO_RELAY_ATOF_FILENAME`
- `SR_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `SR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `SR_NEMO_RELAY_ATIF_AGENT_NAME`
- `SR_NEMO_RELAY_ATIF_AGENT_VERSION`
- `SR_NEMO_RELAY_ATIF_MODEL_NAME`
- `SR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export SR_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `SR_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`SR_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`SR_NEMO_RELAY_ATIF_*` fallback setup is also skipped so SR does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
SR keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, SR routes LLM and tool
...
For the full generic SR middleware contract, see
...
export SR_HOME=/tmp/sr-nemo-relay-docs/sr-home
mkdir -p "$SR_HOME"

cat > "$SR_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent SR session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export SR_NEMO_RELAY_ATOF_ENABLED=1
export SR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sr-nemo-relay-docs/subagent/atof
export SR_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export SR_NEMO_RELAY_ATOF_MODE=overwrite
export SR_NEMO_RELAY_ATIF_ENABLED=1
export SR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sr-nemo-relay-docs/subagent/atif
export SR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export SR_NEMO_RELAY_ATIF_AGENT_NAME='SR Agent E2E'
export SR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export SR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"sr.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "SR Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. SR dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/sr-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/sr-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/sr-nemo-relay-docs/workdir/beta.txt
cd /tmp/sr-nemo-relay-docs/workdir

export SR_NEMO_RELAY_ATOF_ENABLED=1
export SR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/sr-nemo-relay-docs/parallel/atof
export SR_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export SR_NEMO_RELAY_ATOF_MODE=overwrite
export SR_NEMO_RELAY_ATIF_ENABLED=1
export SR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/sr-nemo-relay-docs/parallel/atif
export SR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export SR_NEMO_RELAY_ATIF_AGENT_NAME='SR Agent E2E'
export SR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- SR sessions map to `agent` scopes.
- SR API request hooks map to `llm` scope start/end events.
- SR tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses SR execution middleware to hand
...
```bash
export SR_NEMO_RELAY_PLUGINS_TOML=/tmp/sr-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, SR routes execution
...
SR provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> SR provider adapter next_call(...)

SR tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> SR tool dispatcher next_call(...)
```
...
middleware for a local SR run. This path requires a NeMo Relay runtime that
...
```bash
export SR_HOME=/tmp/sr-middleware-test/sr-home
mkdir -p "$SR_HOME" /tmp/sr-middleware-test/nemo-relay
...
cat > /tmp/sr-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/sr-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/sr-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "SR Middleware E2E"
agent_version = "local"
...
export SR_NEMO_RELAY_PLUGINS_TOML=/tmp/sr-middleware-test/nemo-relay/plugins.toml
