...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=sr"
...
def _load_supermemory_config(sr_home: str) -> dict:
    config = _default_config()
    config_path = Path(sr_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, sr_home: str) -> None:
    config_path = Path(sr_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "sr"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes SR writes into the "SR" Space in the Supermemory
        ...
        merged = {"sm_source": "sr", **(metadata or {})}
...
def _resolve_container_tag_for_setup(sr_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(sr_home)
...
def _probe_supermemory_connection(api_key: str, sr_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(sr_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(sr_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `sr memory setup`.
        # All other options are documented for $SR_HOME/supermemory.json
    ...
    def save_config(self, values, sr_home):
        ...
        _save_supermemory_config(sanitized, sr_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, sr_home)
    ...
    def post_setup(self, sr_home: str, config: dict) -> None:
        ...
        from sr_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(sr_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from sr_constants import get_sr_home
        self._sr_home = kwargs.get("sr_home") or str(get_sr_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._sr_home)
