...
- Supermemory API key from [app.supermemory.ai/integrations?connect=sr](http://app.supermemory.ai/integrations?connect=sr)
...
Config file: `$SR_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `sr` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `sr-{identity}` → `sr-coder`). |
...
All Supermemory API calls send `x-sm-source: sr`, and document writes stamp
`metadata.sm_source: sr`. This is a **functional routing key, not telemetry**:
it groups SR-written memories into a dedicated "SR" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per SR profile:

```json
{
  "container_tag": "sr-{identity}"
}
```

For a profile named `coder`, this resolves to `sr-coder`. The default profile resolves to `sr-default`. Without `{identity}`, all profiles share the same container.
