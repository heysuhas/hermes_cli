...
``{title, url, description, position}`` rows every other SR web
provider produces.
...
prefers SR-managed xAI Grok OAuth (via ``sr auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.sr/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``sr tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `sr auth` to sign in with "
