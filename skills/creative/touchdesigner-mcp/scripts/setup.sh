...
SR_HOME_DIR="${SR_HOME:-$HOME/.sr}"
SR_CFG="${SR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure SR config has twozero_td MCP entry ──
if [[ ! -f "$SR_CFG" ]]; then
    echo -e " ${FAIL} SR config not found at ${SR_CFG}"
    manual_steps+=("Create ${SR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in SR config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to SR config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SR_CFG} manually"); }
    manual_steps+=("Restart SR session to pick up config change")
fi
