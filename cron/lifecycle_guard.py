...
``sr gateway restart`` (or ``launchctl kickstart ai.sr.gateway``
or ``systemctl restart sr-gateway``).  When the cron fires, the
...
``sr cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``sr gateway``, ``launchctl ... sr-gateway``,
``systemctl ... sr-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_SR_GATEWAY=1``, and
``sr gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `sr gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:sr\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a sr-gateway label. macOS launchd
    # labels look like `ai.sr.gateway` / `sr-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated sr services (e.g.
    # `launchctl unload ai.sr.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bsr[.\-]?gateway)"
    # Branch C: systemctl ops on a sr-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bsr[.\-]?gateway)"
    # Branch D: pkill / kill targeting the sr gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<SR_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.sr/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_sr_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `sr gateway restart` from a shell outside "
