...
def looks_like_unexecuted_tool_action(
    agent,
    assistant_content: str,
    messages: List[Dict[str, Any]],
    current_turn_user_idx: int,
) -> bool:
    """Return True when the model promises an action but calls no tool.

    This is deliberately domain-neutral. It does not classify the user's
    request or map words such as "email" to a specific function. Instead it
    enforces the model's own explicit commitment ("I'll use...", "let me
    check...", etc.): when tools are available and no tool result exists in
    the current turn, a future-tense action acknowledgement is incomplete.

    The conversation loop uses this signal for one API-local recovery retry
    with ``tool_choice=required``. The rejected acknowledgement is not written
    to durable history and the cached system prompt remains byte-stable.
    """
    ...
    assistant_text = agent._strip_think_blocks(assistant_content or "").strip().lower()
    if not assistant_text or len(assistant_text) > 2000:
        return False

    commitment = re.search(
        r"\b("
        r"i['â€™]ll|i will|let me|i(?:'m| am) going to|"
        r"i can do that|i can help with that"
        r")\b",
        assistant_text,
    )
    if not commitment:
    ...
    # Require an action verb after the commitment. This avoids retrying benign
    # conversational promises ("I'll be concise") while covering arbitrary
    # tool domains without maintaining a service- or product-specific map.
    remainder = assistant_text[commitment.end():]
    return bool(
        re.search(
            r"\b("
            r"use|call|run|execute|read|write|create|update|delete|list|"
            r"search|find|check|inspect|open|fetch|get|send|save|load|"
            r"query|retrieve|analy[sz]e|review|test|build|edit|modify|download|upload"
            r")\b",
            remainder,
        )
    )
...
def looks_like_false_tool_capability_denial(assistant_content: str) -> bool:
    """Detect a model denying access despite a clearly matched live tool."""
    ...
    return bool(
        re.search(
            r"\b("
            r"i (?:do not|don't) have (?:direct )?access|"
            r"i (?:do not|don't) have (?:a|the) tool|"
            r"i (?:do not|don't) have the capability|"
            r"i cannot directly|i can't directly|i am unable to|i'm unable to|"
            r"i (?:cannot|can't) (?:actually )?(?:read|retrieve|access)|"
            r"no (?:available |appropriate |email |matching )?tool|"
            r"without access to (?:a|the) .* tool"
            r")\b",
            text,
        )
    )
...
def select_recovery_tool(
    ...
    """Select one clearly relevant tool from the live function schemas.

    Uses Hermes's existing BM25 tool catalog. Ambiguous requests deliberately
    return ``None`` so recovery can require any tool without guessing.
    """
    try:
        from tools.tool_search import build_catalog, search_catalog_scored
    except Exception:
        return None

    ranked = search_catalog_scored(build_catalog(tool_defs), user_message, limit=3)
    if not ranked:
        return None
    from tools.tool_search import _tokenize
    _RECOVERY_STOPWORDS = {
        "a", "an", "and", "are", "can", "could", "for", "i", "in", "is",
        "it", "me", "my", "of", "on", "please", "the", "to", "you",
    }
    meaningful_query_tokens = {
        token for token in _tokenize(user_message)
        if token not in _RECOVERY_STOPWORDS and not token.isdigit()
    }
    ...
    # Require a meaningful lead so generic prompts such as "search" do not
    # force an arbitrary search implementation.
    if first_score < 1.0 or first_score < (second_score * 1.2):
        return None
    return first.name


def build_compact_tool_recovery_request(
    tool_defs: List[Dict[str, Any]],
    expected_tool: str,
    user_message: str,
) -> Optional[Dict[str, Any]]:
    """Build a bounded schema-only repair request for a missed tool call.

    The request is API-local and does not mutate durable conversation history.
    Keeping only the selected live schema avoids prompt saturation on small
    local models while leaving argument selection to the model.
    """
    selected = next(
        (
            tool_def
            for tool_def in tool_defs
            if (tool_def.get("function") or {}).get("name") == expected_tool
        ),
        None,
    )
    if selected is None:
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Repair a missed function call. Call the one provided "
                    "function using arguments grounded in the user's request. "
                    "Do not answer with prose and do not invent results."
                ),
            },
...
def parse_forced_textual_tool_call(
    assistant_content: str,
    expected_tool: str,
    allowed_arguments: Optional[set[str]] = None,
) -> Optional[Dict[str, Any]]:
    """Parse a weak-provider textual rendering of one already-forced tool.

    Accepts function-style and shell-like text emitted by weak compatible
    providers. Only the semantically selected tool may be reconstructed, and
    only arguments present in its live schema survive. Values are parsed as
    literals where possible; executable expressions are rejected and no
    textual command is ever executed.
    """
    if not assistant_content or not expected_tool:
        return None
    arguments: Dict[str, Any] = {}

    def normalize_argument_name(name: str) -> Optional[str]:
        normalized = name.replace("-", "_")
        if allowed_arguments is None or normalized in allowed_arguments:
            return normalized
        # Common protocol vocabulary used by OpenAI-compatible providers.
        # Apply aliases only when the destination exists in the live schema.
        aliases = {
            "count": "limit",
            "max_results": "limit",
            "page_size": "limit",
        }
        destination = aliases.get(normalized)
        if destination and destination in allowed_arguments:
            return destination
        return None

    # Some compatible providers serialize a function call as JSON text rather
    # than populating ``tool_calls``. Accept only the already-selected function
    # and filter its arguments through the live schema.
    for raw_json in re.findall(r"(?s)```(?:json)?\s*(\{.*?\})\s*```", assistant_content):
        try:
            rendered = json.loads(raw_json)
        except (TypeError, ValueError):
            continue
        rendered_name = rendered.get("tool_name") or rendered.get("name")
        rendered_arguments = rendered.get("arguments")
        if rendered_name != expected_tool or not isinstance(rendered_arguments, dict):
            continue
        for key, value in rendered_arguments.items():
            normalized_key = normalize_argument_name(str(key))
            if normalized_key:
                arguments[normalized_key] = value
        return {"name": expected_tool, "arguments": arguments}

    # URI-style tool references are emitted by some local chat templates.
    # The URI carries no arguments, so schema defaults apply.
    if re.search(
        rf"\bhermes_tools://{re.escape(expected_tool)}\b",
        assistant_content,
    ):
        return {"name": expected_tool, "arguments": {}}

    # Bracketed pseudo-calls are another local-template representation:
    # ``[Calling function_name: {"arg": "value"}]``.
    bracketed_call = re.search(
        rf"(?is)\[\s*calling\s+{re.escape(expected_tool)}\s*:\s*(\{{.*?\}})\s*\]",
        assistant_content,
    )
    if bracketed_call:
        try:
            rendered_arguments = json.loads(bracketed_call.group(1))
        except (TypeError, ValueError):
            rendered_arguments = None
    ...
    match = re.search(
        r"(?ims)^\s*(?:tool:)?([A-Za-z_][A-Za-z0-9_.:-]*)\s*\((.*?)\)"
        r"\s*(?=$|```)",
        assistant_content,
    )
    if match and match.group(1) == expected_tool:
        raw_args = match.group(2).strip()
        if not raw_args:
            return {"name": expected_tool, "arguments": {}}
        try:
            expression = ast.parse(f"_tool_({raw_args})", mode="eval").body
        except SyntaxError:
            return None
        ...
        for keyword in expression.keywords:
            if keyword.arg is None:
                return None
            normalized_key = normalize_argument_name(keyword.arg)
            if not normalized_key:
                continue
            try:
                arguments[normalized_key] = ast.literal_eval(keyword.value)
            except (ValueError, TypeError):
                return None
        return {"name": expected_tool, "arguments": arguments}

    # Weak local models also emit shell-like pseudo invocations inside a code
    # fence: ``mail_list_messages limit=2``. Parse tokens only; never execute
    # the line. Parameters absent from the selected tool's live schema are
    # ignored.
    line_match = re.search(
        rf"(?im)^\s*{re.escape(expected_tool)}(?:\s+([^\r\n`]+))?\s*$",
        assistant_content,
    )
    if line_match:
        try:
            tokens = shlex.split(line_match.group(1) or "", posix=True)
        except ValueError:
            return None
        for token in tokens:
            if "=" not in token:
            ...
            normalized_key = normalize_argument_name(key)
            if not normalized_key:
                continue
            try:
                arguments[normalized_key] = ast.literal_eval(raw_value)
            except (ValueError, SyntaxError):
                arguments[normalized_key] = raw_value
        return {"name": expected_tool, "arguments": arguments}

    # Last compatibility shape: the provider substitutes a CLI/skill command
    # even though a native function was explicitly forced. Salvage only flags
    # whose normalized names exist in that function's schema; the command
    # itself is ignored and never executed.
    if allowed_arguments:
        fenced_lines = re.findall(r"```[A-Za-z0-9_-]*\s*\n([^\n`]+)", assistant_content)
        for line in fenced_lines:
            ...
            while index < len(tokens):
                ...
                if token.startswith("--"):
                    ...
                    key, raw_value = token.split("=", 1)
                normalized_key = normalize_argument_name(key)
                if normalized_key and raw_value is not None:
                    try:
                        salvaged[normalized_key] = ast.literal_eval(raw_value)
                    except (ValueError, SyntaxError):
                        salvaged[normalized_key] = raw_value
                index += 1
            if salvaged:
                return {"name": expected_tool, "arguments": salvaged}
    return None
