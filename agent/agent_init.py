...
def init_agent(
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from agent.corporate_policy import (
                get_corporate_policy,
                validate_outbound_url,
            )

            _corporate_policy = get_corporate_policy()
            if _corporate_policy.enabled:
                allowed, reason = validate_outbound_url(str(agent.base_url or ""))
                if not allowed:
                    raise RuntimeError(
                        f"Corporate local mode rejected model endpoint "
                        f"{agent.base_url!r}: {reason}"
                    )
        except RuntimeError:
            raise
        except Exception:
    ...
    if agent._ollama_num_ctx:
        ...
        if getattr(agent, "context_compressor", None) is not None:
            ...
            cc.context_length = agent._ollama_num_ctx
            if hasattr(cc, "_compute_threshold_tokens"):
                cc.threshold_tokens = cc._compute_threshold_tokens(
                    agent._ollama_num_ctx, cc.threshold_percent, cc.max_tokens
                )
                cc.tail_token_budget = int(cc.threshold_tokens * cc.summary_target_ratio)
                cc.max_summary_tokens = min(
                    int(agent._ollama_num_ctx * 0.05), 500
                )
