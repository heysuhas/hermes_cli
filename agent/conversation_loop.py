...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    reserve = max(256, min(2048, runtime_ctx // 32))
    remaining_output = runtime_ctx - request_tokens - reserve
    runtime_too_small = runtime_ctx < MINIMUM_CONTEXT_LENGTH
    request_too_large = remaining_output < 256
    if not runtime_too_small and not request_too_large:
        return None

    logger.warning(
        "Ollama runtime context too small for Hermes tool use: "
        "model=%s provider=%s base_url=%s runtime_context=%d "
        "minimum_context=%d estimated_request_tokens=%d remaining_output=%d "
        "tool_count=%d "
        "session=%s",
        model,
        provider,
    ...
    if request_too_large:
        detail = (
            f"The current prompt and live tool schemas need approximately "
            f"{request_tokens:,} tokens, leaving fewer than 256 tokens for "
            "Gemma's reasoning and answer."
        )
    else:
        detail = (
            f"Hermes needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens for "
            "reliable tool use."
        )
    context_phrase = (
        f"with only {runtime_ctx:,} tokens"
        if runtime_too_small
        else f"with {runtime_ctx:,} tokens"
    )
    return (
        f"Ollama loaded `{model}` {context_phrase} of runtime "
        f"context. {detail}\n\n"
...
def run_conversation(
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # API-local recovery after a model promised to act but emitted plain
        # text instead of a function call. Keep this out of ``messages`` so we
        # preserve durable role history and out of the system prompt so the
        # cached prefix remains byte-stable.
        compact_recovery_tool_schema = None
        compact_grounded_response = False
        if tool_recovery_instruction:
            for am in reversed(api_messages):
                ...
                if isinstance(content, str):
                    am["content"] = f"{content}\n\n{tool_recovery_instruction}"
                elif isinstance(content, list):
                    am["content"] = [
                        *content,
                        {"type": "text", "text": tool_recovery_instruction},
                    ]
                break
            if forced_recovery_tool:
                ...
                compact_recovery = build_compact_tool_recovery_request(
                    agent.tools or [],
                    forced_recovery_tool,
                    original_user_message,
                )
                if compact_recovery:
                    api_messages = compact_recovery["messages"]
                    compact_recovery_tool_schema = compact_recovery["tools"]
        elif compact_recovery_response_pending:
            api_messages = [
                {
                    "role": "system",
                    "content": (
                        "Answer the user's request using only the function result "
                        "in this conversation. State that the data was retrieved "
                        "through the executed function. Do not claim the user "
                        "supplied it, do not deny the demonstrated capability, "
                        "and do not invent fields or results."
                    ),
                },
                *[dict(message) for message in messages[current_turn_user_idx:]],
            ]
            compact_grounded_response = True
            compact_recovery_response_pending = False
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if compact_recovery_tool_schema:
                    api_kwargs["tools"] = compact_recovery_tool_schema
                elif compact_grounded_response:
                    api_kwargs.pop("tools", None)
                    api_kwargs.pop("tool_choice", None)
                if force_next_tool_call and agent.valid_tool_names:
                    if forced_recovery_tool:
                        api_kwargs["tool_choice"] = {
                            "type": "function",
                            "function": {"name": forced_recovery_tool},
                        }
                    else:
                ...
                if finish_reason == "length":
                    ...
                    if not _trunc_reasoning and _trunc_msg is not None:
                        _provider_data = getattr(_trunc_msg, "provider_data", None)
                        if isinstance(_provider_data, dict):
                            _trunc_reasoning = (
                                _provider_data.get("reasoning")
                                or _provider_data.get("reasoning_content")
                            )
                    ...
                    _thinking_exhausted = (
                        ...
                        and bool(_has_think_tags or _trunc_reasoning)
                        and (
        ...
        try:
            ...
            # Some OpenAI-compatible local providers ignore tool_choice and
            # serialize the selected call as plain text. Repair only the exact
            # function Hermes already selected for this recovery attempt.
            if not assistant_message.tool_calls and forced_recovery_tool:
                from agent.agent_runtime_helpers import parse_forced_textual_tool_call
                allowed_arguments = None
                for tool_def in agent.tools or []:
                    fn = tool_def.get("function") or {}
                    if fn.get("name") != forced_recovery_tool:
                        continue
                    allowed_arguments = set(
                        ((fn.get("parameters") or {}).get("properties") or {}).keys()
                    )
                    break
                repaired_text_call = parse_forced_textual_tool_call(
                    assistant_message.content or "",
                    forced_recovery_tool,
                    allowed_arguments=allowed_arguments,
                )
                if repaired_text_call:
                    assistant_message.tool_calls = [
                        SimpleNamespace(
                            id=f"call_recovered_{uuid.uuid4().hex}",
                            type="function",
                            function=SimpleNamespace(
                                name=repaired_text_call["name"],
                                arguments=json.dumps(
                                    repaired_text_call["arguments"],
                                    ensure_ascii=False,
                                ),
                            ),
                        )
                    ...
                    assistant_message.content = ""
                    logger.info(
                        "Repaired forced textual tool call into structured call: %s",
                        forced_recovery_tool,
                    )
            
            # Check for tool calls
            if assistant_message.tool_calls:
                recovery_tool_call_succeeded = bool(forced_recovery_tool)
                # A required-tool recovery succeeded. Subsequent iterations
                # (after the tool result) return to normal automatic choice so
                # the model can provide a final answer instead of being forced
                # into an endless tool loop.
                force_next_tool_call = False
                forced_recovery_tool = None
                tool_recovery_instruction = ""
                if not agent.quiet_mode:
                ...
                if recovery_tool_call_succeeded:
                    compact_recovery_response_pending = True
                ...
                recovery_tool = agent._select_recovery_tool(original_user_message)
                should_recover_unexecuted_action = (
                    agent._looks_like_unexecuted_tool_action(
                        assistant_content=final_response,
                        messages=messages,
                        current_turn_user_idx=current_turn_user_idx,
                    )
                    or (
                        recovery_tool is not None
                        and agent._looks_like_false_tool_capability_denial(
                            final_response
                        )
                    )
                )
                if (
                    unexecuted_action_recoveries < 1
                    and should_recover_unexecuted_action
                ):
                    unexecuted_action_recoveries += 1
                    force_next_tool_call = True
                    forced_recovery_tool = recovery_tool
                    tool_recovery_instruction = (
                        "[Tool execution required: your previous response promised "
                        "an action but emitted no function call. Call one of the "
                        "available tools now. Do not print or simulate a command."
                        + (
                            f" Use `{forced_recovery_tool}` for this request."
                            if forced_recovery_tool
                            else ""
                        )
                        + "]"
                    ...
                    logger.info(
                        "Recovering unexecuted model action with required tool choice "
                        "(model=%s provider=%s)",
                        agent.model,
                        agent.provider,
                    )
                ...
                if force_next_tool_call and forced_recovery_tool:
                    logger.warning(
                        "Model did not execute forced recovery tool %s; "
                        "discarding ungrounded final content",
                        forced_recovery_tool,
                    )
                    final_response = (
                        "I couldn't execute the required tool, so I can't provide "
                        "a trustworthy result for this request."
                    )
