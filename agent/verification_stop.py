...
def _session_is_messaging_surface() -> bool:
    ...
    ``SR_SESSION_PLATFORM``; the CLI and TUI set ``SR_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``SR_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("SR_PLATFORM", "") or os.environ.get(
            "SR_SESSION_PLATFORM", ""
        )
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``SR_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("SR_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `sr-verify-` filename prefix, run it "
