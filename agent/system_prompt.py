...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    try:
        ...
        corporate_policy = get_corporate_policy()
    except Exception:
        corporate_policy = None
    if corporate_policy is not None and corporate_policy.enabled:
        stable = (
            "You are Hermes Corporate Local Assistant, running entirely on this "
            "employee's Windows laptop with a local Ollama model.\n"
            "Use the available local tools to complete work. You may read and "
            "edit approved local files, execute governed local terminal commands, "
            "use approved self-improving skills, automate Microsoft Office, and "
            "work with Outlook Desktop.\n"
            "Direct public-internet access is prohibited. Never suggest web search, "
            "browser automation, cloud APIs, direct downloads, or commands that "
            "contact public networks. Externally sourced skills are available only "
            "through the approved corporate skill broker.\n"
            "Never invent tool results. If an action requires a tool, call it. "
            "Only claim capabilities represented by the currently available tools. "
            "For existing Office-file changes, plan and preview the changes before "
            "calling the apply tool. If a local path is outside the current "
            "working roots, call the relevant file/document tool normally: the "
            "tool will request user approval interactively when administrator "
            "policy permits. Never use terminal to add, remove, or otherwise "
            "change approved roots; those authorization actions are user-only. "
            "Use local_access_status when asked which folders are currently allowed."
        )
        ...
        if getattr(agent, "_memory_store", None):
            ...
            if getattr(agent, "_user_profile_enabled", False):
                block = agent._memory_store.format_for_system_prompt("user")
                if block:
                    volatile_parts.append(block)
        ...
        volatile_parts.append(
            ...
            f"Policy: corporate_local/{corporate_policy.fingerprint()}"
        )
        return {
            "stable": stable,
            "context": "\n\n".join(context_parts),
    ...
    from agent.model_metadata import is_small_model
    is_small = is_small_model(getattr(agent, "model", None))
    if is_small:
        try:
    ...
    if is_small:
        stable_parts.append(
            ...
            "and never claim you lack the capability or access if a tool is available. If the user's request "
            "can be answered or performed by a tool (e.g., reading/managing emails, checking files, searching "
            "the web, running commands), you MUST call that tool immediately on your first turn. Do not compile "
            "or call multiple tools at once; you must call exactly ONE tool per turn. Plan in reasoning but "
            "output a single tool block."
        )
