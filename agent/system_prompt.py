...
upstream prefix cache warm.  See ``sr-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``SR_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("SR_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  SR never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SR_HOME persona while keeping
    ...
    # Pointer to the sr-agent skill + docs for user questions about SR itself.
    stable_parts.append(SR_AGENT_HELP_GUIDANCE)
    ...
    # SR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base SR, any interactive coding surface in a code
    ...
    # Active-profile hint — names the SR profile the agent is running
    # under so it doesn't conflate ~/.sr/skills/ (default profile) with
    # ~/.sr/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active SR profile: default. Other profiles (if any) live "
            "under ~/.sr/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active SR profile: {active_profile}. This session reads "
            f"and writes ~/.sr/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.sr/skills/, ~/.sr/plugins/, "
            f"~/.sr/cron/, ~/.sr/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — SR never rebuilds or reinjects parts of it
