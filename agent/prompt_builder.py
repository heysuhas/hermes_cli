...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "Only claim or offer actions represented by the tools currently available to you. "
    "Do not infer unsupported capabilities from a related tool or from data returned "
    "by a tool. When a tool succeeds, describe its output as retrieved through that "
    "tool; do not then deny having the capability or claim the user supplied the data."
)
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "A dedicated native function tool is different from a basic fallback: when a directly "
            "relevant native tool is already available, call it to execute the capability instead "
            "of loading a CLI skill for the same job. Use the skill only for missing setup, deeper "
            "workflow guidance, or when the native tool reports that it cannot complete the task. "
