...
from sr_constants import get_sr_home, get_skills_dir, is_wsl
...
_SR_MD_NAMES = (".sr.md", "SR.md")


def _find_sr_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.sr.md`` or ``SR.md``.
    ...
    # could pick up a .sr.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _SR_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are SR Agent, an intelligent AI assistant created by Samsung Research. "
...
SR_AGENT_HELP_GUIDANCE = (
    "You run on SR Agent (by Samsung Research). When the user needs help with "
    "SR itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://sr-agent.samsung.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `sr-agent` skill with skill_view(name='sr-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.sr/kanban.db`. Your task id is in "
    "`$SR_KANBAN_TASK`; your workspace is `$SR_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SR_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $SR_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${SR_KANBAN_BRANCH:-wt/$SR_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$SR_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`sr profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sr kanban <verb>` for board operations. Use "
...
#
# The sr-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to sr-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `sr computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that SR "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the SR terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the SR desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the SR WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where SR itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host SR runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where SR itself is running. The host OS, home, and cwd "
                f"of the SR process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where SR "
    ...
    # Embedder-supplied environment description. Lets a host that wraps SR
    ...
    extra = (os.getenv("SR_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sr_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("SR_PLATFORM") or os.environ.get("SR_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sr/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot SR Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `sr-agent` skill "
            "first. It has the actual commands (e.g. `sr config set …`, `sr tools`, "
            "`sr setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Disabled for corporate environment."""
    return ""
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from SR_HOME and return its content, or None.
    ...
    """
    try:
        from sr_cli.config import ensure_sr_home
        ensure_sr_home()
    except Exception as e:
        logger.debug("Could not ensure SR_HOME before loading SOUL.md: %s", e)

    soul_path = get_sr_home() / "SOUL.md"
...
def _load_sr_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.sr.md / SR.md — walk to git root."""
    sr_md_path = _find_sr_md(cwd_path)
    if not sr_md_path:
        return ""
    try:
        content = sr_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sr_md_path.name
        try:
            rel = str(sr_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".sr.md", context_length=context_length,
            read_path=str(sr_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", sr_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .sr.md / SR.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SR_HOME is independent and always included when present.
    ...
    project_context = (
        _load_sr_md(cwd_path, context_length)
    ...
    # SOUL.md from SR_HOME only — skip when already loaded as identity
    if not skip_soul:
