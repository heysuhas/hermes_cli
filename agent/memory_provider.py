...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `sr backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - sr_home (str): The active SR_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.sr``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "sr").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'sr memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], sr_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'sr memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``sr_home`` is the active SR_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE SR_HOME.

        ``sr backup`` only walks SR_HOME, so any provider state kept
        ...
        of the archive, and ``sr import`` restores them to their original
