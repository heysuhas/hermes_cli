...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into SR with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.sr/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of SR
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sr tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``sr setup`` for
