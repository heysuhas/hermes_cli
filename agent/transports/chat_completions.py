...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        # When ollama_num_ctx is set, the ENTIRE context (input + output) must
        # fit within that window. Resolve the output cap from the actual request
        # footprint instead of reserving an arbitrary half-window: Hermes'
        # system prompt and tool schemas can consume substantially more than
        # half on small contexts, while compact/local requests should be able
        # to use nearly all remaining capacity.
        _num_ctx = params.get("ollama_num_ctx")
        if _num_ctx:
            _request_input = params.get("request_input_tokens")
            try:
                _request_input = max(0, int(_request_input))
            except (TypeError, ValueError):
                _request_input = None
            if _request_input is None:
                _safe_out = max(512, _num_ctx // 2)
            else:
                # Keep a small accounting margin for tokenizer-estimation
                # error and chat-template framing. The preflight guard emits a
                # clear error when the request itself leaves no useful room.
                _reserve = max(256, min(2048, _num_ctx // 32))
                _safe_out = max(1, _num_ctx - _request_input - _reserve)
            if ephemeral is not None:
                ephemeral = min(ephemeral, _safe_out)
            if user_max is not None:
                user_max = min(user_max, _safe_out)
            if profile_max is not None:
                profile_max = min(profile_max, _safe_out)
