...
Owns one Codex thread per SR session. Drives `turn/start`, consumes
...
# SR' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_SR_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse SR/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per SR session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _SR_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("SR_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="sr",
            client_title="SR Agent",
            client_version=_get_sr_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but SR' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into SR' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own sr-tools callback we
            # auto-accept — the user already approved SR' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map SR approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    SR returns 'once', 'session', 'always', or 'deny'.
...
def _get_sr_version() -> str:
    """Best-effort SR version string for codex's userAgent line."""
    try:
