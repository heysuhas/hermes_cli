...
projection into SR' display, approval bridging, transcript projection into
...
"codex_app_server"`. SR' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 SR secret — gateway
        ...
        spawn_env = sr_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("SR_KANBAN_TASK"):
            kanban_db = spawn_env.get("SR_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "SR_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("SR_HOME", os.path.expanduser("~/.sr")),
                        "kanban",
                    ),
                )
            )
