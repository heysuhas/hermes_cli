...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the SR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # SR runtime / deployment
    "SR_HOME", "SR_PROFILE", "SR_GATEWAY_LOCK_DIR",
    "SR_MAX_ITERATIONS", "SR_MAX_TOKENS", "SR_API_TIMEOUT",
    "SR_REDACT_SECRETS", "SR_SR_TIMEOUT_SECONDS",
    "_SR_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "SR_KANBAN_DB", "SR_KANBAN_WORKSPACES_ROOT", "SR_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "SR_KANBAN_",
    "SR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(sr_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(sr_home) / ".env")
