...
def _write_through_provider_state_to_global_root(
    ...
    ``sr_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``sr model`` / ``sr auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another SR process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``sr`` invocation that defaults to the active provider
        (e.g. setup wizard, ``sr auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # sr_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across SR processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "SR_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("sr_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another SR
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("sr_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another SR process
            ...
            if self.provider == "nous":
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    ...
                    singleton_sources = {
                        auth_mod.SR_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.SR_DEVICE_CODE_SOURCE}",
                    }
                    removed_ids = [
                        item.id for item in self._entries
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other SR profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `sr model` / `sr auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `sr model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``sr auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `sr auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `sr auth remove <provider> <N>` is stable across all source types.
    try:
        from sr_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, SR PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `sr setup` ("Claude
        ...
        # Claude Code CLI, sr_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.sr/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `sr setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"sr_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_sr_oauth_credentials

        for source_name, creds in (
            ("sr_pkce", read_sr_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `sr auth add nous --label <name>`).  Fall back to the
        ...
        # SR auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from sr_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.sr/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `sr auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `sr auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the SR auth store.  Without this gate the removal is instantly
        ...
        # SR owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``sr model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``sr auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from sr_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.sr/.env over os.environ — the user's config file is the
    # authoritative source for SR credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_sr_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `sr auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.sr/.env.
    ...
    try:
        from sr_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from sr_cli.env_loader import get_secret_source
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `sr auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and SR
