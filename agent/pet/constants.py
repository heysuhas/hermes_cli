...
row taxonomy is inferred from the atlas shape so SR can render both legacy
...
# rows (1536x1872), while older SR/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are SR' activity state names. They are not always identical to the
...
# Legacy SR/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical SR activity names -> accepted row-name aliases in descending
