...
Storage
-------
Bundles live in ``~/.sr/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``sr bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SR_HOME.

    Honors ``SR_BUNDLES_DIR`` for tests; falls back to
    ``<SR_HOME>/skill-bundles``.
    """
    override = os.environ.get("SR_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_sr_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `sr bundles` CLI subcommand.
# ---------------------------------------------------------------------------
