...
``auth.json``.  They intentionally have no dependency on ``sr_cli.auth`` so
...
# Sources SR owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and SR-owned OAuth/device-code state)
