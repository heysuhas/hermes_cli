"""Unified removal contract for every credential source SR reads from.

SR seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.sr/.env
    claude_code   — ~/.claude/.credentials.json
    sr_pkce   — ~/.sr/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``sr auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), sr_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.sr/.env  → clear it
    ...
    """
    from sr_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to SR.",
            f"  The pool entry is now suppressed — SR will ignore "
            f"{env_var} until you run `sr auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so SR stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `sr auth add anthropic` to re-enable if needed.",
    ])


def _remove_sr_pkce(provider: str, removed) -> RemovalResult:
    """~/.sr/.anthropic_oauth.json is ours — delete it outright."""
    from sr_constants import get_sr_home

    result = RemovalResult()
    oauth_file = get_sr_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared SR Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `sr auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `sr auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``sr auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `sr model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the SR auth store is not enough — _seed_from_singletons() would
    ...
    via ``sr auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `sr auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sr auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sr auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="sr_pkce",
        remove_fn=_remove_sr_pkce,
        description="~/.sr/.anthropic_oauth.json",
    ))
    register(RemovalStep(
