...
from sr_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``SR_KANBAN_TASK`` / ``SR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("SR_KANBAN_TASK") or os.getenv("SR_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The SR Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``sr_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SR_PLATFORM`` or
            ``SR_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("SR_PLATFORM")
        or get_session_env("SR_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``sr`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sr/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``sr`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SR_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sr/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.sr/skills/`` (including via symlinks) or configured
    ...
    # ~/.sr/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: SR can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sr = metadata.get("sr") or {}
    if not isinstance(sr, dict):
        sr = {}
    return {
        "fallback_for_toolsets": sr.get("fallback_for_toolsets", []),
        "requires_toolsets": sr.get("requires_toolsets", []),
        "fallback_for_tools": sr.get("fallback_for_tools", []),
        "requires_tools": sr.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes SR metadata, VCS, virtualenv/dependency, cache, and skill
