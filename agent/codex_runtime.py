...
def _codex_note_to_tool_progress(note: dict) -> tuple[str, str, dict] | None:
    """Map a Codex app-server ``item/started`` notification to a SR
    ...
    surfaced them as SR tool-progress events — so gateways (Telegram, etc.)
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in SR state.

    The app-server owns the compacted thread context, so SR should not
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into SR'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to SR' standard prompt flow if a
        ...
        # explicitly opted out of SR approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / SR_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing SR UI. Defaults (manual/smart/unset) preserve the
        ...
        def _on_codex_event(note: dict) -> None:
            # Bridge Codex app-server item/started notifications to SR
