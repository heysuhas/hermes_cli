...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time SR starts and finds ``~/.openclaw/``.

    Points users at ``sr claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sr claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to SR, run "
        "`sr claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sr claw cleanup` (renames it to ~/.openclaw.pre-migration — "
