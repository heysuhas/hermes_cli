...
as ``kind: backend``) or ``~/.sr/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``sr tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SR_HOME/cache/videos/``, creating parents as needed."""
    from sr_constants import get_sr_home

    path = get_sr_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SR_HOME/cache/videos/``.
