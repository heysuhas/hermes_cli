...
Every SR request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to SR Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=sr-agent",
        "client=sr-client-v<__version__>",
    ]

The version is sourced live from ``sr_cli.__version__`` so it auto-aligns
...
``sr_cli.__version__`` is the canonical source of truth.
"""
...
def _sr_version() -> str:
    """Return the current SR release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``sr_cli`` cannot be imported (should
...
def sr_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=sr-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=sr-client-v{_sr_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=sr-agent", sr_client_tag()]
