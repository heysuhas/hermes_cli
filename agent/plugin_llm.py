...
Plugins built on SR Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing SR
...
:class:`~sr_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors SR' main config
...
handles every provider, fallback chain, and per-task override SR
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sr_cli.plugins.PluginContext`
