...
cache problem must never block SR startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the SR home used for cache paths.

    ``home_path`` is whatever ``load_sr_dotenv()`` already resolved;
    falling back to ``$SR_HOME`` / ``~/.sr`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("SR_HOME", Path.home() / ".sr"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<sr_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if SR_HOME is ever made traversable.
            try:
