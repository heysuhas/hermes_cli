# OpenAI-Compatible API Server for SR Agent
...
SSE streaming. By exposing this endpoint, sr-agent becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.sr/.env`
2. Run `sr gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with sr-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns sr-agent as a model) |
...
Authorization: Bearer sr-api-key-here
...
But sr-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "sr-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured sr-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of sr-agent's core prompt
...
Once implemented, sr-agent works as a drop-in backend for:
