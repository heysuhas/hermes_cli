...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.sr/sr-agent/utils.py)
...
2. ``sr update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``sr_cli/main.py`` (the ``sr update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``sr_constants`` after the
