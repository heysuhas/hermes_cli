...
def _isolate(tmp_path, monkeypatch):
    """Isolate SR_HOME so tests don't touch real config."""
    home = tmp_path / ".sr"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal SRCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = SRCLI.__new__(SRCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["sr-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below SR' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly SR' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above SR' minimum context length."""
