...
class TestCliApprovalUi:
    ...
    def test_approval_prompt_restores_existing_draft_after_response(self):
        cli = _make_cli_stub()
        cli._app.current_buffer = _FakeBuffer("draft message", cursor_position=6)
        result = {}

        def _run_callback():
            result["value"] = cli._approval_callback(
                r"Local folder access: C:\Users\test\Downloads",
                "Allow Hermes to read this document?",
            )
        ...
        assert cli._app.current_buffer.text == "draft message"
        assert cli._app.current_buffer.cursor_position == 6

    def test_local_path_approval_has_specific_title(self):
        cli = _make_cli_stub()
        cli._approval_state = {
            "command": r"Local folder access: C:\Users\test\Downloads",
            "description": "Allow Hermes to read this document?",
            "choices": ["once", "session", "always", "deny"],
            "selected": 0,
            "response_queue": queue.Queue(),
        }

        rendered = "".join(text for _, text in cli._get_approval_display_fragments())

        assert "Local Folder Access" in rendered
