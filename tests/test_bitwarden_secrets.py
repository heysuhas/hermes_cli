...
binary download are exercised manually by `sr secrets bitwarden
...
def sr_home(tmp_path, monkeypatch):
    """Point SR at an isolated home directory."""
    home = tmp_path / ".sr"
    home.mkdir()
    monkeypatch.setenv("SR_HOME", str(home))
    # Some modules cache get_sr_home; clear if needed.
    import sr_constants
    if hasattr(sr_constants, "_SR_HOME_CACHE"):
        sr_constants._SR_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(sr_home, monkeypatch):
...
def test_install_bws_happy_path(sr_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(sr_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(sr_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from sr_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under sr_home/cache/."""
    home = tmp_path / ".sr"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
