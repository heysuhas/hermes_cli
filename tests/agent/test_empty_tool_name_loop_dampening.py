...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated SR_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="sr_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".sr"))
    prev_home = os.environ.get("SR_HOME")
    os.environ["SR_HOME"] = os.path.join(test_home, ".sr")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("sr_"):
            del sys.modules[mod]
