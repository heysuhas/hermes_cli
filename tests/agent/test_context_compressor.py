...
class TestCompress:
    ...
    def test_threshold_below_window_at_minimum_ctx(self):
        ...
        assert t == 13600  # 85% of 16000

    def test_threshold_below_window_for_small_ctx(self):
        # 8K model: the 16000 floor exceeds the window — trigger at 85%.
        t = ContextCompressor._compute_threshold_tokens(8000, 0.50)
        assert t == 6800  # 85% of 8000
        assert t < 8000
    ...
    def test_threshold_floored_for_large_ctx(self):
        ...
        # 30K model at 50% = 15000 (below floor) — floored to MINIMUM.
        assert ContextCompressor._compute_threshold_tokens(30000, 0.50) == MINIMUM_CONTEXT_LENGTH
    ...
    def test_minimum_ctx_model_can_actually_compress(self):
        ...
        at 100%."""
        with patch("agent.context_compressor.get_model_context_length", return_value=16000):
            c = ContextCompressor(model="small-16k", quiet_mode=True)
            c.context_length = 16000
            c.threshold_tokens = c._compute_threshold_tokens(16000, c.threshold_percent)
        assert c.threshold_tokens == 13600
        assert c.threshold_tokens < 16000
        # At 85%+ usage compaction fires; below it, it doesn't (no premature compact).
        assert c.should_compress(14000) is True
        assert c.should_compress(10000) is False
    ...
    def test_max_tokens_reservation_with_small_window_floors(self):
        ...
        # 32K window, 20000 reserved → effective 12000 (< MINIMUM 16000).
        # Floor (16000) >= effective window (12000) → 85% of effective.
        t = ContextCompressor._compute_threshold_tokens(32000, 0.50, 20000)
        assert t == int(12000 * 0.85)  # 10200
        assert t < 12000
    ...
    def test_max_tokens_exceeding_window_falls_back_to_full(self):
        ...
        t = ContextCompressor._compute_threshold_tokens(16000, 0.50, 20000)
        # effective_window <= 0 → fall back to full context (16000) → 85% guard.
        assert t == 13600  # 85% of 16000, same as no-reservation small-ctx case
        assert t > 0
...
class TestSummaryTargetRatio:
    ...
    def test_default_threshold_is_50_percent(self):
        """Default compression threshold should be 50%, with a 16K floor."""
        with patch("agent.context_compressor.get_model_context_length", return_value=30_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.50
        # 50% of 30K = 15K, but the floor is 16K
        assert c.threshold_tokens == 16_000

    def test_threshold_floor_does_not_apply_above_128k(self):
        """On large-context models the 50% percentage is used directly."""
        with patch("agent.context_compressor.get_model_context_length", return_value=200_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        # 50% of 200K = 100K, which is above the 16K floor
