...
def isolated_home(monkeypatch):
    """Temp SR_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="sr_test_31179_")
    sr_home = os.path.join(test_home, ".sr")
    os.makedirs(sr_home)
...
def _fresh_modules():
    """Drop cached sr modules so each test reloads against current env."""
