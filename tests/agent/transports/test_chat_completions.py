...
class TestChatCompletionsBuildKwargs:
    ...
    def test_ollama_num_ctx_caps_max_tokens(self, transport):
        """ollama_num_ctx must cap max_tokens so we never request more output
        tokens than the entire context window (regression: finish_reason=length
        on every turn with num_ctx=16384 and default_max_tokens=65536)."""
        from providers import get_provider_profile

        profile = get_provider_profile("custom")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
            model="qwen3.5:9b", messages=msgs,
            provider_profile=profile,
            max_tokens_param_fn=lambda n: {"max_tokens": n},
            ollama_num_ctx=16384,
            request_input_tokens=12000,
        )
        # Reserve 512 tokens for estimation/template overhead, leaving 3872.
        assert kw["max_tokens"] == 3872

    def test_ollama_user_max_tokens_also_capped(self, transport):
        """A user-set max_tokens larger than num_ctx//2 should also be capped."""
        from providers import get_provider_profile
    ...
    def test_ollama_user_max_tokens_within_budget_kept(self, transport):
        """A user-set max_tokens smaller than the safe budget is kept as-is."""
        from providers import get_provider_profile
    ...
    def test_ollama_compact_request_can_use_remaining_context(self, transport):
        from providers import get_provider_profile
        ...
        assert kw["max_tokens"] == 14872

    def test_ollama_continuation_override_is_also_context_clamped(self, transport):
        from providers import get_provider_profile
        ...
        kw = transport.build_kwargs(
            ...
            ollama_num_ctx=16384,
            request_input_tokens=14000,
        )
        assert kw["max_tokens"] == 1872
