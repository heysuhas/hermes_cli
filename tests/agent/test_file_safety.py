...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_sr_env(self):
        """SR' own .env inside SR_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: sr internal .env is in ~/.sr/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.sr/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.sr/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal SR cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        sr_home = tmp_path / ".sr"
        cache = sr_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._sr_home_path", return_value=sr_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal SR cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        sr_home = tmp_path / ".sr"
        hub = sr_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_sr_home(self, tmp_path):
        """The env basename guard does not depend on SR_HOME resolution."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        sr_home = tmp_path / ".sr"
        cache = sr_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
