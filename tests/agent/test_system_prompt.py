...
def test_small_model_steering():
    from agent.model_metadata import is_small_model
    assert is_small_model("qwen3.5:7b") is True
    assert is_small_model("gpt-4o") is False
    assert is_small_model("llama-3-8b") is True
    assert is_small_model("gemini-1.5-flash") is False
    ...
    assert "You MUST use your tools to take action" in stable
    assert "you must call exactly ONE tool per turn" in stable
