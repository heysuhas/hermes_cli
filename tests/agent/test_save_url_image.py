...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SR_HOME under tmp_path."""
    monkeypatch.setenv("SR_HOME", str(tmp_path / ".sr"))
    (tmp_path / ".sr").mkdir()

    # Force the constants/image cache helpers to re-read SR_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("sr_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_sr_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under SR_HOME — gateway cleanup
