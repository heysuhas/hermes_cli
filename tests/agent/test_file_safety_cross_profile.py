"""Tests for the cross-SR-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another SR profile's
...
Reference: May 2026 incident — a sr-security profile session
accidentally edited skills under both ~/.sr/profiles/sr-security/skills/
AND ~/.sr/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake SR root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_sr(tmp_path, monkeypatch):
    """Build a fake SR layout:
    ...
    sec_home = root / "profiles" / "sr-security"
    ...
    import sr_constants
    monkeypatch.setattr(sr_constants, "get_default_sr_root", lambda: root)

    # The reloads below ensure get_cross_profile_warning/classify see the patched root.
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_sr_root_path", lambda: root)
...
def _set_active_home(monkeypatch, sr_home: Path):
    """Point file_safety._sr_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "sr-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_sr, monkeypatch):
        """If SR_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_sr_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sr["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_sr, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sr["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "sr-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_sr, monkeypatch):
        ...
        assert result["target_profile"] == "sr-security"

    def test_named_to_named_cross_profile(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_sr["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_sr, monkeypatch, area):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_sr["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_sr_path_returns_none(self, fake_sr, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any SR root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_sr_config_not_classified_as_cross_profile(self, fake_sr, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_sr["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_sr, monkeypatch):
        _set_active_home(monkeypatch, fake_sr["security_home"])
        from agent.file_safety import get_cross_profile_warning
