...
path via ``sr chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp SR_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point SR_HOME at a temp dir and turn moa.save_traces on."""
    sr_home = tmp_path / ".sr"
    sr_home.mkdir()
    monkeypatch.setenv("SR_HOME", str(sr_home))

    # save_moa_turn reads config via sr_cli.config.load_config; stub it to
