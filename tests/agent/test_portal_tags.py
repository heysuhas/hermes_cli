...
def test_sr_client_tag_includes_current_version():
    """The client tag must reflect sr_cli.__version__ verbatim."""
    from sr_cli import __version__
    from agent.portal_tags import sr_client_tag

    assert sr_client_tag() == f"client=sr-client-v{__version__}"


def test_sr_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import sr_client_tag

    tag = sr_client_tag()
    assert tag.startswith("client=sr-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import sr_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=sr-agent" in tags
    assert sr_client_tag() in tags
    assert len(tags) == 2
