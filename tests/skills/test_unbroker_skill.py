...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and sr-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # sr-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_sr_home():
    prev = os.environ.get("SR_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["SR_HOME"] = d
        try:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    prev_home = os.environ.get("SR_HOME")
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("SR_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:
