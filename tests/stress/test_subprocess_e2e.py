...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves sr_cli.kanban_db on its own
  ...
  - worker logs are captured to SR_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "SR_KANBAN_TASK": task.id,
            "SR_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="sr_e2e_")
    os.environ["SR_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from sr_cli import kanban_db as kb

    # Point the `sr` CLI child processes will run at the worktree
    # sr_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "sr")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m sr_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/sr_e2e_*/worker_*.log
    import glob
