...
def server():
    with patch.dict(
        "sys.modules",
        {
            "sr_constants": MagicMock(
                get_sr_home=MagicMock(return_value="/tmp/sr_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'sr-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'sr-release' patched"
    }
