"""Shared fixtures for the sr-agent test suite.
...
2. **Isolated SR_HOME.** SR_HOME points to a per-test tempdir so
   code reading ``~/.sr/*`` via ``get_sr_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".sr"`` instead of the canonical
   ``get_sr_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SR_SESSION_* inheritance** — the agent's current gateway
...
# SR_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SR_BEHAVIORAL_VARS = frozenset({
    "SR_YOLO_MODE",
    "SR_INTERACTIVE",
    "SR_QUIET",
    "SR_TOOL_PROGRESS",
    "SR_TOOL_PROGRESS_MODE",
    "SR_MAX_ITERATIONS",
    "SR_SESSION_PLATFORM",
    "SR_SESSION_CHAT_ID",
    "SR_SESSION_CHAT_NAME",
    "SR_SESSION_THREAD_ID",
    "SR_SESSION_SOURCE",
    "SR_SESSION_KEY",
    "SR_GATEWAY_SESSION",
    "SR_CRON_SESSION",
    ...
    "SR_TIMEZONE",
    "SR_REDACT_SECRETS",
    "SR_BACKGROUND_NOTIFICATIONS",
    "SR_EXEC_ASK",
    "SR_HOME_MODE",
    "SR_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.sr/kanban.db instead of the per-test SR_HOME.
    "SR_KANBAN_DB",
    "SR_KANBAN_BOARD",
    "SR_KANBAN_HOME",
    "SR_KANBAN_WORKSPACES_ROOT",
    "SR_KANBAN_LOGS_ROOT",
    "SR_KANBAN_TASK",
    "SR_KANBAN_WORKSPACE",
    "SR_KANBAN_RUN_ID",
    "SR_KANBAN_CLAIM_LOCK",
    "SR_KANBAN_DISPATCH_IN_GATEWAY",
    "SR_TENANT",
    ...
    "SR_DASHBOARD_OAUTH_CLIENT_ID",
    "SR_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SR_HOME to per-test tempdirs so code that
    reads ``~/.sr/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SR_* vars that could change test semantics.
    for name in _SR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect SR_HOME to a per-test tempdir. Code that reads
    #    ``~/.sr/*`` via ``get_sr_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.sr/*`` via
    #    ``Path.home() / ".sr"`` instead of ``get_sr_home()``
    #    is a bug to fix at the callsite.
    fake_sr_home = tmp_path / "sr_test"
    fake_sr_home.mkdir()
    (fake_sr_home / "sessions").mkdir()
    (fake_sr_home / "cron").mkdir()
    (fake_sr_home / "memories").mkdir()
    (fake_sr_home / "skills").mkdir()
    monkeypatch.setenv("SR_HOME", str(fake_sr_home))
    ...
    #    ~/.sr/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import sr_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_sr_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal sr config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``sr-gateway`` process
...
# correlated with a ``tests/sr_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> sr-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    def _matches_sr_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SR_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_sr_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch sr-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m sr_cli.main".
                if (
                    "sr" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live sr-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting sr/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `sr update` (or the
        # equivalent `python -m sr_cli.main update`).  These commands
        ...
        if "update" in low and (
            # sr update / sr update --gateway / setsid bash -c ... sr update
            ("sr" in low and "update" in low.split())
            or
            # python -m sr_cli.main update --gateway
            ("sr_cli" in low and "update" in low.split())
            or
            # venv/bin/sr update  (absolute path variant used in tests)
            (".venv/bin/sr" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`sr update` against the real checkout, fetching "
