...
The ``sr update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m sr_cli.main gateway run`` — image name
    ``pythonw``, not ``sr.exe`` — so the ``taskkill /IM sr.exe`` guard
    ...
    # The sr.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM sr.exe' in text
