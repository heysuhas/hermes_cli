...
def test_root_session_no_compression(db):
    ...
    assert prov["currentSRSessionId"] == "root1"
    assert prov["rootSRSessionId"] == "root1"
    assert prov["parentSRSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_sr_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentSRSessionId"] == "old"
    assert prov["rootSRSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousSRSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootSRSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootSRSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_sr_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousSRSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"sr"}
    assert "sessionProvenance" in meta["sr"]
    assert meta["sr"]["sessionProvenance"]["currentSRSessionId"] == "root1"
