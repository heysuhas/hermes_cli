"""Tests for ACP Registry metadata shipped with SR."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "sr-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "SR Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/SamsungResearch/sr-agent"
    assert data["website"].startswith("https://sr-agent.samsung.com/")
    assert data["authors"] == ["Samsung Research"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"sr-agent[acp]=={data['version']}"
    assert uvx["args"] == ["sr-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"sr-agent[acp]=={_pyproject_version()}"
    )
