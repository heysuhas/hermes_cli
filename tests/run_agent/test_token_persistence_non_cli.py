...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    sr_state = ModuleType("sr_state")
    sr_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "sr_state", sr_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "SR"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "SR"
