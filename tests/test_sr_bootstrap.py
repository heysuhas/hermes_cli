"""Tests for sr_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every SR entry point
(sr, sr-agent, sr-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every SR entry point imports sr_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the SR CLI
...
class TestEntryPointsImportBootstrap:
    """Every SR entry point must import sr_bootstrap as its
    ...
    # Entry points that invoke SR as a process.  Each one must
    # import sr_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sr_cli/main.py",   # sr CLI (console_script)
        "run_agent.py",          # sr-agent (console_script)
        "acp_adapter/entry.py",  # sr-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sr_bootstrap' and that
        ...
        half-finished ``sr update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``sr_bootstrap`` as a top-level module) leaves sr
        ...
        """
        # Resolve relative to the sr-agent repo root.  Tests live
        # at tests/test_sr_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # sr start even when sr_bootstrap hasn't been
        ...
        assert first_import_name == "sr_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sr_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sr_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing SR's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("SR_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["SR_PYTHON_SRC_ROOT"] = env
            elif "SR_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["SR_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/sr")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SR_PYTHON_SRC_ROOT", None)
            else:
                os.environ["SR_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/sr", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # SR root.  The guard must relocate SR to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/sr"])
        assert result[0] == "/opt/sr"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the SR root.
        assert result.index("/opt/sr") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/sr", "/opt/sr", ""])
        assert result.count("/opt/sr") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["SR_PYTHON_SRC_ROOT"] = "/env/sr"
            hb.harden_import_path()
            assert sys.path[0] == "/env/sr"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("SR_PYTHON_SRC_ROOT", None)
