...
Uses a throwaway SR_HOME so it never touches ~/.sr. No real names.
"""
...
def _fresh_home():
    """Point SR_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_sr_home() at import time)."""
...
# a different get_sr_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"SR_HOME (throwaway): {HOME}")
