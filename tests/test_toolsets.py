...
class TestToolsetConsistency:
    ...
    def test_sr_platforms_share_core_tools(self):
        """All sr-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on sr-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["sr-cli", "sr-telegram", "sr-discord", "sr-whatsapp", "sr-slack", "sr-signal", "sr-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_sr_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("sr-whatsapp")

    def test_sr_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("sr-api-server")
