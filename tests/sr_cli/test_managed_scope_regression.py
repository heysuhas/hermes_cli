...
def sr_home(tmp_path, monkeypatch):
    ...
    # /etc/sr on the dev/CI box can't influence the test.
    monkeypatch.setenv("SR_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(sr_home, monkeypatch):
    from sr_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(sr_home, monkeypatch):
    from sr_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(sr_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_sr_dotenv(sr_home=str(home))
