...
``SR_PORTAL_BASE_URL=https://portal.staging-samsung.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SR_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert _nous_portal_env_override() is None

    def test_sr_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "SR_PORTAL_BASE_URL", "https://portal.staging-samsung.com/"
        )
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert (
            _nous_portal_env_override() == "https://portal.staging-samsung.com"
        )

    def test_nous_portal_base_url_used_as_fallback(self, monkeypatch):
        monkeypatch.delenv("SR_PORTAL_BASE_URL", raising=False)
        monkeypatch.setenv(
            "NOUS_PORTAL_BASE_URL", "https://portal.staging-samsung.com"
        )
        assert (
            _nous_portal_env_override() == "https://portal.staging-samsung.com"
        )
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        assert "portal.staging-samsung.com" not in _NOUS_PORTAL_ALLOWED_HOSTS
        assert (
            _nous_portal_env_override() == "https://portal.staging-samsung.com"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("sr_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior SR_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-samsung.com"
        monkeypatch.setenv("SR_HOME", str(tmp_path))
        monkeypatch.setenv("SR_PORTAL_BASE_URL", staging_portal)
