"""Tests for the top-level `./sr` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./sr` should use `sr_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "sr"
    called = []

    fake_main_module = types.ModuleType("sr_cli.main")

    def fake_main():
        called.append("sr_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sr_cli.main", fake_main_module)
