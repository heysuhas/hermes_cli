...
``sr_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``sr --help``, ``sr version``,
``sr logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``sr --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from sr_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["sr", "--tui", "chat"], "chat"),
        (["sr", "-w", "logs"], "logs"),
        (["sr", "chat", "hello world"], "chat"),
        (["sr", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["sr", "-m", "gpt5", "chat"], "chat"),
        (["sr", "--model", "gpt5", "chat", "hi"], "chat"),
        (["sr", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["sr", "-z", "hello world"], None),
        (["sr", "-z", "hello", "chat"], "chat"),
        (["sr", "--model=gpt5", "chat"], "chat"),     # inline form
        (["sr", "--", "chat"], "chat"),               # -- terminator
        (["sr", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["sr", "some-plugin-cmd"], "some-plugin-cmd"),
        (["sr", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["sr"],                          # bare → chat
        ["sr", "--help"],                # top-level help
        ["sr", "-h"],
        ["sr", "version"],               # known built-in
        ["sr", "logs"],
        ["sr", "gateway", "run"],
        ["sr", "--tui"],
        ["sr", "-w", "--tui"],
        ["sr", "chat", "hi"],
        ["sr", "help"],                  # accepted built-in-ish
        ["sr", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["sr", "meet", "join"],          # potential google_meet plugin
        ["sr", "honcho", "status"],      # potential memory plugin
        ["sr", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``sr help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"sr_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
