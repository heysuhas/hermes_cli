...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (SR_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via SR_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in SR_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from sr_cli.providers import SR_OVERLAYS
        assert "bedrock" in SR_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from sr_cli.providers import SR_OVERLAYS
        assert SR_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from sr_cli.providers import SR_OVERLAYS
        assert SR_OVERLAYS["bedrock"].auth_type == "aws_sdk"
