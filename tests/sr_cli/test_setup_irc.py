"""Tests for IRC gateway configuration via `sr setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh SR install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new SR install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#sr")
            monkeypatch.setenv("IRC_NICKNAME", "sr-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `sr setup gateway` experience with IRC present."""
