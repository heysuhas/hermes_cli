...
@pytest.fixture()
def sr_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in SR auth store (no Codex CLI needed)."""
    sr_home = tmp_path / ".sr"
    sr_home.mkdir()
...
def test_normal_path_still_works(sr_auth_only_env):
    """openai-codex appears when tokens are already in SR auth store."""
...
def test_codex_picker_uses_live_codex_catalog(sr_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or SR
    auth store."""
    sr_home = tmp_path / ".sr"
    sr_home.mkdir()
