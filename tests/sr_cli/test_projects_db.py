"""Tests for the per-profile Projects store (sr_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="SR Agent", folders=["/tmp/sr"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "sr-agent"
    assert proj.name == "SR Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/sr"
    assert [f.path for f in proj.folders] == ["/tmp/sr"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "sr-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="SR Agent")
    pdb.create_project(conn, name="SR Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["sr-agent", "sr-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' SR_HOME.
...
def test_db_path_under_sr_home():
    # Resolves under SR_HOME (set by the autouse isolation fixture).
