...
When ``SR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.sr/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.sr            (has its own auth.json fixture)
    * Profile     -> tmp_path/.sr/profiles/coder   (active, SR_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"SR is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from sr_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    sr_home = tmp_path / "classic"
    sr_home.mkdir()
    monkeypatch.setenv("SR_HOME", str(sr_home))

    _write(sr_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SR_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.sr as the "real home" — which is NOT equal
    # to the SR_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    sr_home = tmp_path / "classic"
    ...
    _write(sr_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from sr_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SR_HOME is set to a custom path that is NOT under
    # ~/.sr/profiles/ — get_default_sr_root() returns SR_HOME
