...
the SAME provider universe as ``sr model`` (the CLI/TUI picker).
...
def test_every_sr_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `sr model` universe."""
    ...
    assert not missing, (
        "providers shown in `sr model` but not configurable in the desktop "
