...
sr_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `sr config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_sr_home = get_sr_home()`` at import time (module
    singleton), so monkeypatching SR_HOME after import doesn't move it.
    ...
    this only matters for tests that swap SR_HOME.)
    """
    import cli
