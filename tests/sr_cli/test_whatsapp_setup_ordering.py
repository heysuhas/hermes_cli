...
Before the fix, ``sr whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``sr gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SR_HOME", str(sr))
...
def _env_value(sr_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from sr_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("sr_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``sr whatsapp`` with an existing paired session and
