"""Tests for the SR plugin system (sr_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<sr_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<sr_home>/plugins/``; we derive
    ``<sr_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SR_HOME>/config.yaml.
        # Config is always read from SR_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        sr_home_str = os.environ.get("SR_HOME")
        if sr_home_str:
            sr_home = Path(sr_home_str)
        else:
            sr_home = base.parent
        sr_home.mkdir(parents=True, exist_ok=True)
        cfg_path = sr_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.sr/plugins/ are discovered."""
        plugins_dir = tmp_path / "sr_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("SR_HOME", str(tmp_path / "sr_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("sr_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SR_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".sr" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under sr_plugins.<name>."""
        plugins_dir = tmp_path / "sr_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SR_HOME", str(tmp_path / "sr_test"))

        # Clean up any prior namespace module
        sys.modules.pop("sr_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "sr_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="sr_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from sr_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "sr_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_block_returns_message(self, monkeypatch):
        from sr_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "sr_cli.plugins.invoke_hook",
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from sr_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "sr_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("sr_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("sr_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from sr_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from sr_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("sr_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import sr_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `sr plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("sr_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "sr_test" / "plugins"
        # Set SR_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("sr_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("sr_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("sr_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("sr_cli.plugins.PluginContext.dispatch_tool.__module__", "sr_cli.plugins"):
...
class TestPluginDebugLogging:
    """SR_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SR_PLUGINS_DEBUG", raising=False)
        from sr_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With SR_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("SR_PLUGINS_DEBUG", "1")
        from sr_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from SR_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """SR_HOME at the root resolves to 'default'."""
        home = tmp_path / ".sr"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """SR_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".sr" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".sr" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(prof))
        ctx = self._ctx()
