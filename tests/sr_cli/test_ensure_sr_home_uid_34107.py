"""Regression tests for #34107 — Docker UID/GID handling in ensure_sr_home.

When SR runs in Docker with ``SR_UID=1000`` / ``SR_GID=911``,
the entrypoint chowns the top-level ``SR_HOME`` once at startup. But
...
The fix is a ``_chown_to_sr_uid`` helper that reads the env vars and
...
class TestResolveSRUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("SR_UID", "1000")
        monkeypatch.setenv("SR_GID", "911")
        from sr_cli.config import _resolve_sr_uid_gid
        uid, gid = _resolve_sr_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("SR_UID", raising=False)
        monkeypatch.delenv("SR_GID", raising=False)
        from sr_cli.config import _resolve_sr_uid_gid
        uid, gid = _resolve_sr_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("SR_UID", "not-a-number")
        monkeypatch.setenv("SR_GID", "911")
        from sr_cli.config import _resolve_sr_uid_gid
        uid, gid = _resolve_sr_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("SR_UID", "")
        monkeypatch.setenv("SR_GID", "")
        from sr_cli.config import _resolve_sr_uid_gid
        uid, gid = _resolve_sr_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToSRUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_sr_uid(d)  # must not raise
