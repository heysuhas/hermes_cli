...
class TestVerifyConsoleScriptsInstalled:
    def test_no_action_when_all_shims_present(self, temp_pyproject, fake_scripts_dir):
        for name in ("sr", "sr-agent", "sr-acp"):
            (fake_scripts_dir / f"{name}.exe").write_bytes(b"fake")

        with patch("sr_cli.main._is_windows", return_value=True), \
             patch("sr_cli.main._venv_scripts_dir", return_value=fake_scripts_dir), \
             patch("sr_cli.main._run_quarantined_install") as mock_install:
            from sr_cli.main import _verify_console_scripts_installed

            _verify_console_scripts_installed(["uv", "pip"], env={})

        mock_install.assert_not_called()

    def test_triggers_reinstall_when_sr_exe_missing(
        self, temp_pyproject, fake_scripts_dir
    ):
        (fake_scripts_dir / "sr-agent.exe").write_bytes(b"fake")
        (fake_scripts_dir / "sr-acp.exe").write_bytes(b"fake")
    ...
    def test_skips_off_windows(self, temp_pyproject, fake_scripts_dir):
        with patch("sr_cli.main._is_windows", return_value=False), \
             patch("sr_cli.main._run_quarantined_install") as mock_install:
            from sr_cli.main import _verify_console_scripts_installed
    ...
    def test_load_console_script_names_reads_pyproject(self, temp_pyproject):
        from sr_cli.main import _load_console_script_names

        names = _load_console_script_names()
        assert names == ["sr", "sr-agent", "sr-acp"]
    ...
    def test_primary_install_success_still_verifies_scripts(self):
        ...
        with patch("sr_cli.main._is_windows", return_value=False), \
             patch("sr_cli.main._run_quarantined_install") as mock_install, \
             patch("sr_cli.main._verify_console_scripts_installed") as mock_verify:
    ...
    def test_quarantine_shims_include_declared_console_scripts(
        ...
        with patch("sr_cli.main._is_windows", return_value=True):
            names = {path.name for path in main_mod._sr_exe_shims(fake_scripts_dir)}

        assert {"sr.exe", "sr-agent.exe", "sr-acp.exe"} <= names
        assert "sr-gateway.exe" in names
