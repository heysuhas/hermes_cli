...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SR_HOME with .container-mode file."""
    sr_home = tmp_path / ".sr"
    sr_home.mkdir()
    monkeypatch.setenv("SR_HOME", str(sr_home))
    monkeypatch.delenv("SR_DEV", raising=False)

    container_mode = sr_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=sr-agent\n"
        "exec_user=sr\n"
        "sr_bin=/data/current-package/bin/sr\n"
    )
    return sr_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("sr_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "sr-agent"
    assert info["exec_user"] == "sr"
    assert info["sr_bin"] == "/data/current-package/bin/sr"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("sr_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_sr_dev(container_env, monkeypatch):
    """Returns None when SR_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SR_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_sr_dev_zero(container_env, monkeypatch):
    """SR_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SR_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        sr_home = Path(tmpdir) / ".sr"
        sr_home.mkdir()
        (sr_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("sr_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_sr_home": lambda: sr_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SR_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=sr-custom\n"
        "exec_user=myuser\n"
        "sr_bin=/opt/sr/bin/sr\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "sr-custom"
    assert info["exec_user"] == "myuser"
    assert info["sr_bin"] == "/opt/sr/bin/sr"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from sr_cli.main import _exec_in_container
