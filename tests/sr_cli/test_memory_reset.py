"""Tests for the `sr memory reset` CLI command.
...
- Profile-scoped reset (uses SR_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SR_HOME with memory files."""
    sr_home = tmp_path / ".sr"
    memories = sr_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SR_HOME", str(sr_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nSR repo is at ~/.sr/sr-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `sr memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        sr_home = tmp_path / ".sr"
        (sr_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SR_HOME", str(sr_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SR_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_sr_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
