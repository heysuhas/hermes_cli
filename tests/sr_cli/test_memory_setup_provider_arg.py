"""Tests for `sr memory setup [provider]` routing.
...
``sr memory setup honcho``) without the interactive picker — which matters
because the per-provider ``sr <provider>`` subcommand is only registered
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("sr_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("sr_cli.tools_config.subprocess.run", fake_run):
