...
def test_managed_scope_signature_verification(tmp_path, monkeypatch):
    import hmac
    import hashlib
    from hermes_cli import managed_scope
    
    # Configure enterprise secret
    monkeypatch.setenv("HERMES_ENTERPRISE_SECRET", "test-secret-123")
    ...
    # Clear test bypass guard to run production verification
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    
    config_content = "model:\n  default: v1\n"
    (managed / "config.yaml").write_text(config_content, encoding="utf-8")
    
    # 1. Verification fails when signature is missing
    import pytest
    with pytest.raises(ValueError, match="config.yaml.sig is missing"):
        managed_scope.load_managed_config()
        
    # 2. Verification fails when signature is invalid
    (managed / "config.yaml.sig").write_text("invalid-sig", encoding="utf-8")
    with pytest.raises(ValueError, match="signature verification failed"):
        managed_scope.load_managed_config()
        
    # 3. Verification succeeds with valid signature
    with open(managed / "config.yaml", "rb") as f:
        config_bytes = f.read()
    expected_sig = hmac.new(b"test-secret-123", config_bytes, hashlib.sha256).hexdigest()
    (managed / "config.yaml.sig").write_text(expected_sig, encoding="utf-8")
    managed_scope.invalidate_managed_cache()
    
    config = managed_scope.load_managed_config()
    assert config["model"]["default"] == "v1"
