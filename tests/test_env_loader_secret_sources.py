"""Tests for the secret-source tracking in ``sr_cli.env_loader``.

These cover the small public surface that lets `sr model` / `sr setup`
...
from sr_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_sr_dotenv()`` is called at module-import time from several
    hot modules (cli.py, sr_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per SR_HOME per process.
    """
    ...
    # cli.py, sr_cli/main.py, run_agent.py, trajectory_compressor.py,
