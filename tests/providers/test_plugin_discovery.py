...
Verifies that:
 ...
 2. User plugins at $SR_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_sr_user_provider")
        ):
            del sys.modules[mod]
...
def test_user_plugin_overrides_bundled(tmp_path, monkeypatch):
    ...
    # Point SR_HOME at a fresh temp dir
    sr_home = tmp_path / ".sr"
    sr_home.mkdir()
    monkeypatch.setenv("SR_HOME", str(sr_home))
    # get_sr_home() may be module-cached depending on codebase; ensure the
    # env var is the source of truth. Most code paths re-read it each call.

    # Drop a user plugin that replaces 'gmi'
    user_gmi = sr_home / "plugins" / "model-providers" / "gmi"
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from sr_cli import plugins as plugin_mod
    ...
    user_plugin = sr_home / "plugins" / "test-model-provider"
