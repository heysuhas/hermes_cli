"""
Tests for mcp_serve — SR MCP server.
...
def _isolate_sr_home(tmp_path, monkeypatch):
    """Redirect SR_HOME to a temp directory."""
    monkeypatch.setenv("SR_HOME", str(tmp_path))
    try:
        import sr_constants
        monkeypatch.setattr(sr_constants, "get_sr_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking sr_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <SR_HOME>/state.db for its mtime gate; the autouse
        # fixture points SR_HOME at tmp_path.
        db_path = tmp_path / "state.db"
