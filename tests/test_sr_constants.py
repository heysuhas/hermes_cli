...
from sr_constants import (
    ...
    find_sr_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_sr_root,
    get_sr_dir,
    get_sr_home,
    heal_sr_managed_node,
    sr_managed_node_tree_present,
    iter_sr_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultSRRoot:
    """Tests for get_default_sr_root() — Docker/custom deployment awareness."""

    def test_no_sr_home_returns_native(self, tmp_path, monkeypatch):
        """When SR_HOME is not set, returns ~/.sr."""
        monkeypatch.delenv("SR_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_sr_root() == tmp_path / ".sr"

    def test_sr_home_is_native(self, tmp_path, monkeypatch):
        """When SR_HOME = ~/.sr, returns ~/.sr."""
        native = tmp_path / ".sr"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(native))
        assert get_default_sr_root() == native

    def test_sr_home_is_profile(self, tmp_path, monkeypatch):
        """When SR_HOME is a profile under ~/.sr, returns ~/.sr."""
        native = tmp_path / ".sr"
    ...
    def test_sr_home_is_docker(self, tmp_path, monkeypatch):
        """When SR_HOME points outside ~/.sr (Docker), returns SR_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(docker_home))
        assert get_default_sr_root() == docker_home

    def test_sr_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SR_HOME outside ~/.sr is treated as the root."""
        custom = tmp_path / "my-sr-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(custom))
        assert get_default_sr_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SR_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SR_HOME", str(profile))
        assert get_default_sr_root() == docker_root

    def test_no_sr_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\sr, not ~/.sr."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SR_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(sr_constants.sys, "platform", "win32")

        assert get_default_sr_root() == local_appdata / "sr"

    def test_no_sr_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/sr without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("SR_HOME", raising=False)
        ...
        assert get_default_sr_root() == home / "AppData" / "Local" / "sr"


class TestGetSRHome:
    """Tests for get_sr_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When SR_HOME is unset on Windows, use %LOCALAPPDATA%\\sr."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("SR_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(sr_constants.sys, "platform", "win32")
        monkeypatch.setattr(sr_constants, "_profile_fallback_warned", False)

        assert get_sr_home() == local_appdata / "sr"


class TestSRManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_sr_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sr_constants.sys, "platform", "win32")
        monkeypatch.setenv("SR_HOME", str(home))
        monkeypatch.setattr(sr_constants, "node_tool_runnable", lambda path: True)

        assert find_sr_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SR_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(sr_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(sr_constants, "heal_sr_managed_node", lambda: False)
        monkeypatch.setattr(
            sr_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert sr_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_sr_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "sr"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_sr_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken SR-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(sr_constants, "heal_sr_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_sr_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(sr_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(sr_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".sr"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import sr_cli._subprocess_compat as subprocess_compat
...
class TestGetSRDir:
    """Tests for ``get_sr_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_sr_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_sr_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_sr_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_sr_dir("cache/audio", "audio_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert sr_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sr_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert sr_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert sr_constants.windows_path_to_wsl("/home/alex") is None
        assert sr_constants.windows_path_to_wsl("relative\\dir") is None

    def test_wsl_unc_to_posix_both_spellings(self):
        assert sr_constants.wsl_unc_path_to_posix(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        assert sr_constants.wsl_unc_path_to_posix(r"\\wsl$\Ubuntu\home\alex") == "/home/alex"
        # Forward-slash spelling and distro root.
        assert sr_constants.wsl_unc_path_to_posix("//wsl.localhost/Debian/srv/app") == "/srv/app"
        assert sr_constants.wsl_unc_path_to_posix("\\\\wsl.localhost\\Ubuntu\\") == "/"

    def test_wsl_unc_ignores_non_unc_paths(self):
        assert sr_constants.wsl_unc_path_to_posix(r"C:\Users\alex") is None
        assert sr_constants.wsl_unc_path_to_posix("/home/alex") is None

    def test_translate_is_noop_off_wsl(self, monkeypatch):
        monkeypatch.setattr(sr_constants, "is_wsl", lambda: False)
        assert sr_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == r"C:\Users\alex"

    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(sr_constants, "is_wsl", lambda: True)
        assert sr_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert sr_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert sr_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
