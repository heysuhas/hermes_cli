...
Run from the PR worktree:
    ...
    cd ~/.sr/sr-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.sr/sr-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.sr/sr-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a sr-agent checkout"
)
...
# Isolated SR_HOME for the config write.
