...
Build the real image and verify at runtime:
  ...
  1. /opt/sr is not writable by the sr user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and SR_DISABLE_LAZY_INSTALLS are set
  3. /opt/sr/.install_method contains "docker" (code-scoped stamp)
  4. $SR_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $SR_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_sr(
    built_image: str, container_name: str,
) -> None:
    """The sr user must not be able to modify /opt/sr.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/sr as the sr user
        "touch /opt/sr/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sr user can write to /opt/sr (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/sr/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"sr user can write to /opt/sr/.venv: {r.stdout}"
    )


def test_sr_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    SR_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/sr/.install_method (code-scoped), NOT in $SR_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/sr/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/sr/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $SR_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$SR_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $SR_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $SR_HOME was not healed on boot: "
        f"{r.stdout}"
    )
