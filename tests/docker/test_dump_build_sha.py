"""Regression test: ``sr dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``sr dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$SR_GIT_SHA`` build-arg to ``/opt/sr/.sr_build_sha`` and
``sr_cli/build_info.py`` reads it as a fallback.
...
is absent and ``sr dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/sr/.sr_build_sha`` exists in the image, assert that
  ``sr dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's sr user (UID 10000) can bootstrap
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/sr/.sr_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``SR_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``sr dump``
