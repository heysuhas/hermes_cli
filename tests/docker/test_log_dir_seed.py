"""Runtime smoke test for Docker $SR_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the sr user after container boot.
...
registered later runs its log service as the dropped sr user and
...
def test_logs_gateways_seeded_and_sr_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by sr after boot."""
    ...
    assert "logs=sr" in r.stdout, (
        f"logs/ not owned by sr: {r.stdout}"
    )
    assert "gateways=sr" in r.stdout, (
        f"logs/gateways/ not owned by sr: {r.stdout}"
    )
