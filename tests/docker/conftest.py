...
Override the image with ``SR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("SR_TEST_IMAGE", "sr-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``SR_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("SR_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged sr user
# ---------------------------------------------------------------------------
#
# Background: every SR runtime path inside the container drops to UID
# 10000 (the ``sr`` user) via ``s6-setuidgid sr``. ``docker exec``
...
# fails (PermissionError swallowed) for sr, so a test that ran as root
...
# helpers below run every probe under ``-u sr`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: sr).
