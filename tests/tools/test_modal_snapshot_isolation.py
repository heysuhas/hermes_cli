...
def _restore_tool_modules():
    original_sr_home = os.environ.get("SR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_sr_home is None:
            os.environ.pop("SR_HOME", None)
        else:
            os.environ["SR_HOME"] = original_sr_home
        _reset_modules(("tools", "sr_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    sr_cli = types.ModuleType("sr_cli")
    sr_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["sr_cli"] = sr_cli
    sr_home = tmp_path / "sr-home"
    os.environ["SR_HOME"] = str(sr_home)
    sys.modules["sr_cli.config"] = types.SimpleNamespace(
        get_sr_home=lambda: sr_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="sr-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": sr_home / "modal_snapshots.json",
