...
class TestWriteDenyExactPaths:
    ...
    def test_sr_env(self):
        # ``.env`` under the active SR_HOME (profile-aware, not just
        # ``~/.sr``) must be write-denied. The hermetic test conftest
        # points SR_HOME at a tempdir — resolve via get_sr_home()
        # to match the denylist.
        from sr_constants import get_sr_home
        path = str(get_sr_home() / ".env")
        assert _is_write_denied(path) is True

    def test_sr_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.sr/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: SR_HOME does point to the profile dir, not the root.
        from sr_constants import get_sr_home, get_default_sr_root
        assert get_sr_home() == profile_home
        assert get_default_sr_root() == root
...
class TestWriteAllowed:
    ...
    def test_sr_control_files_requested_writable(self):
        from sr_constants import get_sr_home
