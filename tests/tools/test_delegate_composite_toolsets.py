...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_sr_cli_expands_web(self):
        """sr-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"sr-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("sr-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['sr-cli'] yields ['web']."""
        parent_toolsets = {"sr-cli"}
