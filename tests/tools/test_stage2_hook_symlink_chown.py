...
def _chown_sr_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    sr_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    sr_home = target if sr_home is None else sr_home
    script = (
        "set -eu\n"
        f'SR_HOME="{sr_home}"\n'
        f"{_chown_sr_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_sr_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "sr-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked SR_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_sr_home_trees(stage2_text: str) -> None:
    assert 'chown_sr_tree "$SR_HOME/$sub"' in stage2_text
    assert 'chown_sr_tree "$SR_HOME/profiles"' in stage2_text
    assert 'chown_sr_tree "$SR_HOME/cron"' in stage2_text
    assert 'chown -R sr:sr "$SR_HOME/$sub"' not in stage2_text
    assert 'chown -R sr:sr "$SR_HOME/profiles"' not in stage2_text
    assert 'chown -R sr:sr "$SR_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_sr_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$SR_HOME"' in stage2_text
