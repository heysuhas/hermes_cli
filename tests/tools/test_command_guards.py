...
def _clean_state():
    ...
    saved = {}
    for k in ("SR_INTERACTIVE", "SR_GATEWAY_SESSION", "SR_EXEC_ASK", "SR_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SR_INTERACTIVE"] = "1"
        session_key = os.getenv("SR_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SR_INTERACTIVE or SR_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
