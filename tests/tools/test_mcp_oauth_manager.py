...
def test_sr_provider_subclass_exists():
    """SRMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SR_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SR_PROVIDER_CLS is not None
    assert issubclass(_SR_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_sr_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns SRMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SR_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SR_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _SR_PROVIDER_CLS is not None
    assert isinstance(provider, _SR_PROVIDER_CLS)
    assert provider._sr_server_name == "srv"
