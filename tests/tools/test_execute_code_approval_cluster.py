...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad SR_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: SR_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_sr_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "SR_HOME": "/h", "SR_PROFILE": "p",
        "SR_CONFIG": "/c.yaml", "SR_ENV": "/e",
        # other SR_* → dropped (broad prefix removed)
        "SR_BASE_URL": "https://x", "SR_INTERACTIVE": "1",
        "SR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("SR_HOME", "SR_PROFILE", "SR_CONFIG", "SR_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SR_BASE_URL", "SR_INTERACTIVE", "SR_KANBAN_DB",
...
def test_env_scrub_logs_dropped_sr_vars(caplog):
    """Dropping a non-allowlisted, non-secret SR_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset SR_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "SR_HOME": "/h",          # allowlisted → kept, not logged
        "SR_BASE_URL": "https://x",   # dropped → logged
        "SR_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SR_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SR_HOME" in out and "PATH" in out
    assert "SR_BASE_URL" not in out and "SR_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SR_BASE_URL" in msgs and "SR_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SR_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped SR_* vars."""
    import logging
