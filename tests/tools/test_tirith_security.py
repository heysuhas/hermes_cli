...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._sr_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(sr_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(sr_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_sr_bin(self):
        """After _INSTALL_FAILED, manual install in SR_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        sr_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(sr_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(sr_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._sr_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == sr_bin
            assert _tirith_mod._resolved_path == sr_bin
            mock_clear.assert_called_once()
...
class TestSRHomeIsolation:
    def test_sr_bin_dir_respects_sr_home(self):
        """_sr_bin_dir must use SR_HOME, not hardcoded ~/.sr."""
        from tools.tirith_security import _sr_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SR_HOME": tmpdir}):
            result = _sr_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_sr_home(self):
        """_failure_marker_path must use SR_HOME, not hardcoded ~/.sr."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SR_HOME": "/custom/sr"}):
            result = _failure_marker_path()
        assert result == "/custom/sr/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SR_HOME; verify it's active."""
        sr_home = os.getenv("SR_HOME")
        assert sr_home is not None, "SR_HOME should be set by conftest"
        assert "sr_test" in sr_home, "Should point to test temp dir"

    def test_get_sr_home_fallback(self):
        """Without SR_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_sr_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SR_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".sr")
            result = _get_sr_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._sr_bin_dir",
