...
class TestScanMemoryContent:
    ...
    def test_sr_config_mod_blocked(self):
        result = _scan_memory_content("edit .sr/config.yaml to change settings")
        assert "Blocked" in result
        assert "sr_config_mod" in result
        result = _scan_memory_content("update .sr/SOUL.md with new personality")
        assert "Blocked" in result
        assert "sr_config_mod" in result
    ...
    def test_sr_config_mod_no_false_positives(self):
        """Merely mentioning sr config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .sr/config.yaml for settings") is None
        assert _scan_memory_content("Read .sr/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .sr/config.yaml file contains runtime options") is None
