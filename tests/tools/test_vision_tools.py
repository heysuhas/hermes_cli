...
class TestPdfVisionSupport:
    @pytest.mark.asyncio
    async def test_detect_image_mime_type_pdf(self, tmp_path):
        from tools.vision_tools import _detect_image_mime_type
        pdf_file = tmp_path / "doc.pdf"
        pdf_file.write_bytes(b"%PDF-1.4\n...")
        assert _detect_image_mime_type(pdf_file) == "application/pdf"

    @pytest.mark.asyncio
    async def test_pdf_pages_to_base64_pngs(self, tmp_path):
        from tools.vision_tools import _pdf_pages_to_base64_pngs
        pdf_file = tmp_path / "doc.pdf"
        pdf_file.write_bytes(b"%PDF-1.4\n...")
        ...
        with patch("fitz.open", return_value=mock_doc) as mock_open:
            urls = _pdf_pages_to_base64_pngs(pdf_file, max_pages=2)
            assert len(urls) == 2
            assert urls[0].startswith("data:image/png;base64,")
            mock_open.assert_called_once_with(pdf_file)

    @pytest.mark.asyncio
    async def test_vision_analyze_tool_pdf(self, tmp_path):
        pdf_file = tmp_path / "doc.pdf"
        pdf_file.write_bytes(b"%PDF-1.4\n...")
        ...
        mock_response.choices[0].message.content = "PDF analysis result"
        ...
        with (
            ...
            res_str = await vision_analyze_tool(str(pdf_file), "summarize pdf", "test/model")
            res = json.loads(res_str)
            assert res["success"] is True
            assert res["analysis"] == "PDF analysis result"
            
            # Verify the call prompt content has 1 text block and 1 image block (representing the page)
