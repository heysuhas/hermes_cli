...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        (sr_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"SR_HOME": str(sr_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(sr_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.sr/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        (sr_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        (sr_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.sr/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        sr_home = tmp_path / ".sr"
        (sr_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SR_HOME": str(sr_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.sr")

        assert mounts[0]["container_path"] == "/home/user/.sr/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.sr/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.sr/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SR_HOME."""
        monkeypatch.setenv("SR_HOME", str(tmp_path / ".sr"))
        (tmp_path / ".sr").mkdir()

        # Create a sensitive file one level above sr_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside sr_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SR_HOME must still be registered."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        monkeypatch.setenv("SR_HOME", str(sr_home))
        (sr_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_sr_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SR_HOME must be allowed."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        subdir = sr_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SR_HOME pointing outside must be rejected."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        monkeypatch.setenv("SR_HOME", str(sr_home))

        # Create a sensitive file outside sr_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside sr_home pointing outside
        symlink = sr_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, sr_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SR_HOME."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        monkeypatch.setenv("SR_HOME", str(sr_home))
        ...
        sensitive.write_text("{}")
        self._write_config(sr_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(sr_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SR_HOME via config must still mount."""
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        monkeypatch.setenv("SR_HOME", str(sr_home))

        (sr_home / "oauth.json").write_text("{}")
        self._write_config(sr_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        (sr_home / "cache" / "documents").mkdir(parents=True)
        (sr_home / "cache" / "audio").mkdir(parents=True)
        (sr_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("SR_HOME", str(sr_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.sr/cache/documents" in paths
        assert "/root/.sr/cache/audio" in paths
        assert "/root/.sr/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.sr/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_sr_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        sr_home = tmp_path / ".sr"
        sr_home.mkdir()
        # Use legacy dir name with content — get_sr_dir prefers
        # populated old over new.
        legacy_doc = sr_home / "document_cache"
        legacy_img = sr_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(sr_home / "document_cache") in host_paths
        assert str(sr_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.sr/cache/documents" in container_paths
        assert "/root/.sr/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        ...
        assert (
            ...
            == "/root/.sr/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.sr")
            == "/home/agent/.sr/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        sr_home = tmp_path / ".sr"
        (sr_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("SR_HOME", str(sr_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(sr_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        sr_home = tmp_path / ".sr"
        doc_dir = sr_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        sr_home = tmp_path / ".sr"
        ss_dir = sr_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.sr/cache/screenshots/session_abc/screen1.png"
