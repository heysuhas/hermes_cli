...
- Warning respects SR_AGENT_TIMEOUT_WARNING env var
...
class TestStagedInactivityWarning:
    ...
    def test_warning_env_var_respected(self, monkeypatch):
        """SR_AGENT_TIMEOUT_WARNING env var is parsed correctly."""
        monkeypatch.setenv("SR_AGENT_TIMEOUT_WARNING", "600")
        _warning = float(os.getenv("SR_AGENT_TIMEOUT_WARNING", 900))
        assert _warning == 600.0

    def test_warning_zero_means_disabled(self, monkeypatch):
        """SR_AGENT_TIMEOUT_WARNING=0 disables the warning."""
        monkeypatch.setenv("SR_AGENT_TIMEOUT_WARNING", "0")
        _raw = float(os.getenv("SR_AGENT_TIMEOUT_WARNING", 900))
