...
closure the PR changed, against a real temp ``SR_HOME``.
"""
...
def _fake_switch_result():
    ...
    from sr_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "sr_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_sr_home() / config.yaml`` — point it here.
    monkeypatch.setattr("sr_constants.get_sr_home", lambda: sr_home)
    monkeypatch.setattr("sr_cli.config.get_sr_home", lambda: sr_home)
    return cfg_path
