...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_sr_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <SR_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``sr pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <SR_HOME>/profiles/yangyang/pairing/."""
        from sr_constants import get_sr_home
        monkeypatch.setattr("sr_constants.get_sr_home", lambda: tmp_path)
