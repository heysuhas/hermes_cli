...
SR middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing SR'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before SR calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving SR retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving SR guardrails,
...
- `telemetry_schema_version`: currently `sr.observer.v1`
- `middleware_schema_version`: currently `sr.middleware.v1`
...
SR stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, SR runs
them as a nested chain in registration order. Middleware failures are fail-open:
SR logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, SR applies middleware in this order:
...
For each tool call, SR applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal SR pre-execution path against the effective arguments:
...
```bash
sr plugins enable <plugin-name>
```

For isolated local testing, use one `SR_HOME` for plugin enablement and the
agent run:

```bash
export SR_HOME=/tmp/sr-middleware-test
mkdir -p "$SR_HOME"
sr plugins enable <plugin-name>
sr chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run sr plugins enable <plugin-name>
uv run sr chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["sr_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/sr-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape SR expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, SR treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, SR preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. SR does not convert downstream
