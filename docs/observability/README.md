...
SR observer hooks are the read-only telemetry contract for plugins that
...
They do not replace SR' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. SR catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so SR only builds
