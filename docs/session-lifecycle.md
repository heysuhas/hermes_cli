...
The canonical transcript store is SQLite via `SessionDB` (from `sr_state`). The
...
Counts consecutive restarts via a JSON file (`{SR_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `sr update`, `sr gateway restart`,
or `/restart`.
