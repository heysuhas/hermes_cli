...
When running SR inside Docker, the default `network_mode: host` gives the
...
The SR [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   SR_UID=$(id -u) SR_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=sr,sr-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://sr-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — SR trust model and vulnerability reporting
