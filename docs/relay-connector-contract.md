...
This document is the formal interface between the **SR gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`SamsungResearch/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `sr gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`sr gateway enroll` → connector `/relay/enroll`, or
...
(`SamsungResearch/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `sr` CLI.
