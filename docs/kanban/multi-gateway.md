...
SR supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.sr/config.yaml`:
...
Or set the env var: `SR_KANBAN_DISPATCH_IN_GATEWAY=false`
