"""
SR MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (SR-specific extra)

Usage:
    sr mcp serve
    sr mcp serve --verbose
...
logger = logging.getLogger("sr.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SR_HOME."""
    try:
        from sr_constants import get_sr_home
        return get_sr_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SR_HOME", Path.home() / ".sr")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from sr_constants import get_sr_home
        directory_file = get_sr_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SR_HOME", Path.home() / ".sr")
...
class EventBridge:
    ...
    This is the SR equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from sr_constants import get_sr_home
            db_file = get_sr_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("SR_HOME", Path.home() / ".sr")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the SR MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "sr",
        instructions=(
            "SR Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the SR MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
