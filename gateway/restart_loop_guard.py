...
Defenses 1 and 2 (the ``_SR_GATEWAY`` guard on ``sr gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.sr.gateway")``,
...
State lives in ``<SR_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_sr_home() / "gateway" / "restart_loop.json"
