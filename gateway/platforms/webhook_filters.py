...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.sr to the active profile home."""
    ...
    if raw.startswith("~/.sr/"):
        return sr_home / raw.removeprefix("~/.sr/")
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under SR_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from sr_constants import get_sr_home

    scripts_root = (get_sr_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.sr" or raw_text.startswith("~/.sr/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__sr_ignore__") is True
        ):
            return False, None
