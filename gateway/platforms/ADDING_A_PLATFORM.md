...
There are two ways to add a platform to the SR gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sr/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `sr gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `sr_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the SR core.
...
And add it to the `sr-gateway` composite:

```python
"sr-gateway": {
    "includes": [..., "sr-your-platform"]
}
```
...
## 12. Status Display (`sr_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sr_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sr_cli/ toolsets.py \
  --include="*.py" -l | sort -u
