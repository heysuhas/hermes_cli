...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``sr setup`` display).
    ...
    # built-ins).  Used by ``sr gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `sr`
        # invocation -- including plain `sr chat`, which never touches any
        ...
        # `sr setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``sr setup``/``gateway status``, channel
