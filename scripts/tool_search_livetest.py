#!/usr/bin/env python3
"""Live test harness for SR Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any sr imports.
ORIGINAL_HOME = os.environ.get("SR_HOME")
ORIGINAL_AUTH = Path.home() / ".sr" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.sr/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.sr/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="sr_ts_live_"))
    sr_home = home_dir / ".sr"
    sr_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sr_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".sr" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, sr_home / ".env")
        ...
        load_sr_dotenv(sr_home=str(Path.home() / ".sr"))
    ...
    (sr_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return sr_home
...
def main():
    ...
    # Restore original SR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SR_HOME"] = ORIGINAL_HOME
    else:
