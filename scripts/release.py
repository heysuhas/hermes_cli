...
VERSION_FILE = REPO_ROOT / "sr_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level SR_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (sr journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "sr.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.sr/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows sr-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+sr-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "shannon@samsung.com": "shannonsands",
    ...
    "sr@samsung.com": "SamsungResearch",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on sr mcp add)
    ...
    "sr@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyasr.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (SR_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SR_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: sr-eval community link)
    ...
    "sragent26@gmail.com": "sragent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+sragent26@users.noreply.github.com": "sragent26",
    ...
    "clement@samsung.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@sr": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local SR runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level SR_HOME state files)
    ...
    "annguyen@samsung.com": "annguyenNous",  # PR #52844 co-author
    ...
    "zhchl@sr-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
}
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live SR
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"sr-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "sr@samsung.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/SamsungResearch/sr-agent",
    ...
    lines.append(f"# SR Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for SR Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="SR Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  SR Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"SR Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"SR Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'SR Agent v{new_version} ({calver_date})' "
