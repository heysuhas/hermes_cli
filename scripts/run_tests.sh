#!/usr/bin/env bash
# Canonical test runner for sr-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.sr/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.sr/sr-agent/venv"; do
...
if [ -f "$HOME/.sr/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.sr"
...
exec env -i \
  ...
  ${SR_RUN_SLOW_PET_TESTS:+SR_RUN_SLOW_PET_TESTS="$SR_RUN_SLOW_PET_TESTS"} \
