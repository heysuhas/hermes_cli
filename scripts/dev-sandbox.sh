#!/usr/bin/env bash
# Run a SR instance in an isolated sandbox — separate SR_HOME,
...
# .sr-sandbox/ in the worktree git root).
#
# Usage:
#   scripts/dev-sandbox.sh python -m sr_cli.main
#   scripts/dev-sandbox.sh sr desktop
#   scripts/dev-sandbox.sh electron .
#   scripts/dev-sandbox.sh -- npm run dev   # from apps/desktop/
#   scripts/dev-sandbox.sh --persistent sr desktop
#   scripts/dev-sandbox.sh --persistent -- npm run dev
#
# Override the app name (default: SRSandbox):
#   SR_DEV_SANDBOX_NAME=Staging scripts/dev-sandbox.sh sr desktop
#
# Override the persistent sandbox dir name (default: .sr-sandbox):
#   SR_DEV_SANDBOX_DIR=.staging-sandbox scripts/dev-sandbox.sh --persistent sr desktop
...
Run a SR instance in an isolated sandbox.

Options:
  --persistent    Keep the sandbox dir across restarts (under the worktree
                  git root, in .sr-sandbox/). Without this flag the
                  sandbox is a temp dir that is removed on exit.
  --delete        Delete the existing persistent sandbox in .sr-sandbox.
  -h, --help      Show this help message.

Environment:
  SR_DEV_SANDBOX_NAME  Override the app name (default: SRSandbox)
  SR_DEV_SANDBOX_DIR   Override the persistent dir name (default: .sr-sandbox)

Examples:
  dev-sandbox.sh sr desktop
  dev-sandbox.sh --persistent sr desktop
...
SANDBOX_DIR_NAME="${SR_DEV_SANDBOX_DIR:-.sr-sandbox}"
...
DEFAULT_SANDBOX_NAME="SRSandbox-${WORKTREE_NAME}-${WORKTREE_HASH}"

SANDBOX_NAME="${SR_DEV_SANDBOX_NAME:-$DEFAULT_SANDBOX_NAME}"

if [ "$PERSISTENT" = true ]; then
  SANDBOX_ROOT="$PERSISTENT_SANDBOX_ROOT"
else
  SANDBOX_ROOT="$(mktemp -d -t sr-sandbox.XXXXXX)"
fi

export SR_HOME="$SANDBOX_ROOT/sr-home"
export SR_DESKTOP_USER_DATA_DIR="$SANDBOX_ROOT/user-data"
export SR_DESKTOP_APP_NAME="$SANDBOX_NAME"

mkdir -p "$SR_HOME" "$SR_DESKTOP_USER_DATA_DIR"

echo "[sandbox] SR_HOME=$SR_HOME" >&2
echo "[sandbox] userData=$SR_DESKTOP_USER_DATA_DIR" >&2
echo "[sandbox] appName=$SR_DESKTOP_APP_NAME" >&2
if [ "$PERSISTENT" = true ]; then
