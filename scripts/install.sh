...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/heysuhas/hermes_cli/main/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. SR terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:heysuhas/hermes_cli.git"
REPO_URL_HTTPS="https://github.com/heysuhas/hermes_cli.git"
SR_HOME="${SR_HOME:-$HOME/.sr}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SR_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SR_INSTALL_DIR"
...
#   code at /usr/local/lib/sr-agent, command at /usr/local/bin/sr,
#   data still at /root/.sr (SR_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$SR_HOME/.no-bundled-skills so future"
            echo "                   'sr update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> SR.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.sr/sr-agent"
            echo "                   default (root, Linux): /usr/local/lib/sr-agent"
            echo "  --sr-home PATH  Data directory (default: ~/.sr, or \$SR_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, SR installs the code under"
            echo "  /usr/local/lib/sr-agent and links the command into"
            echo "  /usr/local/bin/sr (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SR_HOME"
            echo "  (default /root/.sr).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SR_HOME/sr-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs sr setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ SR Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Samsung Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `sr update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (SR-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download SR Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install sr command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `sr` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SR_HOME/sr-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SR_HOME/sr-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sr-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SR_HOME/sr-agent — then preserve it)
#
# Always no-op when the user set --dir or $SR_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SR_HOME.
    if is_termux; then
        INSTALL_DIR="$SR_HOME/sr-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SR_HOME/sr-agent/.git" ]; then
            INSTALL_DIR="$SR_HOME/sr-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sr-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sr-agent"
        ...
        # /usr/local/bin/sr wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/sr"
        log_info "  Data:    $SR_HOME (unchanged)"
...
# Point a SR-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $SR_HOME/node/bin — which is NOT on
...
# SR's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no SR-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$SR_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$SR_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$SR_HOME/node/etc/npmrc"
}

get_sr_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/sr" ]; then
        echo "$link_dir/sr"
    else
        echo "sr"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/heysuhas/hermes_cli/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # SR owns its own uv at $SR_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sr_cli/managed_uv.py) looks in the same
    # place, so install.sh and `sr update` stay in sync.
    local _managed_uv="$SR_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $SR_HOME/bin ..."
    mkdir -p "$SR_HOME/bin"
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/sr-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sr-uv-installer.$$.sh")"
    ...
    # directly into $SR_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$SR_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the SR-
...
check_node() {
    ...
    # Repair pre-existing SR-managed installs where `npm install -g` lands
    ...
    if [ -x "$SR_HOME/node/bin/node" ] && node_satisfies_build "$("$SR_HOME/node/bin/node" --version)"; then
        export PATH="$SR_HOME/node/bin:$PATH"
        log_success "Node.js $("$SR_HOME/node/bin/node" --version) found (SR-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing SR-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.sr/node/..."
    ...
    # Place into ~/.sr/node/ and symlink binaries into the same bin dir
    # the sr command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$SR_HOME/node"
    mkdir -p "$SR_HOME"
    mv "$extracted_dir" "$SR_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$SR_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$SR_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$SR_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$SR_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.sr/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. SR install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "SR Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `sr update` path
                # (#4735).
                ...
                local stash_name
                stash_name="sr-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``sr update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if SR behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up sr command..."

    if [ "$USE_VENV" = true ]; then
        SR_BIN="$INSTALL_DIR/venv/bin/sr"
    else
        SR_BIN="$(which sr 2>/dev/null || echo "")"
        if [ -z "$SR_BIN" ]; then
            log_warn "sr not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$SR_BIN" ]; then
        log_warn "sr entry point not found at $SR_BIN"
    ...
    # Create a user-facing shim for the sr command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $SR_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$SR_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/sr"
    cat > "$command_link_dir/sr" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SR_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/sr"
    log_success "Installed sr launcher → $command_link_display_dir/sr"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sr command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sr' \
                >/dev/null 2>&1; then
        ...
        log_info "sr not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# SR Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# SR Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# SR Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sr works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.sr directory structure (config at top level, code in subdir)
    mkdir -p "$SR_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.sr/.env (top level, easy to find)
    if [ ! -f "$SR_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SR_HOME/.env"
            log_success "Created ~/.sr/.env from template"
        else
            touch "$SR_HOME/.env"
            log_success "Created ~/.sr/.env"
        fi
    else
        log_info "~/.sr/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SR_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.sr/config.yaml (top level, easy to find)
    if [ ! -f "$SR_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SR_HOME/config.yaml"
            log_success "Created ~/.sr/config.yaml from template"
        fi
    else
        log_info "~/.sr/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in sr_cli/default_soul.py — the
    ...
    if [ ! -f "$SR_HOME/SOUL.md" ]; then
        cat > "$SR_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.sr/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.sr/"

    # Seed bundled skills into ~/.sr/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `sr update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'sr update'." \
            > "$SR_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $SR_HOME/.no-bundled-skills"
        log_info "  Future 'sr update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.sr/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.sr/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SR_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$SR_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.sr/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# SR Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "SR will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# SR Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (sr --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `sr update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sr setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m sr_cli.main setup < /dev/tty
    else
        python -m sr_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SR_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for SR to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SR_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'sr whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SR_CMD="$(get_sr_command_path)"
                $SR_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'sr whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sr gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SR_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SR_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: sr gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sr gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SR_CMD gateway > "$SR_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.sr/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: sr gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: sr gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SR_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SR_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SR_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}sr${NC}              Start chatting"
    echo -e "   ${GREEN}sr setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sr config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sr config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sr gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sr update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sr' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sr' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sr' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SR_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SR_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SR_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$SR_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$SR_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up SR for pip install"
    ...
    SR_CMD="$(command -v sr 2>/dev/null || echo "")"
    if [ -n "$SR_CMD" ]; then
        log_info "Running sr setup..."
        "$SR_CMD" setup
    else
        log_warn "sr command not found on PATH"
        log_info "Try: python -m sr_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`SR` rename
...
# _purge_electron_build_cache() used by `sr desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the SR-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `sr desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/SR" ]; then
            app="$desktop_dir/release/linux-unpacked/SR"
        elif [ -x "$desktop_dir/release/linux-unpacked/sr" ]; then
            app="$desktop_dir/release/linux-unpacked/sr"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/SR.app" \
            "$desktop_dir/release/mac/SR.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("SR is damaged and can't be opened").
    ...
    # checkout stays clean for the next `sr update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the SR-managed Node
            # provisioned during prerequisites/node-deps (at $SR_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $SR_HOME.
    # $SR_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'sr update' on this host install.
