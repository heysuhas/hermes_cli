...
# 5. Symlinks the 'sr' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sr-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink sr into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sr command..."

SR_BIN="$SCRIPT_DIR/venv/bin/sr"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SR_BIN" "$COMMAND_LINK_DIR/sr"
echo -e "${GREEN}✓${NC} Symlinked sr → $COMMAND_LINK_DISPLAY_DIR/sr"
...
# ============================================================================
# Seed bundled skills into ~/.sr/skills/
# ============================================================================

SR_SKILLS_DIR="${SR_HOME:-$HOME/.sr}/skills"
mkdir -p "$SR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sr/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SR_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sr status        # Check configuration"
if is_termux; then
    echo "  sr gateway       # Run gateway in foreground"
else
    echo "  sr gateway install # Install gateway service (messaging + cron)"
fi
echo "  sr cron list     # View scheduled jobs"
echo "  sr doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sr_cli.main setup
fi
