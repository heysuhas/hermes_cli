"""ACP session manager — maps ACP sessions to SR AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.sr/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when SR itself is running in WSL.

    Windows ACP clients can launch ``sr acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from sr_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch SR from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["sr-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed SR agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by SR AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current SR runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.sr/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SR_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_sr_home``).
        """
        ...
        try:
            ...
            sr_home = get_sr_home()
            self._db_instance = SessionDB(db_path=sr_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the SR daemon's process cwd.
