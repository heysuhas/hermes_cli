"""ACP auth helpers — detect and advertise SR authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "sr-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active SR runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if SR can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for SR.
    ...
    not have SR provider credentials configured yet, so SR always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate SR using the currently configured "
