...
#
# Wired into the image as /etc/cont-init.d/01-sr-setup by the
...
SR_HOME="${SR_HOME:-/opt/data}"
INSTALL_DIR="/opt/sr"

# Drop to sr via s6-setuidgid, but skip it when already non-root.
as_sr() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid sr "$@"; }
...
# starts non-root. The baked install tree under /opt/sr is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $SR_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass SR_UID/SR_GID — or the PUID/PGID aliases — which the
...
# `id -u sr` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# sr UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u sr)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-sr UID).
...
and the baked /opt/sr install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e SR_UID=\$(id -u) -e SR_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the sr user to that UID/GID at boot and chowns the data
...
# --- Bootstrap SR_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid sr mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom SR_HOME paths whose parents only
# root can create (e.g. `SR_HOME=/home/sr/.sr` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for SR_UID/SR_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  SR_UID/
# SR_GID still win when both are set.  See #15290, salvages #25872.
SR_UID="${SR_UID:-${PUID:-}}"
SR_GID="${SR_GID:-${PGID:-}}"

if [ -n "${SR_UID:-}" ] && validate_uid_gid "$SR_UID" && [ "$SR_UID" != "$(id -u sr)" ]; then
    echo "[stage2] Changing sr UID to $SR_UID"
    usermod -u "$SR_UID" sr
fi
if [ -n "${SR_GID:-}" ] && validate_uid_gid "$SR_GID" && [ "$SR_GID" != "$(id -g sr)" ]; then
    echo "[stage2] Changing sr GID to $SR_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$SR_GID" sr 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised sr user (UID 10000)
...
# sr process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes sr. Idempotent across container
...
#   - sr is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G sr 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] sr already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" sr 2>/dev/null; then
        echo "[stage2] Added sr to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group sr failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When SR_UID is remapped or the top-level $SR_HOME isn't owned by
# the runtime sr UID, restore ownership to sr — but ONLY for the
# directories sr actually writes to. The full $SR_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of sr-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_sr_uid=$(id -u sr)
...
chown_sr_tree() {
    ...
    chown -R sr:sr "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$SR_HOME" 2>/dev/null)" != "$actual_sr_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $SR_HOME (targeted) to sr ($actual_sr_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $SR_HOME: chown the directory itself (not its contents)
    # so sr can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$SR_HOME"; then
        :
    else
        chown sr:sr "$SR_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $SR_HOME failed (rootless container?) — continuing"
    fi
    # SR-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by sr (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$SR_HOME/$sub" ]; then
            chown_sr_tree "$SR_HOME/$sub"
        fi
    done
fi
...
# sr user. Hosted/container instances keep mutable state under
# $SR_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# SR_DISABLE_LAZY_INSTALLS=1. Keeping /opt/sr root-owned and
...
# $SR_HOME/lazy-packages dir on the data volume (Dockerfile sets
# SR_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to sr
...
# Always reset ownership of $SR_HOME/profiles to sr on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> sr …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as sr and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$SR_HOME/profiles" ]; then
    chown_sr_tree "$SR_HOME/profiles"
fi

# Always reset ownership of $SR_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged sr runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$SR_HOME/cron" ]; then
    chown_sr_tree "$SR_HOME/cron"
fi
...
# sr pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged sr gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $SR_HOME is
...
# is negligible.
if [ -d "$SR_HOME/platforms/pairing" ]; then
    chown_sr_tree "$SR_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$SR_HOME/pairing" ]; then
    chown_sr_tree "$SR_HOME/pairing"
fi

# Reset ownership of sr-owned top-level state files on every boot.
# The targeted data-volume chown above only covers sr-owned
# *subdirectories*; loose state files living directly under $SR_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> sr …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged sr runtime then hits
...
# sweep so host-owned files in a bind-mounted $SR_HOME are never
...
# entries of sr_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$SR_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$SR_HOME/$f"; then
            :
        else
            chown sr:sr "$SR_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the sr runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$SR_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$SR_HOME/config.yaml"; then
        :
    else
        chown sr:sr "$SR_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$SR_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as sr user ---
# Run as sr via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $SR_HOME values
...
as_sr mkdir -p \
    ...
    "$SR_HOME/home" \
    "$SR_HOME/pairing" \
    "$SR_HOME/platforms/pairing" \
    "$SR_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/sr/.install_method (see Dockerfile), NOT written here into
# $SR_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $SR_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.sr:/opt/data) and sometimes shared with a
...
# 'sr update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $SR_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/sr stamp, so deleting the data-dir copy is safe).
if [ -f "$SR_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$SR_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$SR_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$SR_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$SR_HOME/$dest"; then
            :
        else
            as_sr cp "$INSTALL_DIR/$src" "$SR_HOME/$dest"
        fi
    fi
}
...
if [ -f "$SR_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$SR_HOME/.env"; then
        :
    else
        chown sr:sr "$SR_HOME/.env" 2>/dev/null || true
        chmod 600 "$SR_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $SR_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `sr update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set SR_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$SR_HOME/config.yaml" ]; then
    s6-setuidgid sr "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$SR_HOME/auth.json" ] && [ -n "${SR_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$SR_HOME/auth.json"; then
        :
    else
        printf '%s' "$SR_AUTH_JSON_BOOTSTRAP" > "$SR_HOME/auth.json"
        chown sr:sr "$SR_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$SR_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via SR_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$SR_HOME/auth.json" ] && [ -n "${SR_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$SR_HOME/auth.json"; then
        :
    else
        s6-setuidgid sr "$INSTALL_DIR/.venv/bin/python" \
...
# can set SR_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# SR_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$SR_HOME/gateway_state.json" ] && \
        [ "${SR_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$SR_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$SR_HOME/gateway_state.json"
        chown sr:sr "$SR_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$SR_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_sr "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/sr/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI SR spawns for the
...
# main-wrapper.sh propagates it into the supervised ``sr`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-sr, dashboard, etc.).
