#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the SR docs site.
...
Both publish at:
  https://sr-agent.samsung.com/docs/llms.txt
  https://sr-agent.samsung.com/docs/llms-full.txt
...
SITE_BASE = "https://sr-agent.samsung.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use SR as a Python Library", None),
        ("guides/use-mcp-with-sr", "Use MCP with SR", None),
        ("guides/use-voice-mode-with-sr", "Use Voice Mode with SR", None),
        ("guides/use-soul-with-sr", "Use SOUL.md with SR", None),
        ("guides/build-a-sr-plugin", "Build a SR Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with SR"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# SR Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Samsung Research. A terminal-native "
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/SamsungResearch/"
        "sr-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/SamsungResearch/sr-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# SR Agent — Full Documentation\n",
        (
            "This file is the entire SR Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using SR, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://sr-agent.samsung.com/docs\n",
        "Short index: https://sr-agent.samsung.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
