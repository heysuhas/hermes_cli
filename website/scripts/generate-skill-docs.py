...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/SamsungResearch/sr-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sr_meta = (fm.get("metadata") or {}).get("sr") or {}
    tags = sr_meta.get("tags") or []
    related = sr_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sr skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with SR Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "SR ships with a large built-in skill library copied into `~/.sr/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "SR also syncs bundled skills on `sr update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.sr/skills/` tree, it is still shipped with SR; restore it with `sr skills reset <name> --restore`.",
        "",
