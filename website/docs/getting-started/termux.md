...
description: "Run SR Agent directly on an Android phone with Termux"
---

# SR on Android with Termux
...
SR Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop SR from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

SR now ships a Termux-aware installer path:
...
- links `sr` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `sr` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/sr" "$PREFIX/bin/sr"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `sr` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.sr/.env`.
...
### `sr doctor` says ripgrep or Node is missing
