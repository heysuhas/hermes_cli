...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by SR Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during SR terminal calls (desktop app flow only)
...
### Service Account (recommended for SR)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${SR_HOME:-~/.sr}/.env` (the skill will prompt for this on first load).
...
## SR Execution Pattern (desktop app flow)

SR terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/sr-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sr-op.sock"
