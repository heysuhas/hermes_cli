...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/productivity/canvas` |
...
4. Name the token (e.g., "SR Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${SR_HOME:-~/.sr}/.env`:
...
```bash
CANVAS="python $SR_HOME/skills/productivity/canvas/scripts/canvas_api.py"
