...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SR_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client sr
```
...
```bash
PUBLISH="${SR_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client sr
```
...
```bash
DRIVE="${SR_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `sr`)    |
