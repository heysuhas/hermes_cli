---
title: "Telephony — Give SR phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give SR phone capabilities without core tool changes"
---
...
Give SR phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sr skills install official/productivity/telephony` |
...
This optional skill gives SR practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${SR_HOME:-~/.sr}/.env`
...
It does **not** turn SR into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
### 1) "I want SR to own a real phone number"
Use **Twilio**.
...
- pairs well with SR `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.sr/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
sr skills search telephony
sr skills install official/productivity/telephony
```
...
3. Buy it and save it into `${SR_HOME:-~/.sr}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is SR calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing SR's existing `text_to_speech` support.

Use this when:
- you want the call to use SR's configured TTS voice rather than Twilio `<Say>`
...
Recommended SR TTS -> Twilio Play workflow:

1. Generate the audio with SR `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask SR to create the message audio with `text_to_speech`
...
Important note:
- SR TTS is great for prerecorded outbound messages
...
- SR STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to SR memory.
...
- Do not store arbitrary third-party phone numbers in SR memory.
...
3. persist that number to `${SR_HOME:-~/.sr}/.env`
