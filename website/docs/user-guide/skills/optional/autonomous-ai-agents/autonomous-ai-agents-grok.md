...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), SR Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`sr-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sr-agent) |
...
# Grok Build CLI — SR Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the SR terminal. Grok
...
1. Prepare stable input files first with SR tools (`read_file`,
...
2. **Don't conflate SR' xAI auth with the `grok` CLI's auth.** SR'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume SR' xAI auth carries
   over.
