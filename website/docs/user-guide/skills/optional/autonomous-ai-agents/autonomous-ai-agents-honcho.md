...
description: "Configure and use Honcho memory with SR -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with SR -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sr skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for SR

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every SR profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/sr#running-honcho-locally-with-sr

### Verify

```bash
sr honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this SR profile's AI peer
...
Honcho models conversations as interactions between **peers**. SR creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this SR instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each SR run |
| `global` | Single session across all directories |

Manual override: `sr honcho map my-project-name`
...
Each SR profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block sr.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `sr.coder` host block in `honcho.json`
...
```bash
sr honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for SR when Honcho memory is active.
...
Config file: `$SR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `sr honcho setup`. Ensure `memory.provider: honcho` is in `~/.sr/config.yaml`.

### Memory not persisting across sessions
Check `sr honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sr profile create <name> --clone`. For existing profiles: `sr honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sr honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sr honcho status` | Show resolved config, connection test, peer info for active profile |
| `sr honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sr honcho disable` | Disable Honcho for the active profile |
| `sr honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sr honcho peers` | Show peer identities across all profiles |
| `sr honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sr honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sr honcho sessions` | List known directory-to-session-name mappings |
| `sr honcho map <name>` | Map current working directory to a Honcho session name |
| `sr honcho identity` | Seed AI peer identity or show both peer representations |
| `sr honcho sync` | Create host blocks for all SR profiles that don't have one yet |
| `sr honcho migrate` | Step-by-step migration guide from OpenClaw native memory to SR + Honcho |
| `sr memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sr memory status` | Show active memory provider and config |
| `sr memory off` | Disable external memory provider |
