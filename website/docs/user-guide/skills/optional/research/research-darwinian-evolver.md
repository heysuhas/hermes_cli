...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), SR Agent |
| License | MIT |
...
import upstream classes into SR itself.
...
```bash
mkdir -p ~/.sr/cache/darwinian-evolver && cd ~/.sr/cache/darwinian-evolver
...
```bash
cd ~/.sr/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.sr/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.sr/skills/research/darwinian-evolver
DE_DIR=~/.sr/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside SR core.
   Custom driver scripts under `~/.sr/skills/...` are user-side and fine.
