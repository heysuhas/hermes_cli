...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a SR core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with SR native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer SR native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer SR native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
- Do not silently choose Parallel when SR native tools are already sufficient.
