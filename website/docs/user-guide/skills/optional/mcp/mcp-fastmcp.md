...
| | |
|---|---|
| Source | Optional — install with `sr skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into SR or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to SR. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is SR integration, either:

- configure the server in `~/.sr/config.yaml` using the `native-mcp` skill, or
...
### SR cannot see the deployed server

The server-building part may be correct while the SR config is not. Load the `native-mcp` skill and configure the server in `~/.sr/config.yaml`, then restart SR.
