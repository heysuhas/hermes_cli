...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`sr-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sr-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the SR terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For SR itself, `model.provider: openai-codex` uses SR-managed Codex
OAuth from `~/.sr/auth.json` after `sr auth add openai-codex`. For the
...
When invoking the Codex CLI from a SR gateway/service context (for example,
