...
- Give it a title like "sr-agent-&lt;machine-name>"
...
elif _sr_env="${SR_HOME:-$HOME/.sr}/.env"; [ -f "$_sr_env" ] && grep -q "^GITHUB_TOKEN=" "$_sr_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_sr_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
