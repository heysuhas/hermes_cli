...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _sr_env="${SR_HOME:-$HOME/.sr}/.env"; [ -f "$_sr_env" ] && grep -q "^GITHUB_TOKEN=" "$_sr_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_sr_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from SR Agent\",
    \"comments\": [
...
```bash
source "${SR_HOME:-$HOME/.sr}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by SR Agent. Code looks clean — good test coverage, no security concerns."
