...
Connect SR to **iMessage** through [Photon][photon], a managed
...
your first iMessage from SR — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, SR runs it in a
...
sr photon setup --phone +15551234567
```
...
2. **Finds or creates** the `SR Agent` project on your account.
...
Runtime credentials are written to `~/.sr/.env`
...
(device token, dashboard project id) lives in `~/.sr/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other SR
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, SR replies with a pairing code. Approve it with:

```bash
sr pairing approve photon <CODE>
```

Use `sr pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.sr/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.sr/.env`):
...
By default SR responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `SR` and
`@SR agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and SR will reply.
...
assigned iMessage line SR uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run sr photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `sr photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `sr photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `sr photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** SR sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | SR wake words  | JSON list / comma / newline regex patterns for group mentions |
