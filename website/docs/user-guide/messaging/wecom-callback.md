...
Connect SR to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
SR supports two WeCom integration modes:
...
> Run `sr gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. SR decrypts the message, queues it for the agent
...
(Use `sr gateway start` only after `sr gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in SR and the token the
...
in `~/.sr/.env` values around `=` will also break signature checks. After
fixing, restart `sr gateway run`.
...
Check `sr gateway run` logs for the bound host/port. If the adapter bound to
