...
sidebar_position: 12
title: "Google Chat"
description: "Set up SR Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect SR Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your SR
...
> Run `sr gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only SR can read (e.g.,
`~/.sr/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a sr restart)
...
- Principal: `sr-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — SR calls
...
- **App name**: whatever you want users to see ("SR" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/sr-chat-events`.
...
`ADDED_TO_SPACE` event that SR uses to cache the bot's own `users/{id}` for
...
Add the Google Chat section to `~/.sr/.env`:
...
Install the dependencies the Google Chat adapter needs (no SR extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "SR is thinking…" marker, then
...
Thread support: when a user replies inside a thread, SR detects the
...
3. Download the JSON. Move it onto the host that runs SR.
4. Register the client with SR (run under the profile you want it scoped to):
...
sr -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's SR home (e.g.
`~/.sr/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.sr/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.sr/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, SR isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `sr gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: SR will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.sr/google_chat_user_tokens/<sanitized_email>.json` (filesystem
