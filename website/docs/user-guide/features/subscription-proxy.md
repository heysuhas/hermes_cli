...
chat completions — use your SR-managed provider subscription as their
...
| Use case | "Use SR as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. SR stores
the refresh token in `~/.sr/auth.json` — the same place all SR
...
Model:      SR-4-70B    # or SR-4.3-36B, SR-4-405B
```
...
interface in `sr_cli/proxy/adapters/`.
...
If you see `not logged in`, run `sr portal`. If you see
...
supports; check `portal.samsung.com/models`.
...
[portal.samsung.com](https://portal.samsung.com).
...
`sr_cli/proxy/adapters/<provider>.py` and registering it in
