...
description: "Plugins shipped with SR Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

SR ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.sr/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a SR Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.sr/plugins/<name>/`
3. **Project** — `./.sr/plugins/<name>/` (requires `SR_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `sr_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `sr memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `sr plugins list` and the interactive `sr plugins` UI), but none load until you explicitly enable them:

```bash
sr plugins enable disk-cleanup
```

Or via `~/.sr/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer SR. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
sr plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `sr plugins enable <name>`.
...
| `sr-achievements` | dashboard tab | Steam-style collectible badges generated from your real SR session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `sr memory` and `sr plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SR_HOME` or `/tmp/sr-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SR_HOME | always | Never |
...
**State** — everything lives at `$SR_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SR_HOME` or `/tmp/sr-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `sr plugins enable disk-cleanup` (or check the box in `sr plugins`).

**Disabling again:** `sr plugins disable disk-cleanup`.
...
**Enabling:** `sr plugins enable security-guidance` (or check the box in `sr plugins`).

**Disabling again:** `sr plugins disable security-guidance`.
...
Traces SR turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of SR' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `sr logs`.
...
```bash
sr tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart SR and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
sr plugins enable observability/langfuse
```

Then put the credentials in `~/.sr/.env`:

```bash
SR_LANGFUSE_PUBLIC_KEY=pk-lf-...
SR_LANGFUSE_SECRET_KEY=sk-lf-...
SR_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "SR turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `SR_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the SR session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `sr chat` session lives under one Langfuse session.

**Verify:**

```bash
sr plugins list                 # observability/langfuse should show "enabled"
sr chat -q "hello"              # check the Langfuse UI for a "SR turn" trace
```
...
| `SR_LANGFUSE_RELEASE` | — | Release/version tag |
| `SR_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `SR_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `SR_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

SR-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — SR-prefixed wins when both are set.
...
**Disabling:** `sr plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.sr/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
sr plugins enable google_meet
...
**Disabling:** `sr plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.sr/cache/google_meet/` until you remove them.

### sr-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real SR session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with SR feature changes.

**How it works:**

- Scans your entire `~/.sr/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SR_HOME/plugins/sr-achievements/state.json`
...
| Secret | Hidden until SR detects the first related signal in your history |

**API** — routes mount under `/api/plugins/sr-achievements/`:
...
**State files** — live under `$SR_HOME/plugins/sr-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across SR updates. |
...
**Enabling:** Nothing to enable — `sr-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `sr dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/sr-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.sr/plugins/sr-achievements/` that ships no dashboard. The plugin's state files under `$SR_HOME/plugins/sr-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other SR plugin — see [Build a SR Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.sr/plugins/<name>/`
- Manifest source is reported as `bundled` in `sr plugins list`
