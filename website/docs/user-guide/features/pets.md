...
SR can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<SR_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), SR renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `sr pets list [query] [--limit N]` |
| List installed pets | `sr pets list --installed` |
| Install a pet | `sr pets install <slug> [--select] [--force]` |
...
| Preview/animate | `sr pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `sr pets off` |
| Remove an installed pet | `sr pets remove <slug>` |
...
Beyond installing pre-made pets from the gallery, SR can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `sr pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `sr tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `SR_PET_IMAGE_PROVIDER` env var (e.g. `SR_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while SR is
