...
description: "Customize SR Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

SR Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SR_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who SR is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

SR now seeds a default `SOUL.md` automatically in:

```text
~/.sr/SOUL.md
```

More precisely, it uses the current instance's `SR_HOME`, so if you run SR with a custom home directory, it will use:
...
- SR creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- SR loads `SOUL.md` only from `SR_HOME`
- SR does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, SR falls back to a built-in default identity
...
If SR loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SR_HOME`, the personality belongs to the SR instance itself.

That also makes it easier to teach users:
- "Edit `~/.sr/SOUL.md` to change SR' default personality."
...
- how SR should handle uncertainty, disagreement, or ambiguity
...
## What SR injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, SR falls back to a built-in default identity ("You are SR Agent, an intelligent AI assistant created by Samsung Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
SR ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain SR, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives SR its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.sr/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.sr/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-sr)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how SR speaks
- `display.skin` and `/skin` affect how SR looks in the terminal
