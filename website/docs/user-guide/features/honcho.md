...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of SR's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple SR instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
sr memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `sr` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — SR syncs them back at session init.
...
The `sr honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `sr memory setup honcho` (or run `sr memory setup` and pick it from the list); the `sr honcho` subcommand then appears on the next invocation.

```bash
sr memory setup honcho    # Configure Honcho directly (works before activation)
sr honcho status          # Connection status, config, and key settings
sr honcho setup           # Redirects to `sr memory setup` (post-activation alias)
sr honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
sr honcho peer            # Show or update peer names + dialectic reasoning level
sr honcho mode            # Show or set recall mode (hybrid/context/tools)
sr honcho tokens          # Show or set token budget for context and dialectic
sr honcho identity        # Seed or show the AI peer's Honcho identity
sr honcho sync            # Sync Honcho config to all existing profiles
sr honcho peers           # Show peer identities across all profiles
sr honcho sessions        # List known Honcho session mappings
sr honcho map             # Map current directory to a Honcho session name
sr honcho enable          # Enable Honcho for the active profile
sr honcho disable         # Disable Honcho for the active profile
sr honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `sr honcho`

If you previously used the standalone `sr honcho setup`:
...
No re-login or re-setup needed. Run `sr memory setup` and select "honcho" — the wizard detects your existing config.
