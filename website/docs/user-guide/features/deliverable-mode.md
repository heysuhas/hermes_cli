...
When SR Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.sr/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.sr/config.yaml` (switchable per session
via `/personality`).
...
If you use SR' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.sr/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. SR Agent's deliverable
