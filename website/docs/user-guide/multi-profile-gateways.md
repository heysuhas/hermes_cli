...
If you only run one SR agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more SR agents that should all
...
(`ai.sr.gateway-<name>.plist`) or systemd user service
(`sr-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
sr profile create coder
sr profile create personal-bot
sr profile create research
...
```bash
sr config set gateway.multiplex_profiles true
sr gateway restart
```

Equivalently, in the default profile's `~/.sr/config.yaml`:
...
You do **not** run `sr gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `sr gateway start` / `run` is a
...
home). `sr status` reports the multiplexer and the profiles it serves;
`sr status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/sr-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: sr-gateways {start|stop|restart|status|list}"
}
...
```bash
sr-gateways start      # start every configured profile
sr-gateways stop       # stop every configured profile
sr-gateways restart    # restart all
sr-gateways status     # status across all
sr-gateways list       # delegates to `sr gateway list`
```

:::tip
The `default` profile is targeted with `sr gateway <action>` (no `-p`),
not `sr -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `sr -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.sr.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/sr-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.sr.gateway.plist` /
`sr-gateway.service`.
...
```bash
# Default profile
tail -f ~/.sr/logs/gateway.log
tail -f ~/.sr/logs/gateway.error.log

# Named profile
tail -f ~/.sr/profiles/<name>/logs/gateway.log
tail -f ~/.sr/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.sr/logs/gateway.log ~/.sr/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
sr logs -f                  # follow default profile
sr -p coder logs -f         # follow one profile
sr logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
sr profile list             # profiles + model + gateway state
sr-gateways status          # full status across every profile
launchctl list | grep sr    # macOS — PIDs and labels
systemctl --user list-units 'sr-gateway-*'   # Linux — units
```
...
The default profile uses `~/.sr/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
sr config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.sr/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=sr --why="gateways running" \
  sleep infinity &
...
`sr-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.sr/.env ~/.sr/profiles/*/.env
```

## Updating the code

`sr update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `sr gateway start` after a previous `sr gateway stop`. The
...
```bash
ps -ef | grep "sr_cli.*-p <profile>"
cat ~/.sr/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.sr.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.sr.gateway-<profile>.plist

# Linux
systemctl --user restart sr-gateway-<profile>.service
```

### Health check

```bash
sr doctor                  # default profile
sr -p <profile> doctor     # one profile
```
