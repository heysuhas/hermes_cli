...
`~/.sr/config.yaml`, `~/.sr/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/sr`:

```text
/etc/sr/
├── config.yaml     # managed config layer (wins over ~/.sr/config.yaml)
└── .env            # managed env layer (wins over ~/.sr/.env + shell)
```
...
The location can be relocated with the `SR_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `SR_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by SR.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export SR_MANAGED_DIR=/opt/org/sr-policy
```

:::warning
A user who can set `SR_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `sr doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/sr/config.yaml` (managed) | `/etc/sr/.env` (managed) |
| 2 | `~/.sr/config.yaml` (user) | `~/.sr/.env` (user) |
...
```bash
sr config        # shows a header naming the managed source + the pinned keys
sr doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, SR refuses and names the source:

```bash
$ sr config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/sr/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `sr config set` / setup will not write
...
sudo tee /etc/sr/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/sr/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/sr
sudo chmod 0644 /etc/sr/config.yaml /etc/sr/.env
```

Changes take effect on the next SR start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`sr doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs SR as `root`), managed scope is advisory.
