...
title: "Git Worktrees"
description: "Run multiple SR agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

SR Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with SR so each session has a clean, isolated working directory.

## Why Use Worktrees with SR?

SR treats the **current working directory** as the project root:

- CLI: the directory where you run `sr` or `sr chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.sr/config.yaml`
...
git worktree add ../repo-feature feature/sr-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sr-experiment` checked out in that directory

Now you can `cd` into the new worktree and run SR there:

```bash
cd ../repo-feature

# Start SR in the worktree
sr
```
...
git worktree add ../repo-experiment-a feature/sr-a
git worktree add ../repo-experiment-b feature/sr-b
```
...
- Works on its own branch (`feature/sr-a` vs `feature/sr-b`).
...
- SR checkpoint data under `~/.sr/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per SR experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sr-checkpoints-docs`, `feature/sr-refactor-tests`.
- **Commit frequently**
...
- **Avoid running SR from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sr -w` (Automatic Worktree Mode)

SR has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sr/sr-<hash>`).
...
```bash
sr -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `sr -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each SR session its own clean checkout.
