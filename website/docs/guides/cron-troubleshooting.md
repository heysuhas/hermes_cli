...
description: "Diagnose and fix common SR cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`sr gateway` for foreground, or `sr gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `sr cron tick`.
...
```bash
date
sr cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.sr/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.sr/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.sr/.env` |
...
| `local` | Write access to `~/.sr/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `sr cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `sr skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `sr skills list`.
...
2. `~/.sr/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `sr cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the SR config directory). Verify:
```bash
ls ~/.sr/scripts/your-script.py   # Must exist
sr cron edit <job_id> --script ~/.sr/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `sr skills install <skill-name>`.
...
Jobs are stored in `~/.sr/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.sr/cron/jobs.json
chmod 600 ~/.sr/cron/jobs.json   # Your user should own it
```
...
```bash
sr cron list                    # Show all jobs, states, next_run times
sr cron run <job_id>            # Schedule for next tick (for testing)
sr cron edit <job_id>           # Fix configuration issues
sr logs                         # View recent SR logs
sr skills list                  # Verify installed skills
```
...
1. Run the job with `sr cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.sr/logs/agent.log` for scheduler messages and `~/.sr/logs/errors.log` for warnings
3. Open an issue at [github.com/SamsungResearch/sr-agent](https://github.com/SamsungResearch/sr-agent) with:
