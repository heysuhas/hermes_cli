...
```bash
sr teams-pipeline token-health
sr teams-pipeline token-health --force-refresh
```
...
```bash
sr teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
sr teams-pipeline maintain-subscriptions
sr teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: SR cron (recommended if you already run the SR gateway)

SR ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.sr/scripts/`. First create the script:

```bash
mkdir -p ~/.sr/scripts
cat > ~/.sr/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec sr teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.sr/scripts/maintain-teams-subscriptions.sh
```
...
Create `/etc/systemd/system/sr-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=SR Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=sr
EnvironmentFile=/etc/sr/env
ExecStart=/usr/local/bin/sr teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/sr-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run SR Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now sr-teams-pipeline-maintain.timer
systemctl list-timers sr-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/sr teams-pipeline maintain-subscriptions >> /var/log/sr/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.sr/.env` at the top of a wrapper script that crontab calls.
...
```bash
sr teams-pipeline subscriptions   # should show expirationDateTime advanced
sr teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
sr teams-pipeline list
sr teams-pipeline list --status failed
sr teams-pipeline show <job-id>
```

### Replay a stored job

```bash
sr teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
sr teams-pipeline fetch --meeting-id <meeting-id>
sr teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `sr teams-pipeline maintain-subscriptions --dry-run`
- inspect `sr teams-pipeline list --status failed`
...
- run `sr teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `sr teams-pipeline run`
...
- [ ] `sr teams-pipeline validate` returns an OK snapshot
- [ ] `sr teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (SR cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
