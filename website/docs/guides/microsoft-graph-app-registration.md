...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.sr/.env` at the end.
...
4. Fill in:
   - **Name:** `SR Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `sr-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.sr/.env`:
...
SR ships a Graph auth smoke-test. From your SR install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.sr/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `sr gateway restart`.
