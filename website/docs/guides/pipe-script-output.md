...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `sr send`."
---

# Pipe Script Output to Messaging Platforms

`sr send` is a small, scriptable CLI that pushes a message to any
messaging platform SR is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | sr send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `sr
...
sr send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
sr send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
sr send --list

# Filter by platform
sr send --list telegram
```
...
Any platform SR ships adapters for works as a target:
...
`sr send` resolves the message body in this order:

1. **Positional argument** — `sr send --to telegram "hi"`
2. **`--file PATH`** — `sr send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | sr send --to telegram`

When stdin is a TTY (no pipe), SR does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  sr send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `sr send` reuses your SR config, the same script works on
any host where SR is installed — no need to export bot tokens into
...
`sr send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  sr send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | sr send \
...
```bash
./train.py --epochs 200 && \
  sr send --to telegram "training done" || \
  sr send --to telegram "training failed (exit $?)"
```
...
sr send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(sr send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `sr send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `sr send` calls
the platform's REST endpoint directly using credentials from
`~/.sr/.env` and `~/.sr/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `sr gateway start` and retry.
...
The listing is built from `~/.sr/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`sr gateway
...
| Approach | Multi-platform | Reuses SR creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `sr send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`sr send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `sr send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `sr send` shares with cron delivery.
