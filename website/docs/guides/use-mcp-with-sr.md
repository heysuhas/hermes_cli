---
sidebar_position: 6
title: "Use MCP with SR"
description: "A practical guide to connecting MCP servers to SR Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with SR

This guide shows how to actually use MCP with SR Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native SR tool
- you want SR to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect SR to internal APIs, databases, or company systems without modifying SR core

Do not use MCP when:
- a built-in SR tool already solves the job well
...
- SR remains the agent
- MCP servers contribute tools
- SR discovers those tools at startup or reload time
...
If you installed SR with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- SR banner/status should show MCP integration when configured
- ask SR what tools it has available
...
## WSL2: bridge SR in WSL to Windows Chrome
...
In this setup, SR does **not** connect to Chrome directly. Instead:

- SR runs in WSL
- SR starts a local stdio MCP server
...
```text
SR (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- SR stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on SR core browser transport
...
```bash
sr mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
sr mcp test chrome-devtools-win
```

Then start a fresh SR session or run:
...
Once loaded, SR can use the MCP-prefixed browser tools directly. For example:
...
If SR runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start SR from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start SR from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in SR:
...
So SR will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want SR to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want SR to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. SR remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
sr mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
sr mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `sr mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want SR to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize SR to merge, publish, deploy, or spawn runtimes.
...
Now SR can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing SR core.
...
Because SR now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
