---
sidebar_position: 8
title: "Use Voice Mode with SR"
description: "A practical guide to setting up and using SR voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with SR
...
- you want SR sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in SR.
...
## Step 1: make sure normal SR works first
...
SR supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, SR checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
- asking SR to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full SR loop.

## Tuning CLI behavior

### Silence threshold

If SR starts/stops too aggressively, tune:
...
SR stays a normal chat bot, but can speak replies.
...
- you want SR to function like a portable research or ops assistant
...
SR joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- SR detects speech boundaries
- transcripts are posted in the associated text channel
- SR responds in text and audio
...
1. get text SR working
2. install `sr-agent[voice]`
