...
description: "How SR stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `sr_cli/cron.py` | CLI `sr cron` subcommands |
...
Jobs are stored in `~/.sr/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `sr cron` commands are run or during active CLI sessions.
...
```python
# ~/.sr/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.sr/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `sr cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `sr cron` CLI provides direct job management:

```bash
sr cron list                    # Show all jobs
sr cron create                  # Interactive job creation (alias: add)
sr cron edit <job_id>           # Edit job configuration
sr cron pause <job_id>          # Pause a running job
sr cron resume <job_id>         # Resume a paused job
sr cron run <job_id>            # Trigger immediate execution
sr cron remove <job_id>         # Delete a job
```
