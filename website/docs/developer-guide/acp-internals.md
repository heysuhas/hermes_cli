...
The ACP adapter wraps SR' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
sr acp / sr-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.sr/.env
  -> configure stderr logging
  -> construct SRACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'sr-agent[acp]==<version>' sr-acp`, pointed at the `sr-agent` PyPI release.
...
- `allow_once` -> SR `once`
- `allow_always` -> SR `always`
- reject options -> SR `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps SR tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["sr-acp"])
...
Instead it reuses SR' runtime resolver:

- `acp_adapter/auth.py`
- `sr_cli/runtime_provider.py`

So ACP advertises and uses the currently configured SR provider/credentials. It also always advertises a terminal setup auth method (`sr-setup`, args `--setup`) so first-run registry clients can open SR' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.sr/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `sr-acp` toolset definition
- `sr_cli/main.py` — `sr acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `sr-acp` script
