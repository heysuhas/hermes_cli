...
sidebar_position: 1
title: "Architecture"
description: "SR Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of SR Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ sr_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # SRCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── sr_state.py           # SQLite session/state database with FTS5
├── sr_constants.py       # SR_HOME, profile-aware paths
...
│
├── sr_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `sr` subcommands (large file)
...
│   ├── skills_config.py      # sr skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # sr tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # sr gateway start/stop
│
...
```text
User input → SRCLI.process_input()
...
Three discovery sources: `~/.sr/plugins/` (user), `.sr/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `sr plugins` or `config.yaml`.
...
Exposes SR as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sr -p <name>`) gets its own SR_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
