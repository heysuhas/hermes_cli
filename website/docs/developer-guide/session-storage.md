...
SR Agent uses a SQLite database (`~/.sr/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `sr_state.py`


## Architecture Overview

```
~/.sr/state.db (SQLite, WAL mode)
...
Multiple sr processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.sr/state.db
...
Default path: `~/.sr/state.db`

This is derived from `sr_constants.get_sr_home()` which resolves to
`~/.sr/` by default, or the value of `SR_HOME` environment variable.
