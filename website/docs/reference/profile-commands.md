...
This page covers all commands related to [SR profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `sr profile` without a subcommand shows help.
...
Sets `<name>` as the active profile. All subsequent `sr` commands (without `-p`) will use this profile.
...
```bash
sr profile use work
sr profile use default
```

## `sr profile create`

```bash
sr profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `sr profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `sr update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.sr`), use `sr skills opt-out` / `sr skills opt-in`. |
...
sr profile create work2 --clone-from work

# Clone everything from a specific profile
sr profile create work2-backup --clone-from work --clone-all
```

## `sr profile describe`

```bash
sr profile describe [<name>] [options]
```
...
sr profile describe researcher

# Set it explicitly
sr profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
sr profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
sr profile describe --all --auto
```

## `sr profile delete`

```bash
sr profile delete <name> [options]
```
...
```bash
sr profile delete mybot
sr profile delete mybot --yes
```
...
```bash
sr profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's SR home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ sr profile show work
Profile: work
Path:    ~/.sr/profiles/work
...
```bash
sr profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your SR installation.
...
```bash
sr profile alias work
# Creates/updates ~/.local/bin/work

sr profile alias work --name mywork
# Creates ~/.local/bin/mywork

sr profile alias work --remove
...
```bash
sr profile rename <old-name> <new-name>
```
...
```bash
sr profile rename mybot assistant
# ~/.sr/profiles/mybot → ~/.sr/profiles/assistant
...
```bash
sr profile export <name> [options]
```
...
```bash
sr profile export work
# Creates work.tar.gz in the current directory

sr profile export work -o ./work-2026-03-29.tar.gz
```

## `sr profile import`

```bash
sr profile import <archive> [options]
```
...
```bash
sr profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

sr profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`sr profile export` / `import` are still the right commands for
...
```bash
sr profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `sr -p telemetry`). |
...
sr profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
sr profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
sr profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
sr profile install ./telemetry/
```

### `sr profile update`

```bash
sr profile update <name> [--force-config] [--yes]
```
...
```bash
sr profile info <name>
```

Prints the profile's distribution manifest — name, version, required
SR version, author, env var requirements, the source URL/path, and
...
`sr profile list` also shows the distribution name and version in a
`Distribution` column, and `sr profile show <name>` / `delete <name>`
...
sr profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
sr profile install https://github.com/your-org/internal-assistant.git
```
...
`sr_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
SR version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host SR can clone from.
3. Tell recipients to run `sr profile install <your-repo-url>`.
...
```bash
sr -p <name> <command> [options]
sr --profile <name> <command> [options]
```

Global flag to run any SR command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
sr -p work chat -q "Check the server status"
sr --profile dev gateway start
sr -p personal skills list
sr -p work config edit
```
...
- `sr profile <TAB>` — subcommands (list, use, create, etc.)
- `sr profile use <TAB>` — profile names
- `sr -p <TAB>` — profile names
