...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def show_banner(self):
        ...
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Hermes 3 & 4 base models are NOT agentic and are not "
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow charging your card."
        )
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize charging {card.masked} "
    ...
    def _approval_callback(self, command: str, description: str,
        ...
        with self._approval_lock:
            ...
            if command.startswith("Local folder access:"):
                _cprint(
                    f"\n{_ACCENT}  🔐 Local folder approval required — "
                    f"choose 1–4 in the approval panel below.{_RST}"
                )
    ...
    def _get_approval_display_fragments(self):
        ...
        is_path_access = command.startswith("Local folder access:")
        title = "🔐  Local Folder Access" if is_path_access else "⚠️  Dangerous Command"
        cmd_display = command
        choice_labels = {
            "once": "Allow once",
