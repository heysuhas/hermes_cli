...
class AIAgent:
    ...
    def _looks_like_unexecuted_tool_action(
        self,
        assistant_content: str,
        messages: List[Dict[str, Any]],
        current_turn_user_idx: int,
    ) -> bool:
        """Forwarder for generic missing-tool recovery detection."""
        from agent.agent_runtime_helpers import looks_like_unexecuted_tool_action
        return looks_like_unexecuted_tool_action(
            self,
            assistant_content,
            messages,
    ...
    def _select_recovery_tool(self, user_message: str) -> Optional[str]:
        """Choose a clearly matching tool from the live schema list."""
        from agent.agent_runtime_helpers import select_recovery_tool
        return select_recovery_tool(self.tools or [], user_message)

    @staticmethod
    def _looks_like_false_tool_capability_denial(
        assistant_content: str,
    ) -> bool:
        from agent.agent_runtime_helpers import looks_like_false_tool_capability_denial
        return looks_like_false_tool_capability_denial(assistant_content)
