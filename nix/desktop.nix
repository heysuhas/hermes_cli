# nix/desktop.nix — SR Desktop (Electron) app build + wrapper
#
# `srAgent` is the fully-built `.#default` package — it ships the
# `sr` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `SR_DESKTOP_SR` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing SR CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "sr-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "sr-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "sr-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/sr-desktop $out/bin
    cp -r ${renderer}/* $out/share/sr-desktop/
    ...
    substituteInPlace $out/share/sr-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/sr-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # SR_DESKTOP_SR to the absolute path of the nix-built `sr`
    # binary so the desktop's resolver step 4 ("existing SR CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/sr-desktop \
      --add-flags "$out/share/sr-desktop" \
      --set SR_DESKTOP_SR "${lib.getExe srAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for SR Agent";
    homepage = "https://github.com/SamsungResearch/sr-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "sr-desktop";
  };
}
