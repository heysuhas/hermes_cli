...
# container recreation. Environment variables are written to $SR_HOME/.env
# and read by sr at startup — no container recreation needed for env changes.
#
# Tool resolution: the sr wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    sr-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-sr-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "sr-config-attrs";
      description = "SR YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "sr-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # SR_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "sr-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SR_DOC_EOF'\n${value}\nSR_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "sr-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/sr";
    ...
    # sr user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "sr-container-entrypoint" ''
      set -eu

      SR_UID="''${SR_UID:?SR_UID must be set}"
      SR_GID="''${SR_GID:?SR_GID must be set}"

      # ── Group: ensure a group with GID=$SR_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SR_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="sr"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$SR_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$SR_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SR_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SR_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$SR_UID" -g "$SR_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$SR_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SR_UID:$SR_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SR_HOME is owned by the target user.
      ...
      if [ -n "''${SR_HOME:-}" ] && [ -d "$SR_HOME" ]; then
        find "$SR_HOME" \! -user "$SR_UID" -exec chown "$SR_UID:$SR_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/sr-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/sr-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/sr ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/sr
        chmod 0440 /etc/sudoers.d/sr
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SR_UID" --regid="$SR_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SR_HOME/.env.
    ...
    # Default: /var/lib/sr/workspace → /data/workspace.
  ...
  in {
    options.services.sr-agent = with lib; {
      enable = mkEnableOption "SR Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = sr-agent;
        description = "The sr-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/sr";
        description = "State directory. Contains .sr/ subdir (SR_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative SR config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SR_HOME/.env at activation time.
          SR reads this file on every startup via load_sr_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SR_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SR_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `sr gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the sr user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the sr plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. SR discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          sr_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as sr (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in sr-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the sr CLI to environment.systemPackages and export
          SR_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.sr symlink to the service
            stateDir. These users are automatically added to the sr group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.sr-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the sr CLI to system PATH and export SR_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.sr/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.SR_HOME = "${cfg.stateDir}/.sr";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.sr-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the sr user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.sr-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed sr
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure sr is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.sr        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sr/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sr/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sr/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sr/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sr/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."sr-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.sr
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.sr ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.sr ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.sr -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.sr/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.sr/$_subdir"
            chmod 2770 "${cfg.stateDir}/.sr/$_subdir"
            find "${cfg.stateDir}/.sr/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # sr-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.sr/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.sr/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sr/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.sr/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.sr/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sr/.managed
          chmod 0644 ${cfg.stateDir}/.sr/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.sr/.container-mode <<'SR_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    sr_bin=${containerDataDir}/current-package/bin/sr
    SR_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sr/.container-mode
            chmod 0644 ${cfg.stateDir}/.sr/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.sr/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.sr";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.sr" ]; then
                  rm -f "${symlinkPath}"
                  echo "sr-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.sr -> stateDir/.sr for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.sr";
                target = "${cfg.stateDir}/.sr";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "sr-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sr/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.sr/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sr/auth.json
              fi
            ''}
          ''}
          ...
          # SR reads $SR_HOME/.env at startup via load_sr_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.sr/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SR_NIX_ENV_EOF'
    ${envFileContent}
    SR_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.sr/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.sr/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sr/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.sr-agent = {
          description = "SR Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $SR_HOME/.env by the activation script. load_sr_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/sr"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the sr group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.sr-agent = {
          description = "SR Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SR_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SR_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env SR_UID="$SR_UID" \
                --env SR_GID="$SR_GID" \
                --env SR_HOME=${containerDataDir}/.sr \
                --env SR_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/sr gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
