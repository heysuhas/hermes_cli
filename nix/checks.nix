...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      sr-agent = self'.packages.default;
      srVenv = sr-agent.srVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "sr-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${srVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "sr-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "sr-build-package" { } ''
          echo "PASS: package built at ${sr-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "sr-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "sr-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${sr-agent}/bin/sr || (echo "FAIL: sr binary missing"; exit 1)
          test -x ${sr-agent}/bin/sr-agent || (echo "FAIL: sr-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${sr-agent}/bin/sr version 2>&1 | grep -qi "sr" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "sr-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in sr sr-agent sr-acp; do
            test -x ${sr-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "sr-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking sr --help ==="
          ${sr-agent}/bin/sr --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${sr-agent}/bin/sr --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "sr-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${sr-agent}/share/sr-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${sr-agent}/share/sr-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SR_BUNDLED_SKILLS" ${sr-agent}/bin/sr || \
            (echo "FAIL: SR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SR_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "sr-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${sr-agent}/share/sr-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${sr-agent}/share/sr-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SR_BUNDLED_PLUGINS" ${sr-agent}/bin/sr || \
            (echo "FAIL: SR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "sr-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${sr-agent}/share/sr-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${sr-agent}/share/sr-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${sr-agent}/share/sr-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "SR_BUNDLED_LOCALES" ${sr-agent}/bin/sr || \
            (echo "FAIL: SR_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: SR_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (SR_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && SR_BUNDLED_LOCALES=${sr-agent}/share/sr-agent/locales \
            ${srVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with SR_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${srVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${srVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without SR_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "sr-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${sr-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${sr-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SR_TUI_DIR" ${sr-agent}/bin/sr || \
            (echo "FAIL: SR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SR_TUI_DIR set in wrapper"
        ...
        # Verify SR_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        sr-node = pkgs.runCommand "sr-node-version" { } ''
          set -e
          echo "=== Checking SR_NODE in wrapper ==="
          grep -q "SR_NODE" ${sr-agent}/bin/sr || \
            (echo "FAIL: SR_NODE not set in wrapper"; exit 1)
          echo "PASS: SR_NODE present in wrapper"

          SR_NODE=$(sed -n "s/^export SR_NODE='\(.*\)'/\1/p" ${sr-agent}/bin/sr)
          test -x "$SR_NODE" || (echo "FAIL: SR_NODE=$SR_NODE not executable"; exit 1)
          echo "PASS: SR_NODE executable at $SR_NODE"

          NODE_MAJOR=$("$SR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "sr-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SR_MANAGED guards ==="
          check_blocked "config set" ${sr-agent}/bin/sr config set model foo
          check_blocked "config edit" ${sr-agent}/bin/sr config edit
        ...
        extra-python-packages = let
          ...
          srWithExtra = sr-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "sr-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${srWithExtra}/bin/sr || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${srWithExtra}/bin/sr || \
          ...
          if grep -q "PYTHONPATH" ${sr-agent}/bin/sr; then
        ...
        extra-dependency-groups = let
          srWithGroups = sr-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "sr-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${srWithGroups}"
          echo "venv: ${srWithGroups.srVenv}"
        ...
        messaging-variant = pkgs.runCommand "sr-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.srVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "sr-config-roundtrip" {
          ...
          merge_and_load() {
            local sr_home="$1"
            export SR_HOME="$sr_home"
            ${configMergeScript} ${nixSettings} "$sr_home/config.yaml"
            ${srVenv}/bin/python3 -c '
import json, sys
