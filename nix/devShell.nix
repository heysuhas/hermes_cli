...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      srNpmLib = self'.packages.default.passthru.srNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. sr-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${srNpmLib.mkNpmDevShellHook npmPackageJsonPaths}

          # for the devshell to pick up the src
          export SR_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "SR Agent dev shell in $SR_PYTHON_SRC_ROOT"
          echo "Ready. Run 'sr' or 'sandbox sr' to start."
        '';
      };
    };
}
