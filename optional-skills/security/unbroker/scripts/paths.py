...
All per-subject data lives under PDD_DATA_DIR (default: $SR_HOME/unbroker),
which is the same trust boundary SR uses for .env and OAuth tokens.
"""
...
def sr_home() -> Path:
    return Path(os.environ.get("SR_HOME") or (Path.home() / ".sr"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else sr_home() / "unbroker"
