...
The design is **SR-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
sr skills install official/security/unbroker
```

Then start a new SR session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the SR home directory (`0600`, optionally
