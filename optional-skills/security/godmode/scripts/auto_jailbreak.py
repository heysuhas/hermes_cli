...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SR_HOME", os.path.expanduser("~/.sr")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SR_HOME", Path.home() / ".sr")) / "skills" / "red-teaming" / "godmode"
...
SR_HOME = Path(os.getenv("SR_HOME", Path.home() / ".sr"))
CONFIG_PATH = SR_HOME / "config.yaml"
PREFILL_PATH = SR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/SR models — already uncensored, just needs clean prompt
    "sr": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "sr" in model_lower or "nous" in model_lower:
        return "sr"
...
def _get_current_model() -> tuple:
    """Read current model and provider from SR config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sr/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart SR for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart SR for changes to take effect.")
