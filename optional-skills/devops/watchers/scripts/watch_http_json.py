...
Usage (via cron with --no-agent):
    ...
    sr cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SR_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "SR-Watcher/1.0"})
