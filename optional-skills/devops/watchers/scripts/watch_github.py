...
Usage (via cron with --no-agent):
    ...
    sr cron create sr-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SR_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name sr-issues --repo SamsungResearch/sr-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the SR .env file
(``${SR_HOME:-~/.sr}/.env``) to avoid the 60 req/hr
