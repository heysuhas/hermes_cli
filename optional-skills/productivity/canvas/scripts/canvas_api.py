#!/usr/bin/env python3
"""Canvas LMS API CLI for SR Agent.
...
def _check_config():
    ...
    if missing:
        sr_env = os.path.join(
            os.environ.get("SR_HOME", os.path.expanduser("~/.sr")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {sr_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for SR Agent"
    )
