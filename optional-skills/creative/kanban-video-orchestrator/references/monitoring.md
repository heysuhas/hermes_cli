...
sr kanban watch --tenant <project-slug>

# Snapshot of the board
sr kanban list --tenant <project-slug>
sr kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
sr kanban stats --tenant <project-slug>

# Visual dashboard (browser)
sr dashboard

# Inspect a specific task (includes comments + events)
sr kanban show <task-id>

# Follow a single task's event stream
sr kanban tail <task-id>
```

Verify available subcommands with `sr kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `sr kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `sr kanban block <id>` then `sr kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `sr kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `sr kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `sr profile list` (profile exists?), `sr status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(sr kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
sr kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
sr kanban block <task-id>
sr kanban archive <task-id>

# Diagnose what happened
sr kanban show <task-id>      # task body, comments, recent events
sr kanban tail <task-id>      # follow the live event stream
sr kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `sr kanban create` for the director
...
while true; do
    clear
    sr kanban list --tenant <slug>
    echo "---"
    sr kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `sr kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `sr profile list` and start
