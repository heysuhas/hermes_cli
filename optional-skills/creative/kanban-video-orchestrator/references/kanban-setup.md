...
the actual `setup.sh` that creates the project workspace, configures SR
...
> [SamsungResearch/kanban-video-pipeline](https://github.com/SamsungResearch/kanban-video-pipeline).
...
2. **Create profiles** — `sr profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.sr/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `sr kanban create` assigned to the director
...
```bash
sr profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.sr/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.sr/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most SR
...
Each profile gets a `SOUL.md` at `~/.sr/profiles/<name>/SOUL.md` that
...
```bash
sr kanban create "Direct production of <video title>" \
    --assignee director \
...
the SR `.env` (`${SR_HOME:-$HOME/.sr}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _sr_env="${SR_HOME:-$HOME/.sr}/.env"
    if grep -q "^${var}=" "$_sr_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_sr_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_sr_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY sr ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY sr OPENROUTER_API_KEY || exit 1
# ...
```
