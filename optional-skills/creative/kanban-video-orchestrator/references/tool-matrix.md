...
Maps each role archetype to the SR skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public sr-agent
...
## SR skills relevant to video production

### Visual / rendering skills (`sr-agent/skills/creative/`)
...
### Design / pre-production skills (`sr-agent/skills/creative/`)
...
### Audio / media skills (`sr-agent/skills/creative/` + `skills/media/`)
...
These are **not** SR skills but external CLIs / APIs that profiles invoke.
...
## Built-in SR tools for media review

These are native SR tools — not invoked via terminal but through their own
...
key is present in `${SR_HOME:-~/.sr}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every SR profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${SR_HOME:-~/.sr}/.env` → environment variable.
...
When a new SR-public video skill ships:
