#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `sr kanban list` and
...
def sr_available() -> bool:
    return shutil.which("sr") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed sr CLI."""
    try:
        out = subprocess.run(
            ["sr", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `sr kanban list`
    out = subprocess.run(
        ["sr", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not sr_available():
        print("ERROR: 'sr' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
