...
$SR_OSINT_CACHE/icij/ (default: ~/.cache/sr-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("SR_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "sr-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "sr-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $SR_OSINT_CACHE/icij or ~/.cache/sr-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
