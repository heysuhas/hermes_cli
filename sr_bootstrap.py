"""Windows UTF-8 bootstrap for SR entry points.
...
should be imported at the very top of every SR entry point
(``sr``, ``sr-agent``, ``sr-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing SR modules.

    SR ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real SR source root
    ...
    """
    root = src_root or os.environ.get("SR_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``SR_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("SR_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import sr_bootstrap``
# (or ``from sr_bootstrap import apply_windows_utf8_bootstrap``) at
