# SR Agent Security Policy

This document describes SR Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/SamsungResearch/sr-agent/security/advisories/new)
or **security@samsung.com**. Do not open public issues for
security vulnerabilities. **SR Agent does not operate a bug
bounty program.**
...
- Environment details (`sr version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted SR Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in SR Agent's documentation
...
SR Agent supports two OS-level isolation postures. They address
...
SR Agent supports this in two ways:

- **SR Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, SR Agent's in-process heuristics
...
SR Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in SR Agent
itself. Bugs in SR Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   SR Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, SR Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  SR Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  sr-home security, and similar. Reports against those
...
- SR Agent includes supply-chain guards for MCP server
