...
export const $srHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.sr/logs/bootstrap-installer.log')
    $srHome.set('~/.sr')
  ...
  try {
    const [logPath, srHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_sr_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch SR" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (SR-Setup.exe --update);
...
export async function launchSRCorporate(): Promise<void> {
  ...
  await invoke('launch_sr_corporate', { installRoot })
}
