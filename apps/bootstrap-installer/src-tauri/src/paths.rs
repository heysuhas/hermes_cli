...
//!
//! Mirrors `sr_constants.get_sr_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\sr
//!   macOS:   ~/.sr
//!   Linux:   ~/.sr  (override via $SR_HOME)
//!
//! NOTE (macOS): Python's get_sr_home(), scripts/install.sh, and the
//! Electron desktop's resolveSRHome() ALL use ~/.sr on macOS — there
...
/// Returns the canonical SR home directory, respecting $SR_HOME if set.
pub fn sr_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("SR_HOME") {
    ...
    {
        // %LOCALAPPDATA%\sr — matches scripts/install.ps1's $SRHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("sr");
        }
    }

    // macOS + Linux + fallback: ~/.sr (matches Python get_sr_home(),
    // install.sh, and the Electron desktop's resolveSRHome()).
...
/// SR_HOME so it survives repo checkout deletion (unlike anything under
/// sr-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\sr\sr-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_sr`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under SR_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves SR_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    sr_home().join(".sr-update-in-progress")
}
...
pub fn copy_self_to_sr_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to SR_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_SR_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".sr-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under SR_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[sr-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("SR_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_sr_home() -> String {
    sr_home().to_string_lossy().into_owned()
}
