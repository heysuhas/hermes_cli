...
import { buildDesktopBackendEnv, normalizeSRHomeRoot } from './backend-env'
import { canImportSRCli, verifySRCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.SR_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.SR_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.SR_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with SR_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[sr] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[sr] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('sr:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[sr] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[sr] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[sr] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[sr] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// SR_HOME — the user-facing root for everything SR-related. Mirrors
// scripts/install.ps1's $SRHome and scripts/install.sh's $SR_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\sr (matches install.ps1)
//   macOS / Linux: ~/.sr (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.sr directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\sr yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// SR_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// SR_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.sr / %LOCALAPPDATA%\sr.
function resolveSRHome() {
  if (process.env.SR_HOME) {
    return normalizeSRHomeRoot(process.env.SR_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'sr-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a SR_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\sr and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('SR_HOME')

    if (fromRegistry) {
      return normalizeSRHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'sr')
    const legacy = path.join(app.getPath('home'), '.sr')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.sr setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.sr')
}

const SR_HOME = resolveSRHome()

function srManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_sr_node_dirs() in
  // sr_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(SR_HOME, 'node')
...
function pathWithSRManagedNode(...entries) {
  return [...srManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_SR_ROOT — the canonical mutable SR install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_SR_ROOT = path.join(SR_HOME, 'sr-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_SR_ROOT, 'venv')
...
// boots, going straight to `resolveSRBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_SR_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_SR_ROOT, '.sr-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which SR profile the desktop launches its
// local backend as. When set, startSR() passes `sr --profile <name>
// dashboard …`, which deterministically pins SR_HOME (see
// _apply_profile_override in sr_cli/main.py) and bypasses the sticky
// ~/.sr/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors sr_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// srDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under SR_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by sr_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(SR_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (sr_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.SR_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.SR_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.SR_DESKTOP_APP_NAME || 'SR'
...
// 'sr:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.sr) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.sr')
}

// Seed the native About panel with the live SR version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `sr update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSRVersion(),
  copyright: 'Copyright © 2026 Samsung Research'
})
...
const MEDIA_PROTOCOL = 'sr-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(sr-media://…)`.
...
// (the desktop's launch profile) stays managed by srProcess +
// connectionPromise + startSR(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.SR_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.SR_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSR() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning sr serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start SR backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (srLog.length > 300) {
    srLog.splice(0, srLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-sr-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('sr:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the sr:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('sr:bootstrap:event', ev)
}
...
// The Tauri updater writes SR_HOME/.sr-update-in-progress for the whole
...
// (`force_kill_other_sr`, taskkill /IM sr.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(SR_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — SR will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(SR_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `sr` — must not shadow `sr.cmd`/`sr.exe`.
...
function unwrapWindowsVenvSRCommand(command, backendArgs) {
  ...
  if (!/^sr(?:\.exe)?$/i.test(path.basename(resolved))) {
    return null
  }
  ...
  // died mid-`pip install` still has python.exe + sr.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv SR at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing SR Python at ${python}`,
    command: python,
    args: ['-m', 'sr_cli.main', ...backendArgs],
    bootstrap: false,
...
// spawns `sr serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `sr` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'sr_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, SR_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isSRSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'sr_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.SR_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several SR deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. SR' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\sr\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'sr', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'sr', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal SR_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\sr\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'sr', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'sr', 'git', 'bin', 'git.exe'))
  }
...
function recentSRLog() {
  return srLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's sr root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_SR_ROOT.
// SR_DESKTOP_SR_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.SR_DESKTOP_SR_ROOT && path.resolve(process.env.SR_DESKTOP_SR_ROOT),
    !IS_PACKAGED && isSRSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSRSourceRoot(ACTIVE_SR_ROOT) ? ACTIVE_SR_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_SR_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('sr:updates:progress', merged)
  }
}
...
// SR_HOME/sr-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_sr_home). That binary owns
// ALL repo mutation — running `sr update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'sr-setup.exe' : 'sr-setup'
  const candidate = path.join(SR_HOME, name)
...
// Path to the venv shim whose lock decides whether `sr update` can write
// fresh entry points. On Windows this is the file the running backend
// `sr.exe` holds open; on POSIX it's never mandatory-locked.
function venvSRShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'sr.exe')
    : path.join(updateRoot, 'venv', 'bin', 'sr')
}
...
// only signals the direct child, so on Windows a backend `sr.exe` that
// spawned its own grandchildren (a `sr` REPL, a pty terminal session, the
...
// `srProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `sr.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a sr
// REPL, a pty terminal, the gateway) keep `sr.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (srProcess && Number.isInteger(srProcess.pid)) {
    pids.push(srProcess.pid)
  }
  ...
  if (srProcess && !srProcess.killed) {
    try {
      srProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvSRShimPath(updateRoot)
...
// `sr update`). Instead we spawn the staged SR-Setup binary with
// --update and quit, so it can run `sr update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri sr-setup. Unlike Windows
      ...
      // whole update itself: `sr update` (backend) + `sr desktop
    ...
    if (!updater) {
      ...
      // `sr desktop`, never the Tauri installer that self-copies
      // sr-setup.exe into SR_HOME). They DO have a working `sr`
      ...
      // checkout they're on — bare `sr update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `sr update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `sr update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, srRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating SR — this window will close and the updater will open. Don’t reopen SR yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // sr.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running sr, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the SR install open ' +
        '(a second SR window or a terminal running sr?). Close it and retry.'
    ...
    // `sr update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: SR_HOME,
      env: {
        ...process.env,
        SR_HOME,
        PATH: pathWithSRManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(SR_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSR = path.join(venvBin, IS_WINDOWS ? 'sr.exe' : 'sr')
  ...
  // not just the `sr.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvSR) || fileExists(path.join(updateRoot, '.sr-bootstrap-complete'))
...
// Resolve the sr CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `sr` on PATH.
function resolveSRCliBinary(updateRoot) {
  const venvSR = path.join(updateRoot, 'venv', 'bin', 'sr')

  if (fileExists(venvSR)) {
    return venvSR
  }

  return findOnPath('sr') || null
}
...
// macOS/Linux in-app update: backend (`sr update`) + OS-aware GUI rebuild
// (`sr desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const sr = resolveSRCliBinary(updateRoot)

  if (!sr) {
    emitUpdateProgress({ stage: 'manual', message: 'sr update', percent: null })

    return { ok: true, manual: true, command: 'sr update', srRoot: updateRoot }
  }

  // Put the SR-managed Node and the venv on PATH so `sr desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\sr\node, not node\bin.
  const env: Record<string, string> = {
    SR_HOME,
    PATH: pathWithSRManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `sr update` reaps stale `sr serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads SR_DESKTOP_CHILD_PID and excludes
  ...
  if (srProcess && Number.isInteger(srProcess.pid)) {
    desktopChildPids.push(srProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.SR_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating SR (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(sr, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'sr update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'sr update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(sr, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart SR to retry.',
  ...
  // Linux in-app update terminal state (#45205). `sr desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting SR…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `sr-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen SR to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the SR desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen SR to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'SR.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'SR.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart SR to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.sr-update-new"; then
    rm -rf "$DST.sr-update-old" 2>/dev/null || true
    mv "$DST" "$DST.sr-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.sr-update-new" "$DST"
    rm -rf "$DST.sr-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart SR to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_SR_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isSRSourceRoot(ACTIVE_SR_ROOT) &&
    fileExists(venvPython) &&
    canImportSRCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_SR_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `sr
...
function resolveWebDist() {
  const override = process.env.SR_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set SR_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: sr desktop --force-build`
  )
...
function resolveSRCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/SR.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSRCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_SR_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `SR at ${ACTIVE_SR_ROOT}`,
    command,
    args: ['-m', 'sr_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      srHome: SR_HOME,
      pythonPathEntries: [ACTIVE_SR_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_SR_ROOT,
    bootstrap: true,
...
function resolveSRBackend(backendArgs) {
  // 1. Explicit override -- SR_DESKTOP_SR_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.SR_DESKTOP_SR_ROOT && path.resolve(process.env.SR_DESKTOP_SR_ROOT)

  if (overrideRoot && isSRSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `SR source at ${overrideRoot}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_SR_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\sr\sr-agent (Windows) or ~/.sr/sr-agent.
  ...
  //    to spawning sr. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `sr update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `sr` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    SR_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.SR_DESKTOP_IGNORE_EXISTING !== '1') {
    let srCommand = null
    const srOverride = process.env.SR_DESKTOP_SR

    if (srOverride) {
      const resolvedOverride = findOnPath(srOverride)

      if (resolvedOverride) {
        srCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(srOverride, { isWsl: IS_WSL })) {
        srCommand = srOverride
      } else {
        rememberLog(`Ignoring Windows SR override under WSL: ${srOverride}`)
      }
    } else {
      srCommand = findOnPath('sr')
    }

    if (srCommand) {
      if (looksLikeDesktopAppBinary(srCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving SR CLI: ${srCommand}`)
        srCommand = null
      }
    }

    if (srCommand) {
      const unwrapped = unwrapWindowsVenvSRCommand(srCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `sr` shim
      ...
      const shellForProbe = isCommandScript(srCommand)

      if (verifySRCli(srCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvSRCommand(srCommand, backendArgs) || {
            label: `existing SR CLI at ${srCommand}`,
            command: srCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
      ...
      rememberLog(
        `Ignoring existing SR CLI at ${srCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed sr_cli module via system Python.
  ...
  if (python) {
    ...
    // having sr_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\sr\sr-agent\venv.
    if (canImportSRCli(python)) {
      return {
        kind: 'python',
        label: `installed sr_cli module via ${python}`,
        command: python,
        args: ['-m', 'sr_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: sr_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSRBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'SR Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSRBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no SR install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'SR recovery was handed off to SR Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      srHome: SR_HOME,
      logRoot: path.join(SR_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('SR install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `SR bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(SR_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSR() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // sr:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSRBackend(backend.args))
  }
  ...
  if (!isSRSourceRoot(ACTIVE_SR_ROOT)) {
    throw new Error(
      `SR install at ${ACTIVE_SR_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. SR' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\sr\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `sr` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for SR on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isSRSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `SR venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `SR at ${ACTIVE_SR_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'SR runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported SR backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the SR backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to SR backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-SR-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('sr:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSRCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('sr:preview-file-changed', payload)
}
...
async function waitForSR(baseUrl, token) {
  ...
  throw new Error(`SR backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('sr:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('sr:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting SR...".
function sendPowerResume() {
  ...
  webContents.send('sr:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('sr:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('sr:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('sr:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
//
// Hosted SR gateways gate the dashboard behind an OAuth provider (e.g.
// Samsung Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``sr_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / sr #37247).
//     Both are set as HttpOnly cookies (``sr_session_at`` ~15 min,
//     ``sr_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:sr-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to SR Cloud agent…' : 'Sign in to SR gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to SR backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- SR Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:sr-remote-oauth`:
...
// convention (sr_cli/auth.py DEFAULT_SR_PORTAL_URL + the same env names)
// so a single override flips every SR surface to the same portal.
const DEFAULT_SR_PORTAL_URL = 'https://portal.samsung.com'

function resolvePortalBaseUrl() {
  const raw = process.env.SR_PORTAL_BASE_URL || process.env.SR_PORTAL_BASE_URL || DEFAULT_SR_PORTAL_URL
...
// authenticates via PRIVY, not the SR gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for sr_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a SR Cloud sign-in.'))
...
// Discover the hosted (SR Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to SR Cloud. Open Settings → Gateway, choose SR Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your SR Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your SR Cloud session has expired. Sign in to SR Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the SR Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root SR_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.SR_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.SR_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a SR Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted SR Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by SR_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the SR Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered SR Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (sr #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote SR gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote SR gateway is selected, but no session token is saved. ' +
...
//   2. env override (SR_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.SR_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.SR_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'SR_DESKTOP_REMOTE_URL is set but SR_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote SR backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.SR_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every SR
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Samsung Research" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSR())
  ...
  // connect to SR gateway". Mirror the renderer's connect here so the test
...
function resetSRConnection({ soft = false } = {}) {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(srProcess)
...
// startSR() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetSRConnection() nulls srProcess.
  const dying = srProcess && !srProcess.killed ? srProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('sr:connection:applied')
}
...
// profile to startSR() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startSR() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForSR(remote.baseUrl, remote.token)
  ...
  // --profile wins over the inherited SR_HOME env (see _apply_profile_override
  // step 3 in sr_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSRBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const srCwd = resolveSRCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting SR backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: srCwd,
        SR_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        SR_DESKTOP: '1',
        SR_WEB_DIST: webDist,
        ...(readyFile ? { SR_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`SR backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`SR backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`SR backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForSR(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `SR backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_sr_home() recreates the deleted profile directory.
...
async function startSR() {
  ...
  // process, every subsequent startSR() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving SR backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote SR backend at ${remote.baseUrl}`, 24)
      await waitForSR(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote SR backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.sr/active_profile file) and
    // resolves SR_HOME the same way `sr -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving SR runtime', 28)
    const backend = await ensureRuntime(resolveSRBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const srCwd = resolveSRCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting SR backend via ${backend.label}`, 84)
    rememberLog(`Starting SR backend via ${backend.label}`)
    ...
    srProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin SR_HOME for the child so Python's get_sr_home()
          // resolves to the SAME location our resolveSRHome() picked. Without
          // this pin, Python falls back to ~/.sr on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\sr, which differs from C:\Users\<u>\.sr.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets SR_HOME via setx; the desktop
    ...
    srProcess.stdout.on('data', rememberLog)
    srProcess.stderr.on('data', rememberLog)
    ...
    srProcess.once('error', error => {
      rememberLog(`SR backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `SR backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    srProcess.once('exit', (code, signal) => {
      rememberLog(`SR backend exited (${signal || code})`)
      srProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `SR backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `SR backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSRLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for SR backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(srProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for SR backend to become ready', 90)
    await Promise.race([waitForSR(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null srProcess when the child dies.
      childAlive: () => srProcess !== null && srProcess.exitCode === null && !srProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'SR backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while SR is
...
// pushes pet state over IPC (sr:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via sr:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main SR app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the SR icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see sr:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep SR in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('sr:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startSR().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('sr:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting SR…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('sr:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetSRConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote SR backend failed liveness probe; dropping stale connection.')
    resetSRConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('sr:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('sr:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('sr:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('sr:window:openNewSession', async () => {
...
ipcMain.handle('sr:zoom:get', event => {
...
ipcMain.on('sr:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('sr:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('sr:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('sr:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('sr:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('sr:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('sr:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('sr:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('sr:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('sr:pet-overlay:control', payload)
})
ipcMain.handle('sr:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSR() call restarts the
...
ipcMain.handle('sr:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startSR() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('sr:bootstrap:cancel', async () => {
...
ipcMain.handle('sr:boot-progress:get', async () => bootProgressState)
ipcMain.handle('sr:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('sr:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('sr:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('sr:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('sr:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('sr:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('sr:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('sr:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('sr:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on sr:connection:applied.
...
ipcMain.handle('sr:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('sr:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new SR_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('sr:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('sr:requestMicrophoneAccess', async () => {
...
ipcMain.handle('sr:api', async (_event, request) => {
  ...
  // backend calls ensure_sr_home() which recreates the profile directory,
...
ipcMain.handle('sr:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('sr:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('sr:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('sr:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('sr:readFileText', async (_event, filePath) => {
...
ipcMain.handle('sr:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('sr:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('sr:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('sr:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('sr:saveClipboardImage', async () => {
...
ipcMain.handle('sr:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('sr:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('sr:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('sr:titlebar-theme', (_event, payload) => {
...
ipcMain.on('sr:native-theme', (_event, mode) => {
...
ipcMain.on('sr:translucency', (_event, payload) => {
...
ipcMain.handle('sr:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('sr:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSRCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('sr:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSRCwd()
}))

ipcMain.handle('sr:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('sr:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('sr:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('sr:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('sr:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('sr:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: srLog.slice(-200) }))
...
function terminalShellCommand() {
  // SR_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.SR_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip SR' TUI into
  ...
  // Let a sr/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from SR_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.SR_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `sr:terminal:${id}:${suffix}`
}
...
ipcMain.handle('sr:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('sr:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('sr:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('sr:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('sr:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('sr:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('sr:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('sr:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('sr:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('sr:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('sr:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('sr:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('sr:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('sr:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('sr:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('sr:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sr:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sr:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('sr:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('sr:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('sr:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('sr:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('sr:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('sr:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('sr:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('sr:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('sr:terminal:write', (_event, id, data) => {
...
ipcMain.handle('sr:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('sr:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('sr:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('sr:updates:check', async () =>
...
ipcMain.handle('sr:updates:apply', async (_event, payload) =>
...
ipcMain.handle('sr:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('sr:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical SR version (the one `release.py` bumps in
// sr_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real SR version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSRVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'sr_cli', '__init__.py')
...
// Re-resolve the live SR version and push it into the native About panel
// just before showing it, so an in-place `sr update` is reflected without
...
ipcMain.handle('sr:version', async () => ({
  appVersion: resolveSRVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  srRoot: resolveUpdateRoot()
}))
...
// the actual removal via `sr uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (sr_cli/uninstall.py + sr_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_SR_ROOT
  ...
  const fallback = () => ({
    sr_home: SR_HOME,
    agent_installed: isSRSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'sr_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, SR_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no SR agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import sr_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_SR_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `sr-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `sr-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('sr:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('sr:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('sr:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('sr:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// sr:// deep links (e.g. sr://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const SR_PROTOCOL = 'sr'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${SR_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // sr://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('sr:deep-link', payload)
...
ipcMain.handle('sr:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${SR_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(SR_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(SR_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `sr://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching sr:// URL is in our own argv.
