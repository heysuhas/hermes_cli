// Regression guards for Windows `sr` resolution in main.ts.
//
...
//      Git-Bash `sr` shim shadowed the real sr.cmd/sr.exe; the
...
//      --repair by checking ONLY venv\Scripts\sr.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvSRCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow sr.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\sr.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvSR\) \?/,
    'recovery regressed to gating only on the sr.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvSRCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvSRCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvSRCommand must exist in main.ts')
