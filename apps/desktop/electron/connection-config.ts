 ...
 *     `X-SR-Session-Token` header; WS uses `?token=`.
...
// sr_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - sr_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - sr_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / sr #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-sr_session_at', '__Secure-sr_session_at', 'sr_session_at']
const RT_COOKIE_VARIANTS = ['__Host-sr_session_rt', '__Secure-sr_session_rt', 'sr_session_rt']

// The Nous portal (NAS) does NOT use SR gateway session cookies — it is a
...
// SR Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://sr.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a sr session ACCESS-token cookie
 ...
 * the SR gateway session cookies, so cloud sign-in / discovery liveness
