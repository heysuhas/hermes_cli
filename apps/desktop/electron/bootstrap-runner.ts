 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of SR Agent by spawning
...
function bootstrapCacheDir(srHome) {
  return path.join(srHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.sr/sr-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(srHome) {
  if (!srHome) {
    return null
  }
  const candidate = path.join(srHome, 'sr-agent', 'scripts', installScriptName())
...
function cachedScriptPath(srHome, commit) {
  return path.join(bootstrapCacheDir(srHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/SamsungResearch/sr-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(srHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(srHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, srHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass SR_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          SR_HOME: srHome || process.env.SR_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, srHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, srHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--sr-home', srHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, srHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, srHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(srHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, srHome, emit })
