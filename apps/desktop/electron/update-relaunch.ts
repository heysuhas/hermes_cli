 ...
 * Background
 * ----------
 * After `sr update` + `sr desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything SR_DESKTOP_* is preserved
// plus SR_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['SR_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['SR_DESKTOP_']
