...
export function markerPath(srHome) {
  return path.join(srHome, '.sr-update-in-progress')
}
 ...
 *
 * The Tauri-based sr-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(srHome, pid, { now = Date.now } = {}) {
  const file = markerPath(srHome)
