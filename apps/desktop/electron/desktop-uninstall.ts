 ...
 * the three user-facing uninstall modes to the `sr uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m sr_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `sr_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'sr_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/SR.app/Contents/MacOS/SR  → …/SR.app
 *   Windows: …\SR\SR.exe                 → …\SR  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/SR.app/Contents/MacOS/SR → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/SR.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs SR.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]SR$/i.test(dir) || /[\\/]sr-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/sr
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import sr_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, srHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import sr_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but SR install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "SR_HOME=${String(srHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
