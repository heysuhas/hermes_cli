...
// Backend subcommand routing for the desktop-managed SR process.
//
// The desktop app launches its own headless backend via `sr serve` — it
...
// app hasn't updated yet, or an older `sr` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional SR profile to pin via `--profile`.
 */
 ...
 * `-m sr_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `sr_cli/subcommands/dashboard.py` source registers
