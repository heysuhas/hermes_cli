 ...
 * product name (`SR`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/SR'
 *
 * This is a hard failure with no obvious cause for the user — `sr desktop`
 ...
 * (the mac-unpacked SR.app bundle) and Windows (win-unpacked), so we
