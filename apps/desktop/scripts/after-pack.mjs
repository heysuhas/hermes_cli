 ...
 *
 * Stamps the SR icon + identity onto the packed Windows SR.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `sr desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'SR')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'SR'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); SR.exe keeps the stock Electron icon`)
  }
}
