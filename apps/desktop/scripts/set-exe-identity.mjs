#!/usr/bin/env node
// set-exe-identity.mjs — stamp the SR icon + version metadata onto the
// built SR.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked SR.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `sr desktop`, the installer's
...
// Stamp the SR icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      LegalCopyright: 'Copyright (c) 2026 Samsung Research'
    }
  })

  console.log('[set-exe-identity] done — SR icon + identity stamped')
}
