...
export function setZoomPercent(percent: number): void {
  window.srDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.srDesktop?.zoom) {
  void window.srDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.srDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
