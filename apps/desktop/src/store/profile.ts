...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/sr'
...
import type { ProfileInfo } from '@/types/sr'
...
// /api/profiles/active `current`). "default" is the root ~/.sr. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'sr.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'sr.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.srDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new SR_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.srDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.srDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'sr.desktop.showAllProfiles'
...
// Switch to the default (root ~/.sr) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.srDesktop?.touchBackend?.(target).catch(() => undefined)
}
