...
import type { OAuthProvider } from '@/types/sr'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('sr-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('sr-desktop-onboarded-v1')).toBe('1')
  })
