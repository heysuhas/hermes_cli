...
import type { SRGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/sr'
...
const PROJECT_SCOPE_KEY = 'sr.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('SR gateway is not connected')
  }
...
// repo, returning where SR should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<SRGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.srDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.srDesktop?.writeClipboard?.(path)
  }
}
