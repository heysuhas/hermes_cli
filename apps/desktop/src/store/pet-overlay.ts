 ...
 * app's bounds and stays visible while SR is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'sr.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'sr.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.srDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.srDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.srDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.srDesktop?.petOverlay?.close()
}
