...
import type { SRRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: SRRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<SRRepoStatus | null>) {
  ;(window as unknown as { srDesktop?: unknown }).srDesktop = { git: { repoStatus: impl } }
}
