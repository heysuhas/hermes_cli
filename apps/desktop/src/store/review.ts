...
import type { SRReviewFile, SRReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — SR' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'sr.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'sr.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'sr.desktop.reviewTreeMode'
const SELECTED_KEY = 'sr.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<SRReviewFile[]>([])
...
export const $reviewShipInfo = atom<SRReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['srDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: SRReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.srDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.srDesktop?.openExternal?.(url)
    }
