...
const SR_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type SRRefType = (typeof SR_REF_TYPES)[number]
...
const ICON_PATHS: Record<SRRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SRRefType] ?? ICON_FALLBACK
...
const SR_DIRECTIVE_RE = new RegExp(
...
export const srDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(SR_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SRRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SRRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...srDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing SR directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.srDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.srDesktop?.readFileDataUrl(id)
