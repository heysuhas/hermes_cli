import type {
  SRConnection,
  SRReadDirResult,
  SRReadFileTextResult,
  SRSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: SRConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<SRReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<SRReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<SRReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<SRReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: SRSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
