...
/** Why a known SR command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active SR profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show SR Agent version', surface: exec() },
...
function isKnownSRSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * SR' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownSRSlashCommand(normalized)
}
