...
const branchActionLabel = (branch: SRGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `sr/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<SRGitBranch[]>([])
  ...
  const convert = async (branch: SRGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
