...
import { getSRConfig } from '@/sr'
import { persistString } from '@/lib/storage'
import { $currentCwd, setCurrentCwd } from '@/store/session'

import { useSRConfig } from './use-sr-config'

vi.mock('@/sr', () => ({
  getSRConfig: vi.fn(),
  getSRConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'sr.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getSRConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getSRConfig>>)

describe('useSRConfig refreshSRConfig', () => {
  beforeEach(() => {
  ...
  it('applies terminal.cwd from config even when localStorage has a stale value', async () => {
    ...
    await act(async () => {
      await result.current.refreshSRConfig()
    })
