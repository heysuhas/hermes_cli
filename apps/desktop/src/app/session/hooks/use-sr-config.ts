...
import { getSRConfig, getSRConfigDefaults } from '@/sr'
...
interface SRConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSRConfig({ activeSessionIdRef, refreshProjectBranch }: SRConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSRConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSRConfig(), getSRConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSRConfig, sttEnabled, voiceMaxRecordingSeconds }
}
