...
import { getElevenLabsVoices, getSRConfigSchema, saveSRConfig } from '@/sr'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, SRConfigRecord } from '@/types/sr'

import { setSRConfigCache, useSRConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: SRConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<SRConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useSRConfigRecord()
  ...
  const {
    ...
    queryKey: ['sr-config-schema'],
    queryFn: getSRConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: SRConfigRecord) => {
