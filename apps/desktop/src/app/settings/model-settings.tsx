...
import { invalidateSRConfig, setSRConfigCache, useSRConfigRecord } from '../hooks/use-config-record'
...
// SR' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = SR default (medium), shown as Medium.
...
// backend now surfaces the full `sr model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in sr_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useSRConfigRecord()
  const setConfig = setSRConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `sr model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — SR runs the flow for you.`}
          </p>
        )}
