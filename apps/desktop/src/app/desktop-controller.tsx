...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../sr'
...
import { useSRConfig } from './session/hooks/use-sr-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.srDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.srDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.srDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.srDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // sr:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.srDesktop?.onDeepLink?.(payload => {
    ...
    void window.srDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.srDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshSRConfig, sttEnabled, voiceMaxRecordingSeconds } = useSRConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshSRConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshSRConfig])
