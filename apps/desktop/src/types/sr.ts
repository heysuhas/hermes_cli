...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when SR doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in sr_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `sr model` uses — instead of
...
export type SRConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `sr model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors sr_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with SR, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `sr tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not SR); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/sr/update/check` — the backend's own update state.
