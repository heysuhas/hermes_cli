...
const SKIN_KEY = 'sr-desktop-theme-v2'
const MODE_KEY = 'sr-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'sr-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'sr-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'sr-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.srTheme = skinName
  root.dataset.srMode = rendered
  ...
  window.srDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('sr-boot-background', chromeBg)
    window.localStorage.setItem('sr-boot-color-scheme', rendered)
  } catch {
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.srDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active SR backend theme on connect. */
