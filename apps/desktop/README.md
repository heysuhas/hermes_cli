...
<p align="center">
  <a href="https://github.com/SamsungResearch/sr-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [SR Agent](../../README.md) — the self-improving AI agent from [Samsung Research](https://samsung.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other SR surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to SR and hear it back.</td></tr>
...
### Install with SR (recommended)

Already have the SR CLI? Just run:

```bash
sr desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. On first launch SR walks you through picking a provider and model; nothing else to configure.

### Prebuilt installers

Prebuilt installers are built and distributed via [the SR Desktop website.](https://sr-agent.samsung.com/).
...
```bash
# throwaway SR_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the SR Agent runtime into `SR_HOME`
(`~/.sr`, or `%LOCALAPPDATA%\sr` on Windows), using the same layout as a
CLI install.
...
- **SR Agent** runs as a headless `sr serve` process and exposes the
...
1. `SR_DESKTOP_SR_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `SR_DESKTOP_SR`, or `sr` on `PATH`
5. a system Python that can import the SR runtime
...
Desktop supports a managed local backend, explicit remote gateways, and SR
...
Boot logs land in `SR_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\sr\sr-agent\.sr-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\sr\sr-agent\venv"
```

> The default SR home on Windows is `%LOCALAPPDATA%\sr`. Set the `SR_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://sr-agent.samsung.com/docs/)
