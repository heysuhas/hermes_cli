...
How to build SR Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another SR surface can also
...
- A **runtime home change** (switching the underlying `SR_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across SR — do not assume
