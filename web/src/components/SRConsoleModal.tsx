...
export function SRConsoleModal({ open, onClose }: SRConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("sr> ");
  const inputPromptRef = useRef("sr> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "sr> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to SR Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="sr-console-title"
    >
      <div
        className={cn(
          themedBody,
