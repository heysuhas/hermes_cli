/**
 * SR Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__SR_PLUGIN_SDK__`` and
 * ``window.__SR_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__SR_PLUGIN_SDK__`` because:
 ...
 *
 * Versioning: bump ``SRPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@sr/dashboard-plugin-sdk`` types package, or
 ...
 * of calling ``fetch`` with a hand-read ``window.__SR_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__SR_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__SR_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface SRPluginSDK {
...
declare global {
  interface Window {
    __SR_PLUGIN_SDK__?: SRPluginSDK;
    __SR_PLUGINS__?: PluginRegistry;
  }
}
