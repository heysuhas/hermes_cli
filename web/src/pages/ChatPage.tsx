/**
 * ChatPage — embeds `sr --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (sr_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "sr.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__SR_SESSION_TOKEN__ &&
    !window.__SR_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `sr dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__SR_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner SR TUI has enabled xterm mouse-events
    ...
    //      payloads upload to SR_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to SR_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[sr-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__SR_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets SR_HOME pointed at the
    ...
    // forwarding them into SR. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="sr-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
