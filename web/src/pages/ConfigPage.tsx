...
export default function ConfigPage() {
  ...
  const [localPort, setLocalPort] = useState(() => (typeof window !== "undefined" && localStorage.getItem("hermes-local-port")) || "9119");
  const [localToken, setLocalToken] = useState(() => (typeof window !== "undefined" && localStorage.getItem("hermes-local-token")) || "");

  const handleSaveLocalConfig = () => {
    localStorage.setItem("hermes-local-port", localPort);
    localStorage.setItem("hermes-local-token", localToken);
    showToast("Connection configuration saved. Reloading page...", "success");
    setTimeout(() => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!isLocal && (
        <div className="rounded-lg border border-border bg-card text-card-foreground shadow-sm p-4 flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-xs text-muted-foreground">
              Since the dashboard is hosted centrally, it must connect to your local Hermes instance on loopback.
              Please configure the API port and session token configured on your machine.
            </p>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Local API Port</label>
              <Input
                value={localPort}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-muted-foreground">Local Session Token</label>
              <Input
                value={localToken}
                onChange={(e) => setLocalToken(e.target.value)}
                placeholder="Session Token / API Key"
                className="h-8 text-xs"
                type="password"
              />
            </div>
          ...
          <div className="flex justify-end">
            <Button size="sm" onClick={handleSaveLocalConfig}>
              Save and Connect
            </Button>
          </div>
        </div>
