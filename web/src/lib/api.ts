...
export function resolveBackendBase(): string {
  ...
  if (isLocal) {
    return HERMES_BASE_PATH;
  }
  const port = localStorage.getItem("hermes-local-port") || "9119";
  return `http://127.0.0.1:${port}${HERMES_BASE_PATH}`;
}

export function getSyncSessionToken(): string | undefined {
  if (typeof window === "undefined") return undefined;
  const isLocal = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
  if (isLocal) {
    return window.__HERMES_SESSION_TOKEN__;
  }
  return localStorage.getItem("hermes-local-token") || undefined;
}
...
export async function fetchJSON<T>(
  ...
  const token = getSyncSessionToken();
  if (token) {
...
export async function buildWsUrl(
  ...
  const isLocal = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1";
  const localPort = localStorage.getItem("hermes-local-port") || "9119";
  ...
  if (isLocal) {
    ...
    return `ws://127.0.0.1:${localPort}${BASE}${path}?${qs}`;
  }
}
