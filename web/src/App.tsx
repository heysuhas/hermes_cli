          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="sr-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="sr-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateSR = status?.can_update_sr === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'sr update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "sr update";
    return (
      t.status.updateSRConfirmMessage ??
      `This will run 'sr update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateSRConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateSR) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateSR,
      runningLabel: t.status.updatingSR,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the SR gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateSRConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateSRConfirmTitle ?? `${t.status.updateSR}?`}
    />
    </>
  );
}
