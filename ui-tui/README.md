...
React + Ink terminal UI for SR. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SR_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `sr setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.sr/.sr_history` or under `SR_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `sr setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/sr-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external sr setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.sr/tui-parent.log
    ...
    types/
      sr-ink.d.ts            type declarations for @sr/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent SRCLI subprocess for slash commands
```
