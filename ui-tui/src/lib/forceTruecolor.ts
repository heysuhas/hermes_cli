...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.SR_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep SR on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via SR_TUI_TRUECOLOR=1.
