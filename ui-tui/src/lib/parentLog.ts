...
const logDir = join(process.env.SR_HOME?.trim() || join(homedir(), '.sr'), 'logs')
...
// a real ~/.sr (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('sr-tui: parent lifecycle log unavailable\n')
    }
  }
}
