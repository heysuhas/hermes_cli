import type { RunExternalProcess } from '@sr/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`sr ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching sr: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`sr ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
